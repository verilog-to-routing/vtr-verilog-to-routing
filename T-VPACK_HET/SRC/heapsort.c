...
static int get_top_of_heap_index (int *heap, float *sort_values, int
          heap_tail); 
 ...
 * decreasing as one goes through sort_index.  Sort_values is not changed.   */
 ...
 /*index the arrays from 1 rather than 0*/
 sort_index--;
 sort_values--;
 ...
 * overlap, so I get a nice sort-in-place.                                   */
 ...
 /*restore array pointers (not necessary since they are local, but I feel *
 ...
 * before the top item is deleted.                                           */
 ...
 /*index must have one subracted to be consistent with the original array*/
