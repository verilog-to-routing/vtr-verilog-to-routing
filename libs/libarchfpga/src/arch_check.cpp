        ...
        for (t_model_ports* port = ports; port != nullptr; port = port->next) {
            if (!port->clock.empty() && !clocks.count(port->clock)) {
                archfpga_throw(file, line,
                               ...
                               port->clock.c_str(), model.name.c_str(), port->name.c_str());
            }
        }
    }
    return true;
    ...
    for (t_model_ports* port = model.outputs; port != nullptr; port = port->next) {
        if (!port->combinational_sink_ports.empty()) {
            archfpga_throw(file, line,
                           ...
                           model.name.c_str(), port->name.c_str());
        }
    }
        ...
        for (const std::string& sink_port_name : port->combinational_sink_ports) {
            ...
            if (!output_ports.count(sink_port_name)) {
                archfpga_throw(file, line,
                               ...
                               model.name.c_str(), port->name.c_str(), sink_port_name.c_str());
            }
            ...
            if (sink_port->is_clock) {
                archfpga_throw(file, line,
                               ...
                               model.name.c_str(), sink_port->name.c_str(), sink_port->is_clock, port->name.c_str());
            }
        }
    }
                    ...
                    for (const std::string& output_pin : vtr::StringToken(annotation.output_pins).split(" \t\n")) {
                        ...
                        if (iter == e) {
                            archfpga_throw(get_arch_file_name(), annotation.line_num,
                                           ...
                                           model_port->name.c_str(), model.name.c_str(), annot_out.port_name().c_str());
                        }
                    }
                }
            } else {
