...
static void PrintPb_types_recPower(FILE* Echo, const t_pb_type* pb_type, const char* tabs);
...
void PrintArchInfo(FILE* Echo, const t_arch* arch) {
    ...
    switch (arch->Chans.chan_x_dist.type) {
        ...
        case (GAUSSIAN):
            fprintf(Echo,
                    ...
                    arch->Chans.chan_x_dist.peak, arch->Chans.chan_x_dist.width, arch->Chans.chan_x_dist.xpeak,
        ...
        case (DELTA):
            fprintf(Echo,
                    "\t\tx: distr dleta peak %e \
						  Xpeak %e dc %e\n",
                    arch->Chans.chan_x_dist.peak, arch->Chans.chan_x_dist.xpeak, arch->Chans.chan_x_dist.dc);
            break;
        default:
        ...
        case (GAUSSIAN):
            fprintf(Echo,
                    "\t\ty: type gaussian peak %e \
						  width %e Xpeak %e dc %e\n",
                    arch->Chans.chan_y_dist.peak, arch->Chans.chan_y_dist.width, arch->Chans.chan_y_dist.xpeak,
        ...
        case (DELTA):
            fprintf(Echo,
                    "\t\ty: distr dleta peak %e \
						  Xpeak %e dc %e\n",
                    arch->Chans.chan_y_dist.peak, arch->Chans.chan_y_dist.xpeak, arch->Chans.chan_y_dist.dc);
            break;
        default:
            fprintf(Echo, "\t\tInvalid Distribution!\n");
            break;
    ...
    for (i = 0; i < arch->num_switches; i++) {
        ...
        fprintf(Echo, "\t\t\t\tR %e Cin %e Cout %e\n", arch->Switches[i].R, arch->Switches[i].Cin,
                arch->Switches[i].Cout);
        fprintf(Echo, "\t\t\t\t#Tdel values %d buf_size %e mux_trans_size %e\n",
                (int)arch->Switches[i].Tdel_map_.size(), arch->Switches[i].buf_size, arch->Switches[i].mux_trans_size);
    ...
    for (i = 0; i < (int)(arch->Segments).size(); i++) {
        ...
        fprintf(Echo, "\tSegment[%d]: frequency %d length %d R_metal %e C_metal %e\n", i + 1, seg.frequency, seg.length,
    ...
    for (i = 0; i < arch->num_directs; i++) {
        fprintf(Echo, "\tDirect[%d]: name %s from_pin %s to_pin %s\n", i + 1, arch->Directs[i].name,
                arch->Directs[i].from_pin, arch->Directs[i].to_pin);
        fprintf(Echo, "\t\t\t\t x_offset %d y_offset %d z_offset %d\n", arch->Directs[i].x_offset,
                arch->Directs[i].y_offset, arch->Directs[i].sub_tile_offset);
    }
    fprintf(Echo, "*************************************************\n\n");
    ...
    fprintf(Echo, "Power:\n");
    if (arch->power) {
        fprintf(Echo, "\tlocal_interconnect C_wire %e factor %f\n", arch->power->C_wire_local,
                arch->power->local_interc_factor);
    ...
    if (arch->clocks) {
        for (i = 0; i < arch->clocks->num_global_clocks; i++) {
            if (arch->clocks->clock_inf[i].autosize_buffer) {
                fprintf(Echo, "\tClock[%d]: buffer_size auto C_wire %e", i + 1, arch->clocks->clock_inf->C_wire);
            } else {
                fprintf(Echo, "\tClock[%d]: buffer_size %e C_wire %e", i + 1, arch->clocks->clock_inf[i].buffer_size,
                        arch->clocks->clock_inf[i].C_wire);
            }
            fprintf(Echo, "\t\t\t\tstat_prob %f switch_density %f period %e", arch->clocks->clock_inf[i].prob,
                    arch->clocks->clock_inf[i].dens, arch->clocks->clock_inf[i].period);
        }
    }
...
static void PrintPb_types_rec(FILE* Echo, const t_pb_type* pb_type, int level) {
    ...
    for (i = 0; i < pb_type->num_ports; i++) {
        fprintf(Echo, "%s\tport %s type %d num_pins %d\n", tabs, pb_type->ports[i].name, pb_type->ports[i].type,
    ...
    if (pb_type->num_modes > 0) { /*one or more modes*/
        ...
        if (strcmp(pb_type->model->name, MODEL_NAMES) && strcmp(pb_type->model->name, MODEL_INPUT)
            && strcmp(pb_type->model->name, MODEL_OUTPUT)) {
            for (k = 0; k < pb_type->num_annotations; k++) {
                fprintf(Echo, "%s\t\t\tannotation %s %s %s %d: %s\n", tabs, pb_type->annotations[k].clock,
                        pb_type->annotations[k].input_pins, pb_type->annotations[k].output_pins,
                        pb_type->annotations[k].format, pb_type->annotations[k].value[0]);
            }
        }
    }
    ...
    switch (pb_type->pb_type_power->estimation_method) {
        ...
        case POWER_METHOD_IGNORE:
            if (pb_type->parent_mode) {
                ...
                if (pb_type->parent_mode->parent_pb_type->pb_type_power->estimation_method == POWER_METHOD_IGNORE)
                    break;
            }
        ...
        case POWER_METHOD_AUTO_SIZES:
            if (pb_type->parent_mode) {
                /*if NOT top-level pb (all top-level pb has NULL parent_mode, check parent's power method
                 * This is because of the inheritance property of auto-size*/
                if (pb_type->parent_mode->parent_pb_type->pb_type_power->estimation_method == POWER_METHOD_AUTO_SIZES)
                    break;
            }
        ...
        case POWER_METHOD_SPECIFY_SIZES:
            ...
            for (i = 0; i < pb_type->num_ports; i++) {
                ...
                if (pb_type->ports[i].port_power->buffer_type || pb_type->ports[i].port_power->wire_type
                    || pb_type->pb_type_power->absolute_power_per_instance.leakage
                    || pb_type->pb_type_power->absolute_power_per_instance.dynamic) {
                    fprintf(Echo, "%s\t\tport %s type %d num_pins %d\n", tabs, pb_type->ports[i].name,
                            pb_type->ports[i].type, pb_type->ports[i].num_pins);
                    //Buffer size
                    switch (pb_type->ports[i].port_power->buffer_type) {
                        ...
                        case (POWER_BUFFER_TYPE_ABSOLUTE_SIZE):
                            fprintf(Echo, "%s\t\t\tbuffer_size %f\n", tabs, pb_type->ports[i].port_power->buffer_size);
                            break;
                        default:
                            break;
                    }
                    switch (pb_type->ports[i].port_power->wire_type) {
                        ...
                        case (POWER_WIRE_TYPE_C):
                            fprintf(Echo, "%s\t\t\twire_cap: %e\n", tabs, pb_type->ports[i].port_power->wire.C);
                            break;
        ...
        case POWER_METHOD_TOGGLE_PINS:
            if (pb_type->parent_mode) {
                ...
                if (pb_type->parent_mode->parent_pb_type->pb_type_power->estimation_method == POWER_METHOD_TOGGLE_PINS)
                    break;
            }
            ...
            for (i = 0; i < pb_type->num_ports; i++) {
                ...
                if (pb_type->ports[i].port_power->energy_per_toggle || pb_type->ports[i].port_power->scaled_by_port
                    ...
                    if (pb_type->ports[i].port_power->scaled_by_port) {
                        if (pb_type->ports[i].port_power->scaled_by_port->num_pins > 1) {
                            fprintf(Echo,
                                    (pb_type->ports[i].port_power->reverse_scaled
                                         ? "%s\t\t\tscaled_by_static_prob_n: %s[%d]\n"
                                         : "%s\t\t\tscaled_by_static_prob: %s[%d]\n"),
                                    tabs, pb_type->ports[i].port_power->scaled_by_port->name,
        ...
        case POWER_METHOD_C_INTERNAL:
            if (pb_type->parent_mode) {
                ...
                if (pb_type->parent_mode->parent_pb_type->pb_type_power->estimation_method == POWER_METHOD_C_INTERNAL)
                    break;
            }
        ...
        case POWER_METHOD_ABSOLUTE:
            if (pb_type->parent_mode) {
                /*if NOT top-level pb (all top-level pb has NULL parent_mode, check parent's power method
                 * This is because of values at this level includes all children pb's*/
                if (pb_type->parent_mode->parent_pb_type->pb_type_power->estimation_method == POWER_METHOD_ABSOLUTE)
                    break;
            }
