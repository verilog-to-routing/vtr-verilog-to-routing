...
SwitchType t_arch_switch_inf::type() const { return type_; }

bool t_arch_switch_inf::buffered() const { return switch_type_is_buffered(type()); }

bool t_arch_switch_inf::configurable() const { return switch_type_is_configurable(type()); }

e_directionality t_arch_switch_inf::directionality() const { return switch_type_directionaity(type()); }
...
bool t_arch_switch_inf::fixed_Tdel() const { return Tdel_map_.size() == 1 && Tdel_map_.count(UNDEFINED_FANIN); }

void t_arch_switch_inf::set_Tdel(int fanin, float delay) { Tdel_map_[fanin] = delay; }

void t_arch_switch_inf::set_type(SwitchType type_val) { type_ = type_val; }

/*
 * t_rr_switch_inf
 */

SwitchType t_rr_switch_inf::type() const { return type_; }

bool t_rr_switch_inf::buffered() const { return switch_type_is_buffered(type()); }

bool t_rr_switch_inf::configurable() const { return switch_type_is_configurable(type()); }

void t_rr_switch_inf::set_type(SwitchType type_val) { type_ = type_val; }

static bool switch_type_is_buffered(SwitchType type) {
    //Muxes and Tristates isolate thier input and output into
    //seperate DC connected sub-circuits
    return type == SwitchType::MUX || type == SwitchType::TRISTATE || type == SwitchType::BUFFER;
}

static bool switch_type_is_configurable(SwitchType type) {
    //Shorts and buffers are non-configurable
    return !(type == SwitchType::SHORT || type == SwitchType::BUFFER);
}
...
static e_directionality switch_type_directionaity(SwitchType type) {
    if (type == SwitchType::SHORT || type == SwitchType::PASS_GATE) {
        ...
        VTR_ASSERT_SAFE(type == SwitchType::MUX || type == SwitchType::TRISTATE || type == SwitchType::BUFFER);
...
bool t_physical_tile_type::is_empty() const { return std::string(name) == std::string(EMPTY_BLOCK_NAME); }

/*
 * t_logical_block_type
 */

bool t_logical_block_type::is_empty() const { return std::string(name) == std::string(EMPTY_BLOCK_NAME); }
