...
float Timer::elapsed_sec() const { return std::chrono::duration<float>(clock::now() - start_).count(); }
...
float Timer::delta_max_rss_mib() const { return (get_max_rss() - initial_max_rss_) / BYTE_TO_MIB; }
...
///@brief Destructor
ScopedActionTimer::~ScopedActionTimer() { --f_timer_depth; }

///@brief Sets quiet value (when true, prints the timing info)
void ScopedActionTimer::quiet(bool value) { quiet_ = value; }

///@brief Returns the quiet value
bool ScopedActionTimer::quiet() const { return quiet_; }

///@brief Returns the action string
std::string ScopedActionTimer::action() const { return action_; }
...
int ScopedActionTimer::depth() const { return depth_; }
...
ScopedStartFinishTimer::~ScopedStartFinishTimer() {
    if (!quiet()) {
        vtr::printf_info("%s%s took %.2f seconds (max_rss %.1f MiB, delta_rss %+.1f MiB)\n", pad().c_str(),
                         action().c_str(), elapsed_sec(), max_rss_mib(), delta_max_rss_mib());
    }
}
