...
class SetupTimingAnalyzer : public virtual TimingAnalyzer {
    public:
        ...
        const TimingTags& get_setup_launch_clock_tags(NodeId node_id) const { return get_setup_launch_clock_tags_impl(node_id); }
        const TimingTags& get_setup_capture_clock_tags(NodeId node_id) const { return get_setup_capture_clock_tags_impl(node_id); }

    protected:
        virtual const TimingTags& get_setup_data_tags_impl(NodeId node_id) const = 0;
        virtual const TimingTags& get_setup_launch_clock_tags_impl(NodeId node_id) const = 0;
        virtual const TimingTags& get_setup_capture_clock_tags_impl(NodeId node_id) const = 0;
