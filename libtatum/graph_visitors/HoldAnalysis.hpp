...
class HoldAnalysis : public detail::CommonAnalysisVisitor<detail::HoldAnalysisOps> {
    ...
    public:
        ...
        const TimingTags& get_hold_launch_clock_tags(const NodeId node_id) const { return ops_.get_launch_clock_tags(node_id); }
        const TimingTags& get_hold_capture_clock_tags(const NodeId node_id) const { return ops_.get_capture_clock_tags(node_id); }
};
