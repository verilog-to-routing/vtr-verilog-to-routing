...
Lut::Lut(size_t num_inputs)
    : num_inputs_(num_inputs)
    , table_(1 << num_inputs, vtr::LogicValue::DONT_CARE) {}
...
LutOutputDefinition::LutOutputDefinition(std::string definition) {
    ...
    if (slice_start == std::string::npos || slice == std::string::npos || slice_end == std::string::npos || slice_start + 1 > slice - 1 || slice + 1 > slice_end - 1) {
        vpr_throw(
