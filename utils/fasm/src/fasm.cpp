...
FasmWriterVisitor::FasmWriterVisitor(std::ostream& f)
    : os_(f) {}
...
static const t_metadata_dict* get_fasm_type(const t_pb_graph_node* pb_graph_node, std::string target_type) {
    ...
    if (pb_graph_node->pb_type->parent_mode != nullptr && pb_graph_node->pb_type->parent_mode->meta.has("fasm_type")) {
...
void FasmWriterVisitor::check_for_param(const t_pb* atom) {
    ...
    if (atom->pb_graph_node == nullptr || atom->pb_graph_node->pb_type == nullptr) {
        return;
    }
...
void FasmWriterVisitor::output_fasm_mux(std::string fasm_mux,
    ...
    for (const auto& mux_input : mux_inputs) {
        ...
        bool root_level_connection = interconnect->parent_mode->parent_pb_type == mux_input_pin->parent_node->pb_type;
        ...
        } else if (mux_pb_name == pb_name && mux_pb_index == pb_index && mux_port_name == port_name && mux_pin_index == pin_index) {
            if (mux_parts[1] != "NULL") {
                output_fasm_features(mux_parts[1]);
            }
            return;
