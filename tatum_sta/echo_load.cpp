...
tatum::TimingGraph EchoLoader::timing_graph() {
    return std::move(tg_);
}

tatum::TimingConstraints EchoLoader::timing_constraints() {
    return std::move(tc_);
}

tatum::FixedDelayCalculator EchoLoader::delay_calculator() {
    return tatum::FixedDelayCalculator(max_delay_edges_, setup_times_, min_delay_edges_, hold_times_);
}
