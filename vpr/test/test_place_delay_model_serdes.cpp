...
TEST_CASE("round_trip_delta_delay_model", "[vpr]") {
    ...
    DeltaDelayModel model(min_cross_layer_delay, std::move(delays), false);
    const auto& delays1 = model.delays();

    model.write(kDeltaDelayBin);

    DeltaDelayModel model2(min_cross_layer_delay, false);
...
TEST_CASE("round_trip_override_delay_model", "[vpr]") {
    ...
    auto base_model = std::make_unique<DeltaDelayModel>(min_cross_layer_delay, delays, false);
    ...
    OverrideDelayModel model2(min_cross_layer_delay, false);
