...
static float do_one_route(RRNodeId source_node,
    ...
    auto router_lookahead
        = make_router_lookahead(det_routing_arch, router_opts.lookahead_type, router_opts.write_router_lookahead,
                                router_opts.read_router_lookahead, segment_inf, is_flat);

    ConnectionRouter<FourAryHeap> router(device_ctx.grid, *router_lookahead, device_ctx.rr_graph.rr_nodes(),
                                         &device_ctx.rr_graph, device_ctx.rr_rc_data, device_ctx.rr_graph.rr_switch(),
                                         g_vpr_ctx.mutable_routing().rr_node_route_inf, is_flat);
    ...
    if (found_path) {
        ...
        std::tie(std::ignore, rt_node_of_sink)
            = tree.update_from_heap(&cheapest, OPEN, nullptr, router_opts.flat_routing);
...
TEST_CASE("connection_router", "[vpr]") {
    ...
    alloc_routing_structs(chan_width, vpr_setup.RouterOpts, &vpr_setup.RoutingArch, vpr_setup.Segments, arch.Directs,
                          arch.num_directs, router_opts.flat_routing);
    ...
    // Find the route
    float delay
        = do_one_route(source_rr_node, sink_rr_node, vpr_setup.RoutingArch, vpr_setup.RouterOpts, vpr_setup.Segments);
