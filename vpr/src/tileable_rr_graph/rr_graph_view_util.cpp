...
std::vector<RRNodeId> find_rr_graph_grid_nodes(const RRGraphView& rr_graph,
    ...
    for (int pin = 0; pin < device_grid[x][y].type->num_pins; ++pin) {
        ...
        if (true == device_grid[x][y].type->is_ignored_pin[pin]) {
            ...
            if (!include_clock || std::find(device_grid[x][y].type->get_clock_pin_indices().begin(), device_grid[x][y].type->get_clock_pin_indices().end(), pin) != device_grid[x][y].type->get_clock_pin_indices().end()) {
                continue;
            }
        }
