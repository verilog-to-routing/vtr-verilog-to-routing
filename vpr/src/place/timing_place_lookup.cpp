...
bool directconnect_exists(int src_rr_node, int sink_rr_node) {
    ...
    VTR_ASSERT(device_ctx.rr_graph.node_type(RRNodeId(src_rr_node)) == SOURCE && device_ctx.rr_graph.node_type(RRNodeId(sink_rr_node)) == SINK);
    ...
    for (t_edge_size i_src_edge = 0; i_src_edge < rr_nodes[src_rr_node].num_edges(); ++i_src_edge) {
        ...
        if (device_ctx.rr_graph.node_type(RRNodeId(opin_rr_node)) != OPIN) continue;
        ...
        for (t_edge_size i_opin_edge = 0; i_opin_edge < rr_nodes[opin_rr_node].num_edges(); ++i_opin_edge) {
            ...
            if (device_ctx.rr_graph.node_type(RRNodeId(ipin_rr_node)) != IPIN) continue;
