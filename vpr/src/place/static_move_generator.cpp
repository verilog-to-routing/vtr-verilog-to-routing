...
e_create_move StaticMoveGenerator::propose_move(t_pl_blocks_to_be_moved& blocks_affected, float rlim, std::vector<int>& X_coord, std::vector<int>& Y_coord, int& type, const t_placer_opts& placer_opts, const PlacerCriticalities* criticalities) {
    float rand_num = vtr::frand() * total_prob;
    for (size_t i = 0; i < cumm_move_probs.size(); i++) {
        if (rand_num <= cumm_move_probs[i]) {
            type = i;
            return avail_moves[i]->propose_move(blocks_affected, rlim, X_coord, Y_coord, type, placer_opts, criticalities);
        }
    }
    ...
    return avail_moves[avail_moves.size() - 1]->propose_move(blocks_affected, rlim, X_coord, Y_coord, type, placer_opts, criticalities);
}
