...
void zero_initialize_grid_blocks() {
    ...
    for (size_t i = 0; i < device_ctx.grid.width(); i++) {
        for (size_t j = 0; j < device_ctx.grid.height(); j++) {
            ...
            auto tile = device_ctx.grid.get_physical_type(t_physical_tile_loc(i, j));
        ...
        for (size_t j = 0; j < device_ctx.grid.height(); j++) {
            ...
            for (auto sub_tile : tile->sub_tiles) {
                ...
                for (int k = 0; k < capacity.total(); k++) {
                    ...
                    if (device_ctx.grid.get_width_offset(t_physical_tile_loc(i, j)) == 0 && device_ctx.grid.get_height_offset(t_physical_tile_loc(i, j)) == 0) {
