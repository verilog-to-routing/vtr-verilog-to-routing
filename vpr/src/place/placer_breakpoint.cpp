...
std::map<int, std::string> available_move_types = {{0, "Uniform"}};
...
void transform_blocks_affected(const t_pl_blocks_to_be_moved& blocksAffected) {
    ...
    for (size_t i = 0; i < blocksAffected.moved_blocks.size(); i++) {
        ...
        get_bp_state_globals()->get_glob_breakpoint_state()->blocks_affected_by_move.push_back(
            size_t(blocksAffected.moved_blocks[i].block_num));
    }
}
...
void stop_placement_and_check_breakpoints(t_pl_blocks_to_be_moved& blocks_affected,
    ...
    if (draw_state->list_of_breakpoints.size() != 0) {
        ...
        get_bp_state_globals()->get_glob_breakpoint_state()->from_block
            = size_t(blocks_affected.moved_blocks[0].block_num);
        ...
        if (placer_breakpoint_reached())
            breakpoint_info_window(get_bp_state_globals()->get_glob_breakpoint_state()->bp_description,
                                   *get_bp_state_globals()->get_glob_breakpoint_state(), true);
    } else
    ...
    if (placer_breakpoint_reached() && draw_state->show_graphics) {
        ...
        msg += vtr::string_fmt(", Delta_cost: %1.6f (bb_delta_cost= %1.5f , timing_delta_c= %6.1e)", delta_c,
