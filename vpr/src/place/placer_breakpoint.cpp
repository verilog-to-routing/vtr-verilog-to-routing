...
static void transform_blocks_affected(const t_pl_blocks_to_be_moved& blocksAffected);
...
static void transform_blocks_affected(const t_pl_blocks_to_be_moved& blocksAffected) {
    ...
    for (const t_pl_moved_block& moved_block : blocksAffected.moved_blocks) {
        // size_t conversion is required since block_num is of type ClusterBlockId
        // and can't be cast to an int. And this vector has to be of type int to be recognized in expr_eval class
