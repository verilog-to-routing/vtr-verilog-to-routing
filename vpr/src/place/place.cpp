...
static void alloc_and_load_placement_structs(float place_cost_exp,
		const t_placer_opts &placer_opts, t_direct_inf *directs,
		int num_directs);
...
static e_move_result try_swap(const t_annealing_state *state,
		t_placer_costs *costs, MoveGenerator &move_generator,
		ManualMoveGenerator &manual_move_generator,
		SetupTimingInfo *timing_info,
		ClusteredPinTimingInvalidator *pin_timing_invalidator,
		t_pl_blocks_to_be_moved &blocks_affected,
		const PlaceDelayModel *delay_model, PlacerCriticalities *criticalities,
		PlacerSetupSlacks *setup_slacks, const t_placer_opts &placer_opts,
		MoveTypeStat &move_type_stat, const t_place_algorithm &place_algorithm,
...
static float starting_t(const t_annealing_state *state, t_placer_costs *costs,
		t_annealing_sched annealing_sched, const PlaceDelayModel *delay_model,
		PlacerCriticalities *criticalities, PlacerSetupSlacks *setup_slacks,
		SetupTimingInfo *timing_info, MoveGenerator &move_generator,
		ManualMoveGenerator &manual_move_generator,
		ClusteredPinTimingInvalidator *pin_timing_invalidator,
		t_pl_blocks_to_be_moved &blocks_affected,
		const t_placer_opts &placer_opts, MoveTypeStat &move_type_stat);
...
static int find_affected_nets_and_update_costs(
		...
		const PlaceDelayModel *delay_model,
		const PlacerCriticalities *criticalities,
		t_pl_blocks_to_be_moved &blocks_affected, double &bb_delta_c,
...
static void update_net_bb(const ClusterNetId net,
		const t_pl_blocks_to_be_moved &blocks_affected, int iblk,
		const ClusterBlockId blk, const ClusterPinId blk_pin);
static void update_td_delta_costs(const PlaceDelayModel *delay_model,
		const PlacerCriticalities &criticalities, const ClusterNetId net,
		const ClusterPinId pin, t_pl_blocks_to_be_moved &blocks_affected,
...
static void outer_loop_update_timing_info(const t_placer_opts &placer_opts,
		t_placer_costs *costs, int num_connections, float crit_exponent,
		int *outer_crit_iter_count, const PlaceDelayModel *delay_model,
		PlacerCriticalities *criticalities, PlacerSetupSlacks *setup_slacks,
		ClusteredPinTimingInvalidator *pin_timing_invalidator,
		SetupTimingInfo *timing_info);

static void placement_inner_loop(const t_annealing_state *state,
		const t_placer_opts &placer_opts, int inner_recompute_limit,
		t_placer_statistics *stats, t_placer_costs *costs,
		int *moves_since_cost_recompute,
		ClusteredPinTimingInvalidator *pin_timing_invalidator,
		const PlaceDelayModel *delay_model, PlacerCriticalities *criticalities,
		PlacerSetupSlacks *setup_slacks, MoveGenerator &move_generator,
		ManualMoveGenerator &manual_move_generator,
		t_pl_blocks_to_be_moved &blocks_affected, SetupTimingInfo *timing_info,
		const t_place_algorithm &place_algorithm, MoveTypeStat &move_type_stat,
		float timing_bb_factor);

static void recompute_costs_from_scratch(const t_placer_opts &placer_opts,
		const PlaceDelayModel *delay_model,
		const PlacerCriticalities *criticalities, t_placer_costs *costs);
...
static void calculate_reward_and_process_outcome(
		...
		const MoveOutcomeStats &move_outcome_stats, const double &delta_c,
		float timing_bb_factor, MoveGenerator &move_generator);

static void print_place_status_header();

static void print_place_status(const t_annealing_state &state,
		const t_placer_statistics &stats, float elapsed_sec, float cpd,
		float sTNS, float sWNS, size_t tot_moves);
...
void try_place(const t_placer_opts &placer_opts,
		t_annealing_sched annealing_sched, const t_router_opts &router_opts,
		const t_analysis_opts &analysis_opts, t_chan_width_dist chan_width_dist,
		t_det_routing_arch *det_routing_arch,
		std::vector<t_segment_inf> &segment_inf, t_direct_inf *directs,
		int num_directs) {
	...
	if (placer_opts.place_algorithm.is_timing_driven()) {
		...
		pin_timing_invalidator =
				std::make_unique<ClusteredPinTimingInvalidator>(
						cluster_ctx.clb_nlist, netlist_pin_lookup,
		...
		if (isEchoFileEnabled(E_ECHO_INITIAL_PLACEMENT_TIMING_GRAPH)) {
			...
			write_setup_timing_graph_dot(
					getEchoFileName(E_ECHO_INITIAL_PLACEMENT_TIMING_GRAPH)
							+ std::string(".dot"), *timing_info, debug_tnode);
		}
...
static void recompute_costs_from_scratch(const t_placer_opts &placer_opts,
	...
	if (placer_opts.place_algorithm.is_timing_driven()) {
		...
		if (fabs(
				new_timing_cost
						- costs->timing_cost) > costs->timing_cost * ERROR_TOL) {
			std::string msg =
					vtr::string_fmt(
...
static e_move_result try_swap(const t_annealing_state *state,
	...
	if (create_move_outcome == e_create_move::ABORT) {
		...
		if (move_outcome == ACCEPTED) {
			...
			//Highlights the new block in the new location if the manual move was accepted.
...
static void update_td_delta_costs(const PlaceDelayModel *delay_model,
	...
	auto &connection_timing_cost =
			g_placer_ctx.mutable_timing().connection_timing_cost;
	auto &proposed_connection_delay =
			g_placer_ctx.mutable_timing().proposed_connection_delay;
	auto &proposed_connection_timing_cost =
			g_placer_ctx.mutable_timing().proposed_connection_timing_cost;
	...
	if (cluster_ctx.clb_nlist.pin_type(pin) == PinType::DRIVER) {
		...
		for (size_t ipin = 1; ipin < cluster_ctx.clb_nlist.net_pins(net).size();
			...
			proposed_connection_timing_cost[net][ipin] =
					criticalities.criticality(net, ipin) * temp_delay;
...
static void commit_td_cost(const t_pl_blocks_to_be_moved &blocks_affected) {
	...
	auto &proposed_connection_timing_cost =
			p_timing_ctx.proposed_connection_timing_cost;
	...
	for (ClusterPinId pin_id : blocks_affected.affected_pins) {
		...
		connection_delay[net_id][ipin] =
				proposed_connection_delay[net_id][ipin];
		proposed_connection_delay[net_id][ipin] = INVALID_DELAY;
		connection_timing_cost[net_id][ipin] =
				proposed_connection_timing_cost[net_id][ipin];
	...
	if (placer_opts.place_algorithm.is_timing_driven()) {
		...
		for (auto net_id : cluster_ctx.clb_nlist.nets()) {
			for (ipin = 1; ipin < cluster_ctx.clb_nlist.net_pins(net_id).size();
				...
				p_timing_ctx.proposed_connection_delay[net_id][ipin] =
						INVALID_DELAY;

				p_timing_ctx.proposed_connection_timing_cost[net_id][ipin] =
						INVALID_DELAY;

				if (cluster_ctx.clb_nlist.net_is_ignored(net_id))
					continue;

				p_timing_ctx.connection_timing_cost[net_id][ipin] =
						INVALID_DELAY;
			}
		}
	}
...
static void print_place_status(const t_annealing_state &state,
	...
	VTR_LOG("%4zu "
			...
			"%7.3f %10.2f %-10.5g "
			"%7.3f % 10.3g % 8.3f "
			"%7.3f %7.4f %6.1f %8.2f", state.num_temps, elapsed_sec, state.t,
...
static void print_resources_utilization() {
	...
	std::map<t_logical_block_type_ptr,
			std::map<t_physical_tile_type_ptr, size_t>> num_placed_instances;
	for (auto blk_id : cluster_ctx.clb_nlist.blocks()) {
