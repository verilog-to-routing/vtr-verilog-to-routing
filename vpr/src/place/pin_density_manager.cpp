...
void PinDensityManager::find_affected_channels_and_update_costs(const t_pl_blocks_to_be_moved& blocks_affected) {
    ...
    for (ClusterPinId affected_pin_id : blocks_affected.affected_pins) {
        t_physical_tile_loc new_pin_loc = blk_loc_registry.get_coordinate_of_pin(affected_pin_id);
        e_side new_pin_side = blk_loc_registry.pin_side(affected_pin_id);



        PinType pin_type = clb_nlist.pin_type(affected_pin_id);

        if (pin_type == PinType::DRIVER) {

        } else if (pin_type == PinType::SINK) {
