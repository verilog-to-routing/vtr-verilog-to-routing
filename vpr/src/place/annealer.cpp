...
t_swap_result PlacementAnnealer::try_swap_(MoveGenerator& move_generator,
    ...
    // Change in non-timing net cost terms.
    t_net_cost_terms net_cost_terms_delta = {0., 0., 0.};
    ...
    if (create_move_outcome == e_create_move::ABORT) {
        ...
        net_cost_handler_.find_affected_nets_and_update_costs(delay_model_, criticalities_, blocks_affected_,
                                                              timing_delta_c, net_cost_terms_delta);
        ...
        if (place_algorithm == e_place_algorithm::CRITICALITY_TIMING_PLACE) {
            ...
            VTR_LOGV_DEBUG(g_vpr_ctx.placement().f_placer_debug,
                           ...
                           net_cost_terms_delta.bb_cost,
                           costs_.bb_cost_norm);
            delta_c = net_cost_terms_delta.bb_cost * costs_.bb_cost_norm;
        }
...
#ifndef NO_GRAPHICS
        if (manual_move_enabled) {
            move_outcome = pl_do_manual_move(delta_c, timing_delta_c, net_cost_terms_delta.bb_cost, move_outcome);
        }
#endif //NO_GRAPHICS
        ...
        move_outcome_stats.delta_bb_cost_norm = net_cost_terms_delta.bb_cost * costs_.bb_cost_norm;
        move_outcome_stats.delta_timing_cost_norm = timing_delta_c * costs_.timing_cost_norm;

        move_outcome_stats.delta_bb_cost_abs = net_cost_terms_delta.bb_cost;
        move_outcome_stats.delta_timing_cost_abs = timing_delta_c;

        if constexpr (VTR_ENABLE_DEBUG_LOGGING_CONST_EXPR) {
            LOG_MOVE_STATS_OUTCOME(delta_c, net_cost_terms_delta.bb_cost, timing_delta_c, (move_outcome == e_move_result::ACCEPTED ? "ACCEPTED" : "REJECTED"), "");
        }
    }
...
#ifndef NO_GRAPHICS
    stop_placement_and_check_breakpoints(blocks_affected_, move_outcome, delta_c, net_cost_terms_delta.bb_cost, timing_delta_c);
#endif
