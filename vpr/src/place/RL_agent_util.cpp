...
std::pair<std::unique_ptr<MoveGenerator>, std::unique_ptr<MoveGenerator>> create_move_generators(const t_placer_opts& placer_opts,
    ...
    if (!placer_opts.RL_agent_placement) { // RL agent is disabled
        ...
        move_generators.first = std::make_unique<StaticMoveGenerator>(placer_opts.place_static_move_prob);
        move_generators.second = std::make_unique<StaticMoveGenerator>(placer_opts.place_static_move_prob);
        ...
        if (placer_opts.place_agent_algorithm == E_GREEDY) {
            ...
            move_generators.first = std::make_unique<SimpleRLMoveGenerator>(karmed_bandit_agent1,
            ...
            move_generators.second = std::make_unique<SimpleRLMoveGenerator>(karmed_bandit_agent2,
                                                                             noc_attraction_weight,
                                                                             placer_opts.place_high_fanout_net);
        } else {
