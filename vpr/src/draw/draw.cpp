...
static void draw_routing_costs(ezgl::renderer* g) {
    ...
    for (const RRNodeId& id : device_ctx.rr_graph.nodes()) {
        ...
        if (draw_state->show_routing_costs == DRAW_TOTAL_ROUTING_COSTS
            ...
            cost = get_single_rr_cong_pres_cost((size_t)id,
                                                get_draw_state_vars()->pres_fac);
        }
    ...
    for (const RRNodeId& id : device_ctx.rr_graph.nodes()) {
        if (rr_node_costs[(size_t)id] == min_cost) {
            rr_node_costs[(size_t)id] = NAN;
        }
    }
    char msg[vtr::bufsize];
    if (draw_state->show_routing_costs == DRAW_TOTAL_ROUTING_COSTS) {
...
static int draw_check_rr_node_hit(float click_x, float click_y) {
    ...
    for (const RRNodeId& id : device_ctx.rr_graph.nodes()) {
        ...
        switch (rr_graph.node_type(id)) {
            case IPIN:
            case OPIN: {
                int i = rr_graph.node_xlow(id);
                int j = rr_graph.node_ylow(id);
                ...
                int ipin = rr_graph.node_pin_num(id);
                float xcen, ycen;
...
static void draw_router_expansion_costs(ezgl::renderer* g) {
    ...
    for (const RRNodeId& id : device_ctx.rr_graph.nodes()) {
        float cost = get_router_expansion_cost(
            routing_ctx.rr_node_route_inf[(size_t)id],
            draw_state->show_router_expansion_cost);
        rr_costs[(size_t)id] = cost;
    }

    bool all_nan = true;
    for (const RRNodeId& id : device_ctx.rr_graph.nodes()) {
        if (std::isinf(rr_costs[(size_t)id])) {
            rr_costs[(size_t)id] = NAN;
        } else {
            all_nan = false;
        }
    }
...
static void draw_rr_costs(ezgl::renderer* g, const std::vector<float>& rr_costs, bool lowest_cost_first) {
    ...
    float max_cost = -min_cost;
    for (const RRNodeId& id : device_ctx.rr_graph.nodes()) {
        if (std::isnan(rr_costs[(size_t)id])) continue;

        min_cost = std::min(min_cost, rr_costs[(size_t)id]);
        max_cost = std::max(max_cost, rr_costs[(size_t)id]);
    }
