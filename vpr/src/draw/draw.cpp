...
>>>>>>> When displaying the NoC, the noc links are drawn so that any parallel links are seperated and not drawn on top of each other
...
>>>>>>> removed NoC drawing functions from draw.cpp to draw_noc.cpp. Added doxygen comments to explain a few of the NoC drawing functions that were poorly commented before.
