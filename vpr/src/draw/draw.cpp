...
void init_draw_coords(float clb_width, const BlkLocRegistry& blk_loc_registry) {
    ...
    for (size_t x_loc = 0; x_loc < grid.width() - 1; x_loc++) {
        for (size_t y_loc = 0; y_loc < grid.height() - 1; y_loc++) {
            ...
            std::vector<RRNodeId> chanx_nodes = rr_graph.node_lookup().find_channel_nodes(0, x_loc, y_loc, e_rr_type::CHANX);
            ...
            std::vector<RRNodeId> chany_nodes = rr_graph.node_lookup().find_channel_nodes(0, x_loc, y_loc, e_rr_type::CHANY);
