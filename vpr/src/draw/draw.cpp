...
static void draw_main_canvas(ezgl::renderer* g) {
    ...
    if (g_vpr_ctx.server().gate_io.is_running()) {
        ...
        draw_crit_path_elements(server_ctx.crit_paths, server_ctx.crit_path_element_indexes,
                                server_ctx.draw_crit_path_contour, g);
    } else {
        draw_crit_path(g);
    }
#else
...
#ifndef NO_GRAPHICS
void toggle_window_mode(GtkWidget* /*widget*/, ezgl::application* /*app*/) { window_mode = true; }
...
void init_draw_coords(float clb_width, const BlkLocRegistry& blk_loc_registry) {
    ...
    for (const auto& type : device_ctx.physical_tile_types) {
        ...
        if (num_pins > 0) {
            draw_coords->pin_size
                = std::min(draw_coords->pin_size, (draw_coords->get_tile_width() / (4.0F * num_pins)));
        }
    }
    ...
    draw_coords->tile_x[device_ctx.grid.width() - 1]
        = ((device_ctx.grid.width() - 1) * draw_coords->get_tile_width()) + j;
    j = 0;
    ...
    draw_coords->tile_y[device_ctx.grid.height() - 1]
        = ((device_ctx.grid.height() - 1) * draw_coords->get_tile_width()) + j;
    ...
    initial_world = ezgl::rectangle({-VISIBLE_MARGIN * draw_width, -VISIBLE_MARGIN * draw_height},
                                    {(1. + VISIBLE_MARGIN) * draw_width, (1. + VISIBLE_MARGIN) * draw_height});
#else
    (void)clb_width;
...
void act_on_mouse_move(ezgl::application* app, GdkEventButton* /* event */, double x, double y) {
    ...
    if (draw_state->draw_rr_toggle != DRAW_NO_RR) {
        ...
        if (hit_node) {
            ...
            const auto& device_ctx = g_vpr_ctx.device();
            std::string info = describe_rr_node(device_ctx.rr_graph, device_ctx.grid, device_ctx.rr_indexed_data,
                                                hit_node, draw_state->is_flat);
...
ezgl::point2d atom_pin_draw_coord(AtomPinId pin) {
    ...
    const ezgl::point2d point = {x_offset + usable_width * pin_index / ((float)pin_total), pb_bbox.center_y()};
...
std::vector<RRNodeId> trace_routed_connection_rr_nodes(ClusterNetId net_id, int driver_pin, int sink_pin) {
    ...
    trace_routed_connection_rr_nodes_recurr(tree.root(), sink_rr_node, rr_nodes_on_path);
...
ezgl::color to_ezgl_color(vtr::Color<float> color) { return ezgl::color(color.r * 255, color.g * 255, color.b * 255); }
...
static void draw_router_expansion_costs(ezgl::renderer* g) {
    ...
    for (RRNodeId inode : device_ctx.rr_graph.nodes()) {
        float cost
            = get_router_expansion_cost(routing_ctx.rr_node_route_inf[inode], draw_state->show_router_expansion_cost);
        rr_costs[inode] = cost;
    }
    ...
    } else if (draw_state->show_router_expansion_cost == DRAW_ROUTER_EXPANSION_COST_KNOWN
    ...
    } else if (draw_state->show_router_expansion_cost == DRAW_ROUTER_EXPANSION_COST_EXPECTED
...
static void highlight_blocks(double x, double y) {
    ...
    ezgl::rectangle clb_bbox
        = draw_coords->get_absolute_clb_bbox(clb_index, cluster_ctx.clb_nlist.block_type(clb_index));
...
static void set_draw_partitions(GtkWidget* widget, gint /*response_id*/, gpointer /*data*/) {
    ...
    dialog = gtk_dialog_new_with_buttons("Floorplanning Legend", (GtkWindow*)window, GTK_DIALOG_DESTROY_WITH_PARENT,
                                         ("CLOSE"), GTK_RESPONSE_ACCEPT, NULL);
    ...
    g_signal_connect(selection, "changed", G_CALLBACK(highlight_selected_partition), NULL);
...
void highlight_moved_block_and_its_terminals(const t_pl_blocks_to_be_moved& blocks_affected) {
    ...
    draw_highlight_blocks_color(cluster_ctx.clb_nlist.block_type(blocks_affected.moved_blocks[0].block_num),
...
bool highlight_loc_with_specific_color(t_pl_loc curr_loc, ezgl::color& loc_color) {
    ...
    auto it = std::find_if(draw_state->colored_locations.begin(), draw_state->colored_locations.end(),
                           [&curr_loc](const std::pair<t_pl_loc, ezgl::color>& vec_element) {
                               return (vec_element.first.x == curr_loc.x && vec_element.first.y == curr_loc.y
                                       && vec_element.first.layer == curr_loc.layer);
                           });
...
t_draw_layer_display get_element_visibility_and_transparency(int src_layer, int sink_layer) {
    ...
    if (!draw_state->draw_layer_display[sink_layer].visible || !draw_state->draw_layer_display[src_layer].visible
        || (!cross_layer_enabled && src_layer != sink_layer)) {
