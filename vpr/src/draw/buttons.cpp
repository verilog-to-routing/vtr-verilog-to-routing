...
void button_for_toggle_router_ctrs(){
    GObject* main_window = application.get_object(application.get_main_window_id().c_str());
    GObject* main_window_grid = application.get_object("InnerGrid");

    //combo box for toggle_nets
    GtkWidget* toggle_router_ctrs_widget = gtk_combo_box_text_new();
    GtkWidget* toggle_nets_label = gtk_label_new("Toggle CTRS Drawing Mode:");
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(toggle_router_ctrs_widget), "OFF");
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(toggle_router_ctrs_widget), "ON");
    gtk_combo_box_set_active((GtkComboBox*)toggle_router_ctrs_widget, 0); // default set to None which has an index 0
    gtk_widget_set_name(toggle_router_ctrs_widget, "toggle_router_ctrs");

    //attach to the grid
    gtk_grid_attach((GtkGrid*)main_window_grid, toggle_nets_label, label_left_start_col, button_row++, box_width, box_height);
    gtk_grid_attach((GtkGrid*)main_window_grid, toggle_router_ctrs_widget, box_left_start_col, button_row++, box_width, box_height);

    //show newly added contents
    gtk_widget_show_all((GtkWidget*)main_window);

    //connect signals
    g_signal_connect_swapped(GTK_COMBO_BOX_TEXT(toggle_router_ctrs_widget),
                             "changed",
                             G_CALLBACK(toggle_router_ctrs),
                             toggle_router_ctrs_widget);
}
