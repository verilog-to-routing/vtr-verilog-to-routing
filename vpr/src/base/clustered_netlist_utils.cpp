...
ClusteredPinAtomPinsLookup::ClusteredPinAtomPinsLookup(const AtomNetlist& atom_netlist, const ClusteredNetlist& clustered_netlist, const IntraLbPbPinLookup& pb_gpin_lookup) {
    init_lookup(atom_netlist, clustered_netlist, pb_gpin_lookup);
}
...
void ClusteredPinAtomPinsLookup::init_lookup(const AtomNetlist& atom_netlist, const ClusteredNetlist& clustered_netlist, const IntraLbPbPinLookup& pb_gpin_lookup) {
    ...
    atom_pin_to_clustered_pin_.resize(atom_netlist.pins().size());
    for (ClusterPinId clustered_pin : clustered_pins) {
        ...
        for (const auto& atom_pin : clustered_pin_connected_atom_pins_[clustered_pin]) {
            atom_pin_to_clustered_pin_[atom_pin] = clustered_pin;
        }
    }
}

ClusterPinId ClusteredPinAtomPinsLookup::atom_pin_to_cluster_pin(AtomPinId atom_pin) const {
    VTR_ASSERT(atom_pin);
    return atom_pin_to_clustered_pin_[atom_pin];
}
