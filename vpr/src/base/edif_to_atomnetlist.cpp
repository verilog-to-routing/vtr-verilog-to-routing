...
std::vector<std::tuple<std::string, std::string, std::string>> ports_vec;
    ...
    void read_ios(AtomNetlist& main_netlist_,usefull_data& edif_, struct SNode* node_,t_model* blk_model) {
        printf("\n===================reading inputs==================\n");
       //  const t_model* blk_model = find_model(top_cell_);
       ...
       // inputs = new t_model_ports;
        ...
        outpad_model_=blk_model;
         t_model_ports* outputs = blk_model->outputs;
         t_model_ports* inputs = blk_model->inputs;
          inputs = new t_model_ports;
          outputs = new t_model_ports;
    ...
    void read_blocks(AtomNetlist& main_netlist_,usefull_data& edif_, struct SNode* node_,t_model* blk_model) {
        //t_model* ins_model;
    ...
    void read_nets(AtomNetlist& main_netlist_,usefull_data& edif_, struct SNode* node_,t_model* blk_model) {
        ...
        for (auto it = edif_.nets.begin(); it != edif_.nets.end(); it++) {
            ...
            for (k = 0; k < (temp_vec22.size()); k++) {
                ...
                AtomBlockId blk_id = main_netlist_.find_block(inst_ref);
                //const std::string& out_blk = main_netlist_.block_name(blk_id);
                //printf(" block created is from read block section============ ::%s\n", out_blk.c_str());
