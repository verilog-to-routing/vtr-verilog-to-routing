...
static void print_tap_stats(const std::string& label, const TapAccumulator& acc) {
    ...
    for (const auto& [length, n_wires] : acc.num_wires) {
        ...
        for (const auto& p : pin_map)  total_pins  += p.second;
        for (const auto& p : wire_map) total_wires += p.second;
 ...
 *   - CHANX          (INC + DEC)
 *   - CHANY          (INC + DEC)
 *   - INC CHANX      (incremental x-wires only)
 *   - DEC CHANX      (decremental x-wires only)
 *   - INC CHANY      (incremental y-wires only)
 *   - DEC CHANY      (decremental y-wires only)
 */
static void print_tap_utilization(const Netlist<>& net_list, const std::vector<t_segment_inf>& segment_inf) {
    const auto& device_ctx = g_vpr_ctx.device();
    const auto& rr_graph   = device_ctx.rr_graph;
