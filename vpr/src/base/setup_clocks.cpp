...
static MetalLayer get_metal_layer_from_name(std::string metal_layer_name,
    ...
    for (auto clock_network_arch : clock_networks_arch) {
        switch (clock_network_arch.type) {
            case e_clock_type::SPINE: {
                ...
                spine->set_metal_layer(get_metal_layer_from_name(clock_network_arch.metal_layer, clock_metal_layers,
                                                                 clock_network_arch.name));
                spine->set_initial_wire_location(p.parse_formula(clock_network_arch.wire.start, vars),
                                                 p.parse_formula(clock_network_arch.wire.end, vars),
                                                 p.parse_formula(clock_network_arch.wire.position, vars));
                spine->set_wire_repeat(p.parse_formula(clock_network_arch.repeat.x, vars),
                ...
                spine->set_tap_locations(p.parse_formula(clock_network_arch.tap.offset, vars),
            ...
            case e_clock_type::RIB: {
                ...
                rib->set_metal_layer(get_metal_layer_from_name(clock_network_arch.metal_layer, clock_metal_layers,
                                                               clock_network_arch.name));
                rib->set_initial_wire_location(p.parse_formula(clock_network_arch.wire.start, vars),
                                               p.parse_formula(clock_network_arch.wire.end, vars),
                                               p.parse_formula(clock_network_arch.wire.position, vars));
                rib->set_wire_repeat(p.parse_formula(clock_network_arch.repeat.x, vars),
                                     p.parse_formula(clock_network_arch.repeat.y, vars));
                ...
                rib->set_tap_locations(p.parse_formula(clock_network_arch.tap.offset, vars),
                                       p.parse_formula(clock_network_arch.tap.increment, vars));
            ...
            default: {
                VPR_FATAL_ERROR(VPR_ERROR_OTHER, "Found unsupported clock network type for '%s' clock network",
...
void setup_clock_connections(const t_arch& Arch, FormulaParser& p) {
    ...
    for (auto clock_connection_arch : clock_connections_arch) {
        if (clock_connection_arch.from == "ROUTING") {
            ...
            if (RoutingToClockConnection* routing_to_clock
                ...
                routing_to_clock->set_switch_location(p.parse_formula(clock_connection_arch.locationx, vars),
            ...
            if (ClockToPinsConnection* clock_to_pins
            ...
            if (ClockToClockConneciton* clock_to_clock
