...
struct ParseAPGlobalPlacer {
    ...
    ConvertedValue<std::string> to_str(e_ap_global_placer val) {
        ...
        switch (val) {
            case e_ap_global_placer::SimPL_BiParitioning:
                conv_value.set_value("simple-bipartitioning");
                break;
            case e_ap_global_placer::SimPL_FlowBased:
                conv_value.set_value("simple-flowbased");
                break;
            default:
                VTR_ASSERT(false);
        }
    ...
    std::vector<std::string> default_choices() {
        return {"simple-bipartitioning", "simple-flowbased"};
    }
};
...
argparse::ArgumentParser create_arg_parser(const std::string& prog_name, t_options& args) {
    ...
    ap_grp.add_argument<e_ap_global_placer, ParseAPGlobalPlacer>(args.ap_global_placer, "--ap_global_placer")
        .help(
            ...
            " * simple-bipartitioning: Use a SimPL-based Global Placer which uses a quadratic solver and a bi-partitioning legalizer.\n"
            " * simple-flowbased: Use a SimPL-based Global placer which uses a quadratic solver and multi-commmodity-flow-based legalizer.")
        .default_value("simple-bipartitioning")
        .show_in(argparse::ShowIn::HELP_ONLY);
