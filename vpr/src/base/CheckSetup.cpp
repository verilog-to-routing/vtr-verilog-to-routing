...
void CheckSetup(const t_packer_opts& PackerOpts,
    ...
    if ((GLOBAL == RouterOpts.route_type) && (PlacerOpts.place_algorithm.is_timing_driven())) {
    ...
    if (!Timing.timing_analysis_enabled && (PlacerOpts.place_algorithm.is_timing_driven())) {
        /* May work, not tested */
    ...
    if (PlacerOpts.place_algorithm.is_timing_driven() && PlacerOpts.place_static_move_prob.size() > NUM_PL_MOVE_TYPES) {
        VPR_FATAL_ERROR(VPR_ERROR_OTHER,
                        "The number of provided placer move probabilities (%d) should equal or less than the total "
                        "number of supported moves (%d).\n",
    ...
    if (!PlacerOpts.place_algorithm.is_timing_driven()
        ...
        VPR_FATAL_ERROR(VPR_ERROR_OTHER,
                        "The number of placer non timing move probabilities (%d) should equal to or less than the "
                        "total number of supported moves (%d).\n",
                        PlacerOpts.place_static_move_prob.size(), NUM_PL_MOVE_TYPES);
    }
    ...
    if (RouterOpts.doRouting) {
        if (!Timing.timing_analysis_enabled
            ...
            VPR_FATAL_ERROR(VPR_ERROR_OTHER,
                            "base_cost_type must be demand_only or demand_only_normalized_length when timing analysis "
                            "is disabled.\n");
        }
    }

    if (DETAILED == RouterOpts.route_type) {
        if ((Chans.chan_x_dist.type != UNIFORM) || (Chans.chan_y_dist.type != UNIFORM)) {
            VPR_FATAL_ERROR(VPR_ERROR_OTHER,
    ...
    for (i = 0; i < (int)Segments.size(); ++i) {
        ...
        if (!device_ctx.arch_switch_inf[Tmp].buffered()) {
            VPR_FATAL_ERROR(VPR_ERROR_OTHER, "arch_opin_switch (#%d) of segment type #%d is not buffered.\n", Tmp, i);
        }
    }
    ...
    if (UNI_DIRECTIONAL == RoutingArch.directionality) {
        if ((RouterOpts.fixed_channel_width != NO_FIXED_CHANNEL_WIDTH) && (RouterOpts.fixed_channel_width % 2 > 0)) {
            VPR_FATAL_ERROR(VPR_ERROR_OTHER, "Routing channel width must be even for unidirectional.\n");
        }
        if ((PlacerOpts.place_chan_width != NO_FIXED_CHANNEL_WIDTH) && (PlacerOpts.place_chan_width % 2 > 0)) {
            VPR_FATAL_ERROR(VPR_ERROR_OTHER, "Place channel width must be even for unidirectional.\n");
        }
    }
    ...
    if (ServerOpts.is_server_mode_enabled) {
        if (ServerOpts.port_num < DYMANIC_PORT_RANGE_MIN || ServerOpts.port_num > DYNAMIC_PORT_RANGE_MAX) {
            VPR_FATAL_ERROR(VPR_ERROR_OTHER,
                            "Specified server port number `--port %d` is out of range [%d-%d]. Please specify a port "
