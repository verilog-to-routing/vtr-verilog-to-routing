...
void SetupVPR(const t_options* options,
    ...
    if (!options->do_packing
        ...
        packerOpts->doPacking = e_stage_action::STAGE_DO;
        placerOpts->doPlacement = e_stage_action::STAGE_DO;
        apOpts->doAP = e_stage_action::STAGE_SKIP; // AP not default.
        routerOpts->doRouting = e_stage_action::STAGE_DO;
        analysisOpts->doAnalysis = e_stage_action::STAGE_AUTO; //Deferred until implementation status known
    } else {
        ...
        if (options->do_analysis) {
            packerOpts->doPacking = e_stage_action::STAGE_LOAD;
            placerOpts->doPlacement = e_stage_action::STAGE_LOAD;
            routerOpts->doRouting = e_stage_action::STAGE_LOAD;
            analysisOpts->doAnalysis = e_stage_action::STAGE_DO;
        }
        ...
        if (options->do_routing) {
            ...
            placerOpts->doPlacement = e_stage_action::STAGE_LOAD;
            routerOpts->doRouting = e_stage_action::STAGE_DO;
            analysisOpts->doAnalysis = ((options->do_analysis) ? e_stage_action::STAGE_DO : e_stage_action::STAGE_AUTO); //Always run analysis after routing
        }
        ...
        if (options->do_analytical_placement) {
            ...
            packerOpts->doPacking = e_stage_action::STAGE_SKIP;
            placerOpts->doPlacement = e_stage_action::STAGE_SKIP;
            apOpts->doAP = e_stage_action::STAGE_DO;
        }
...
static void SetupAnalysisOpts(const t_options& Options, t_analysis_opts& analysis_opts) {
    if (Options.do_analysis) {
        analysis_opts.doAnalysis = e_stage_action::STAGE_DO;
    }
