 ...
 */
static void print_flat_placement_file_header(FILE* fp, bool flat_place_verbose) {
...
void write_flat_placement(const char* flat_place_file_path,
    ...
    print_flat_placement_file_header(fp, flat_place_verbose);

    // For each cluster, write out the atoms in the cluster at this cluster's
    // location.
    for (ClusterBlockId iblk : cluster_netlist.blocks()) {
        print_flat_cluster(fp, iblk, block_locs, atoms_lookup, flat_place_verbose);
    }
