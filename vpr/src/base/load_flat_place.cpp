...
typedef struct molecule_placement {
     ...
     * - EMPTY_BLOCK_ID if the target cluster is not yet started
     * - INVALID_BLOCK_ID if this is an orphan to be reclustered
     */
 ...
 *         the molecule's target cluster; else, sets target to EMPTY_BLOCK_ID.   */
...
static void legalizer_init(const char* flat_place_file) {
    ...
    atom_ctx.list_of_pack_molecules.reset(alloc_and_load_pack_molecules(list_of_pack_patterns.data(),
...
static bool find_atom_molecule_root(AtomBlockId atom, t_molecule_placement* cur) {
        ...
        auto itr = atom_ctx.atom_molecules.find(atom);
        if (itr == atom_ctx.atom_molecules.end()) {
            /* this can only happen during the initial file parse */
            VTR_LOG("Legalizer: line %d: ", cur->lineno);
            VTR_LOG("skipping primitive missing from vpr prepacking.\n");
            return false;
        }
        t_pack_molecule* mol = itr->second;
...
static void set_molecule_target_cluster(t_molecule_placement* cur) {
    ...
    if (place_ctx.grid_blocks.is_sub_tile_empty({cur->loc.x, cur->loc.y, 0},
                                                 cur->loc.sub_tile)) {
        cur->target_cluster = EMPTY_BLOCK_ID;
    } else {
...
static void place_new_cluster(t_molecule_placement* cur,
    ...
    auto& place_ctx = g_vpr_ctx.mutable_placement();
    place_ctx.block_locs.resize(g_vpr_ctx.placement().block_locs.size()+1);
    if (is_orphan) {
        place_ctx.block_locs[iblk].is_fixed = false;
    } else {
        /* add fixed loc if not an orphan */
        set_block_location(iblk, cur->loc);
...
static ClusterBlockId add_new_mol_to_cluster(t_molecule_placement* cur,
    ...
    auto& atom_ctx = g_vpr_ctx.mutable_atom();
    auto& helper_ctx = g_vpr_ctx.mutable_cl_helper();
    bool is_orphan_cluster = (cur->target_cluster == INVALID_BLOCK_ID);
    bool is_new_cluster = is_orphan_cluster || (cur->target_cluster == EMPTY_BLOCK_ID);
    bool is_existing_cluster = !is_orphan_cluster && !is_new_cluster;
...
static void dissolve_and_rebuild_cluster(ClusterBlockId iblk,
    ...
    for (auto atom : atoms) {
        ...
        tmp->loc.x = place_ctx.block_locs[iblk].loc.x;
        tmp->loc.y = place_ctx.block_locs[iblk].loc.y;
        tmp->loc.sub_tile = place_ctx.block_locs[iblk].loc.sub_tile;
        tmp->loc.layer = place_ctx.block_locs[iblk].loc.layer;
...
int load_flat_placement(t_vpr_setup& vpr_setup, const t_arch& arch) {
     ...
     */
    bool lucky = false;
    enum e_orphan_cluster_type orphan_cluster_type = SINGLES;
    ...
    std::ifstream infile(flat_place_file);
    if (!infile) {
    ...
    while (!get_next_molecule_placement(infile, &lineno, &cur)) {
        ...
        if (!check_cluster_loc(cur) || !find_compatible_cluster_types_at_loc(cur)) {
            ...
            cur->target_cluster = INVALID_BLOCK_ID;
    ...
    float parse_loop_time = parse_loop_timer.elapsed_sec();
    ...
    auto it = cluster_orphans[EMPTY_BLOCK_ID].begin();
    while (it != cluster_orphans[EMPTY_BLOCK_ID].end()) {
        auto mol = *it;
        set_molecule_target_cluster(mol);
        if (mol->target_cluster == EMPTY_BLOCK_ID) {
            ...
            if (INVALID_BLOCK_ID == res) {
                ...
                VTR_LOG("Legalizer: failed to create new cluster for %s.\n",
                         mol->root_type_name.c_str());
                mol->target_cluster = INVALID_BLOCK_ID;
                permanent_orphans.insert(mol);
                it = cluster_orphans[EMPTY_BLOCK_ID].erase(it);
            } else { // success
                it = cluster_orphans[EMPTY_BLOCK_ID].erase(it);
                free(mol);
            }
    ...
    if (cluster_orphans[EMPTY_BLOCK_ID].size() == 0) {
        cluster_orphans.erase(EMPTY_BLOCK_ID);
    } else {
    ...
    VTR_LOG("Legalizer: starting cluster repair and orphan reclustering.\n");
    ClusterBlockId orphan_cluster = INVALID_BLOCK_ID;
    ...
    while (itblk != cluster_orphans.end()) {
        ...
        if (ANNEX == orphan_cluster_type) {
            orphan_cluster = INVALID_BLOCK_ID;
        }
        ...
        while (itmol != cluster_orphans[iblk].end()) {
            ...
            if (res != INVALID_BLOCK_ID) {
                VTR_LOG("Legalizer: successfully added %s on vpr site in original cluster %zu.\n",
                           mol->name.c_str(), size_t(mol->target_cluster));
                itmol = cluster_orphans[iblk].erase(itmol);
                free(mol);
                --final;
            } else {
                VTR_LOG("Legalizer: unsuccessful re-add, adding %s to an orphan cluster.\n",
                           mol->name.c_str());

                // create or add to an orphan cluster
                if (SINGLES == orphan_cluster_type) {
                    mol->target_cluster = orphan_cluster;
                    res = add_new_mol_to_cluster(mol, clustering_data,
                       VPR_SITE, E_DETAILED_ROUTE_FOR_EACH_ATOM, true);
                    if (res != INVALID_BLOCK_ID) {
                        orphan_cluster = INVALID_BLOCK_ID;
                        ++num_orphan_clusters;
                        itmol = cluster_orphans[iblk].erase(itmol);
        ...
        if (cluster_orphans[iblk].size() == 0) {
            ...
            VTR_LOG("Legalizer: some orphans not handled in cluster %zu\n", size_t(iblk));
        }
    }

    /* recluster bad placement orphans */
/*    for (auto it = permanent_orphans.begin(); it != permanent_orphans.end(); ) {
            auto mol = *it;
                if (SINGLES == orphan_cluster_type) {
                    cur->target_cluster = orphan_cluster;
                    ClusterBlockId res = add_new_mol_to_cluster(mol, clustering_data,
                       VPR_SITE, E_DETAILED_ROUTE_FOR_EACH_ATOM, true);
                    if (res != INVALID_BLOCK_ID) {
                        orphan_cluster = INVALID_BLOCK_ID;
                        ++num_orphan_clusters;
                        permanent_orphans.erase(mol);
                        free(mol);
                        ++it;
                    }
                }
