...
bool vpr_flow(t_vpr_setup& vpr_setup, t_arch& arch) {
    ...
    { // Analytical Place
        if (vpr_setup.APOpts.doAP == e_stage_action::STAGE_DO) {
...
bool vpr_pack_flow(t_vpr_setup& vpr_setup, const t_arch& arch) {
    ...
    if (packer_opts.doPacking == e_stage_action::STAGE_SKIP) {
        ...
        if (packer_opts.doPacking == e_stage_action::STAGE_DO) {
            ...
            VTR_ASSERT(packer_opts.doPacking == e_stage_action::STAGE_LOAD);
...
bool vpr_place_flow(const Netlist<>& net_list,
    ...
    if (placer_opts.doPlacement == e_stage_action::STAGE_SKIP) {
        ...
        if (placer_opts.doPlacement == e_stage_action::STAGE_DO) {
            //Do the actual placement
            vpr_place(net_list, vpr_setup, arch);

        } else {
            VTR_ASSERT(placer_opts.doPlacement == e_stage_action::STAGE_LOAD);
...
RouteStatus vpr_route_flow(const Netlist<>& net_list,
    ...
    if (router_opts.doRouting == e_stage_action::STAGE_SKIP) {
        ...
        if (router_opts.doRouting == e_stage_action::STAGE_DO) {
            ...
            VTR_ASSERT(router_opts.doRouting == e_stage_action::STAGE_LOAD);
...
bool vpr_analysis_flow(const Netlist<>& net_list,
    ...
    if (analysis_opts.doAnalysis == e_stage_action::STAGE_SKIP) return true; //Skipped

    if (analysis_opts.doAnalysis == e_stage_action::STAGE_AUTO && !route_status.success()) return false; //Not run

    VTR_ASSERT_MSG(analysis_opts.doAnalysis == e_stage_action::STAGE_DO
                       || (analysis_opts.doAnalysis == e_stage_action::STAGE_AUTO && route_status.success()),
