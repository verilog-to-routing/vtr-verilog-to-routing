...
void vpr_init_with_options(const t_options* options, t_vpr_setup* vpr_setup, t_arch* arch) {
    ...
    SetupVPR(options, vpr_setup->TimingEnabled, true, &vpr_setup->FileNameOpts, arch, &vpr_setup->user_models,
             &vpr_setup->library_models, &vpr_setup->NetlistOpts, &vpr_setup->PackerOpts, &vpr_setup->PlacerOpts,
             &vpr_setup->APOpts, &vpr_setup->AnnealSched, &vpr_setup->RouterOpts, &vpr_setup->AnalysisOpts,
             &vpr_setup->NocOpts, &vpr_setup->ServerOpts, &vpr_setup->RoutingArch, &vpr_setup->PackerRRGraph,
             vpr_setup->Segments, &vpr_setup->Timing, &vpr_setup->ShowGraphics, &vpr_setup->GraphPause,
             &vpr_setup->SaveGraphics, &vpr_setup->GraphicsCommands, &vpr_setup->PowerOpts, vpr_setup);

    /* Check inputs are reasonable */
    CheckArch(*arch);

    /* Verify settings don't conflict or otherwise not make sense */
    CheckSetup(vpr_setup->PackerOpts, vpr_setup->PlacerOpts, vpr_setup->APOpts, vpr_setup->RouterOpts,
               vpr_setup->ServerOpts, vpr_setup->RoutingArch, vpr_setup->Segments, vpr_setup->Timing, arch->Chans);
    ...
    if (vpr_setup->TimingEnabled) {
        auto& timing_ctx = g_vpr_ctx.mutable_timing();
        {
            ...
            timing_ctx.graph = TimingGraphBuilder(atom_ctx.nlist, atom_ctx.lookup)
                                   .timing_graph(options->allow_dangling_combinational_nodes);
    ...
    if (!filename_opts.read_vpr_constraints_file.empty()) {
        ...
        if (g_vpr_ctx.routing().constraints.get_num_route_constraints()
            && options->clock_modeling.provenance() == argparse::Provenance::SPECIFIED) {
...
bool vpr_flow(t_vpr_setup& vpr_setup, t_arch& arch) {
    ...
    { // Analytical Place
        ...
        placement_id = print_place(filename_opts.NetFile.c_str(), cluster_ctx.clb_nlist.netlist_id().c_str(),
                                   filename_opts.PlaceFile.c_str(), block_locs);
    }

    bool is_flat = vpr_setup.RouterOpts.flat_routing;
    const Netlist<>& router_net_list
        = is_flat ? (const Netlist<>&)g_vpr_ctx.atom().nlist : (const Netlist<>&)g_vpr_ctx.clustering().clb_nlist;
...
void vpr_create_device_grid(const t_vpr_setup& vpr_setup, const t_arch& Arch) {
    ...
    device_ctx.grid
        = create_device_grid(vpr_setup.device_layout, Arch.grid_layouts, num_type_instances, target_device_utilization);
...
void vpr_setup_noc_routing_algorithm(const std::string& noc_routing_algorithm_name) {
    ...
    noc_ctx.noc_flows_router
        = NocRoutingAlgorithmCreator::create_routing_algorithm(noc_routing_algorithm_name, noc_ctx.noc_model);
}
...
bool vpr_pack(t_vpr_setup& vpr_setup, const t_arch& arch) {
    ...
    return try_pack(&vpr_setup.PackerOpts, &vpr_setup.AnalysisOpts, &arch, vpr_setup.user_models,
...
void vpr_load_packing(t_vpr_setup& vpr_setup, const t_arch& arch) {
    ...
    VTR_ASSERT_MSG(!vpr_setup.FileNameOpts.NetFile.empty(), "Must have valid .net filename to load packing");
    ...
    cluster_ctx.clb_nlist
        = read_netlist(vpr_setup.FileNameOpts.NetFile.c_str(), &arch, vpr_setup.FileNameOpts.verify_file_digests,
    ...
    process_constant_nets(g_vpr_ctx.mutable_atom().nlist, atom_ctx.lookup, cluster_ctx.clb_nlist,
                          vpr_setup.constant_net_method, vpr_setup.PackerOpts.pack_verbosity);
...
void vpr_place(const Netlist<>& net_list, t_vpr_setup& vpr_setup, const t_arch& arch) {
    ...
    if (vpr_setup.PlacerOpts.place_algorithm.is_timing_driven()) {
        ...
        get_cached_router_lookahead(vpr_setup.RoutingArch, vpr_setup.RouterOpts.lookahead_type,
                                    vpr_setup.RouterOpts.write_router_lookahead,
                                    vpr_setup.RouterOpts.read_router_lookahead, vpr_setup.Segments, is_flat);
    }

    try_place(net_list, vpr_setup.PlacerOpts, vpr_setup.AnnealSched, vpr_setup.RouterOpts, vpr_setup.AnalysisOpts,
              vpr_setup.NocOpts, arch.Chans, &vpr_setup.RoutingArch, vpr_setup.Segments, arch.Directs, arch.num_directs,
              is_flat);
...
void vpr_load_placement(t_vpr_setup& vpr_setup, const t_arch& arch) {
    ...
    place_ctx.placement_id
        = read_place(filename_opts.NetFile.c_str(), filename_opts.PlaceFile.c_str(),
                     place_ctx.mutable_blk_loc_registry(), filename_opts.verify_file_digests, device_ctx.grid);
...
RouteStatus vpr_route_flow(const Netlist<>& net_list, t_vpr_setup& vpr_setup, const t_arch& arch, bool is_flat) {
    ...
    if (router_opts.doRouting == STAGE_SKIP) {
        ...
        if (vpr_setup.Timing.timing_analysis_enabled) {
            ...
            routing_delay_calc
                = std::make_shared<RoutingDelayCalculator>(atom_ctx.nlist, atom_ctx.lookup, net_delay, is_flat);
        ...
        if (router_opts.doRouting == STAGE_DO) {
            ...
            if (NO_FIXED_CHANNEL_WIDTH == chan_width) {
                ...
                route_status
                    = vpr_route_min_W(net_list, vpr_setup, arch, timing_info, routing_delay_calc, net_delay, is_flat);
            } else {
        ...
        if (route_status.success()) {
            ...
            check_route(net_list, router_opts.route_type, router_opts.check_route, is_flat);
            get_serial_num(net_list);

            //Update status
            VTR_LOG("Circuit successfully routed with a channel width factor of %d.\n", route_status.chan_width());
            graphics_msg
                = vtr::string_fmt("Routing succeeded with a channel width factor of %d.\n", route_status.chan_width());
        } else {
            //Update status
            ...
            if (router_opts.generate_rr_node_overuse_report) {
                ...
                report_overused_nodes(net_list, rr_graph, is_flat);

            } else {
                VTR_LOG(
                    "For a detailed report on the RR node overuse information (report_overused_nodes.rpt), specify "
        ...
        if (vpr_setup.Timing.timing_analysis_enabled) {
            if (isEchoFileEnabled(E_ECHO_FINAL_ROUTING_TIMING_GRAPH)) {
                ...
                tatum::write_echo(getEchoFileName(E_ECHO_FINAL_ROUTING_TIMING_GRAPH), *timing_ctx.graph,
                                  *timing_ctx.constraints, *routing_delay_calc, timing_info->analyzer());
            }
...
RouteStatus vpr_route_fixed_W(const Netlist<>& net_list,
                              ...
                              bool is_flat) {
    get_cached_router_lookahead(vpr_setup.RoutingArch, vpr_setup.RouterOpts.lookahead_type,
                                vpr_setup.RouterOpts.write_router_lookahead, vpr_setup.RouterOpts.read_router_lookahead,
                                vpr_setup.Segments, is_flat);
    ...
    bool status = false;
    status = route(net_list, fixed_channel_width, vpr_setup.RouterOpts, vpr_setup.AnalysisOpts, &vpr_setup.RoutingArch,
                   vpr_setup.Segments, net_delay, timing_info, delay_calc, arch.Chans, arch.Directs, arch.num_directs,
                   ScreenUpdatePriority::MAJOR, is_flat);
...
RouteStatus vpr_route_min_W(const Netlist<>& net_list,
    ...
    int min_W = binary_search_place_and_route(
        (const Netlist<>&)g_vpr_ctx.clustering().clb_nlist, net_list, vpr_setup.PlacerOpts, vpr_setup.AnnealSched,
        router_opts, vpr_setup.AnalysisOpts, vpr_setup.NocOpts, vpr_setup.FileNameOpts, &arch,
        router_opts.verify_binary_search, router_opts.min_channel_width_hint, &vpr_setup.RoutingArch,
        vpr_setup.Segments, net_delay, timing_info, delay_calc, is_flat);
...
RouteStatus vpr_load_routing(t_vpr_setup& vpr_setup,
    ...
    bool is_legal
        = read_route(filename_opts.RouteFile.c_str(), vpr_setup.RouterOpts, filename_opts.verify_file_digests, is_flat);
    const Netlist<>& router_net_list
        = is_flat ? (const Netlist<>&)g_vpr_ctx.atom().nlist : (const Netlist<>&)g_vpr_ctx.clustering().clb_nlist;
    if (vpr_setup.Timing.timing_analysis_enabled) {
        //Update timing info
        load_net_delay_from_routing(router_net_list, net_delay);
        timing_info->update();
    }
...
void vpr_create_rr_graph(t_vpr_setup& vpr_setup, const t_arch& arch, int chan_width_fac, bool is_flat) {
    ...
    create_rr_graph(graph_type, device_ctx.physical_tile_types, device_ctx.grid, chan_width, det_routing_arch,
                    vpr_setup.Segments, router_opts, arch.Directs, arch.num_directs, &warnings, is_flat);
...
void vpr_init_graphics(const t_vpr_setup& vpr_setup, const t_arch& arch, bool is_flat) {
    /* Startup X graphics */
    init_graphics_state(vpr_setup.ShowGraphics, vpr_setup.GraphPause, vpr_setup.RouterOpts.route_type,
                        vpr_setup.SaveGraphics, vpr_setup.GraphicsCommands, is_flat);
...
void free_device(const t_det_routing_arch& /*routing_arch*/) {
    ...
    device_ctx.chan_width.max = device_ctx.chan_width.x_max = device_ctx.chan_width.y_max = device_ctx.chan_width.x_min
        = device_ctx.chan_width.y_min = 0;
...
void vpr_read_options(const int argc, const char** argv, t_options* options) { *options = read_options(argc, argv); }
...
void vpr_setup_vpr(t_options* Options,
                   ...
                   t_vpr_setup* vpr_setup) {
    SetupVPR(Options, TimingEnabled, readArchFile, FileNameOpts, Arch, user_models, library_models, NetlistOpts,
             PackerOpts, PlacerOpts, APOpts, AnnealSched, RouterOpts, AnalysisOpts, NocOpts, ServerOpts, RoutingArch,
             PackerRRGraph, Segments, Timing, ShowGraphics, GraphPause, SaveGraphics, GraphicsCommands, PowerOpts,
             vpr_setup);
}

void vpr_check_arch(const t_arch& Arch) { CheckArch(Arch); }
...
void vpr_check_setup(const t_packer_opts& PackerOpts,
    ...
    CheckSetup(PackerOpts, PlacerOpts, APOpts, RouterOpts, ServerOpts, RoutingArch, Segments, Timing, Chans);
}

///@brief Show current setup
void vpr_show_setup(const t_vpr_setup& vpr_setup) { ShowSetup(vpr_setup); }
...
bool vpr_analysis_flow(const Netlist<>& net_list,
    ...
    if (route_status.success()) {
        if (is_flat) {
            ...
            sync_netlists_to_routing(net_list, g_vpr_ctx.device(), g_vpr_ctx.mutable_atom(),
                                     g_vpr_ctx.mutable_clustering(), g_vpr_ctx.placement(),
                                     vpr_setup.PackerOpts.pack_verbosity > 2);
        }

        std::string post_routing_packing_output_file_name = vpr_setup.PackerOpts.output_file + ".post_routing";
        write_packing_results_to_xml(vpr_setup.PackerOpts.global_clocks, Arch.architecture_id,
                                     post_routing_packing_output_file_name.c_str());
    } else {
        VTR_LOG_WARN(
            "Synchronization between packing and routing results is not applied due to illegal circuit "
            "implementation\n");
    }
    VTR_LOG("\n");

    vpr_analysis(net_list, vpr_setup, Arch, route_status, is_flat);
...
void vpr_analysis(const Netlist<>& net_list,
    ...
    routing_stats(net_list, vpr_setup.RouterOpts.full_stats, vpr_setup.RouterOpts.route_type, vpr_setup.Segments,
                  vpr_setup.RoutingArch.R_minW_nmos, vpr_setup.RoutingArch.R_minW_pmos, Arch.grid_logic_tile_area,
                  vpr_setup.RoutingArch.directionality, vpr_setup.RoutingArch.wire_to_rr_ipin_switch, is_flat);
    ...
    if (vpr_setup.TimingEnabled) {
        ...
        if (isEchoFileEnabled(E_ECHO_ANALYSIS_TIMING_GRAPH)) {
            auto& timing_ctx = g_vpr_ctx.timing();
            tatum::write_echo(getEchoFileName(E_ECHO_ANALYSIS_TIMING_GRAPH), *timing_ctx.graph, *timing_ctx.constraints,
                              *analysis_delay_calc, timing_info->analyzer());
        }

        //Timing stats
        VTR_LOG("\n");
        generate_hold_timing_stats(/*prefix=*/"", *timing_info, *analysis_delay_calc, vpr_setup.AnalysisOpts,
                                   vpr_setup.RouterOpts.flat_routing, blk_loc_registry);
        generate_setup_timing_stats(/*prefix=*/"", *timing_info, *analysis_delay_calc, vpr_setup.AnalysisOpts,
                                    vpr_setup.RouterOpts.flat_routing, blk_loc_registry);

        //Write the post-synthesis netlist
        if (vpr_setup.AnalysisOpts.gen_post_synthesis_netlist) {
            netlist_writer(atom_ctx.nlist.netlist_name(), analysis_delay_calc, vpr_setup.AnalysisOpts);
        }
