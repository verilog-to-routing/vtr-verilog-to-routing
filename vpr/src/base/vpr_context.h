...
struct DeviceContext : public Context {
     ...
     */
    RRGraphView rr_graph = RRGraphView(rr_nodes, rr_spatial_lookup); // A read-only view of routing resource graph to be the ONLY database for client functions: GUI, placer, router, timing analyzer etc.
     ...
     */ 
    RRSpatialLookup rr_spatial_lookup = RRSpatialLookup(rr_node_indices);
