...
enum class atok_t_add_atom {IS_REGEX, LOGICAL_BLOCK_LOCATION, NAME_PATTERN};
...
enum class atok_t_add_logical_block {IS_REGEX, NAME_PATTERN};
constexpr const char *atok_lookup_t_add_logical_block[] = {"is_regex", "name_pattern"};

enum class gtok_t_partition {ADD_ATOM, ADD_REGION, ADD_LOGICAL_BLOCK};
...
inline void load_set_global_signal_required_attributes(const pugi::xml_node &root, enum_route_model_type * route_model, const std::function<void(const char *)> * report_error){
	...
	for(pugi::xml_attribute attr = root.first_attribute(); attr; attr = attr.next_attribute()){
		...
		if(astate[(int)in] == 0) astate[(int)in] = 1;
		else noreturn_report(report_error, ("Duplicate attribute " + std::string(attr.name()) + " in <set_global_signal>.").c_str());
		switch(in){
	...
	*offset_debug = root.offset_debug();
	...
	for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()){
		*offset_debug = node.offset_debug();
		...
		else noreturn_report(report_error, ("Duplicate element " + std::string(node.name()) + " in <vpr_constraints>.").c_str());
		switch(in){
