 ...
 *
 * Cmdline: uxsdcxx.py /home/tao/works/dev/route_constraints/vtr-verilog-to-routing/vpr/src/base/vpr_constraints.xsd
 * Input file: /home/tao/works/dev/route_constraints/vtr-verilog-to-routing/vpr/src/base/vpr_constraints.xsd
 * md5sum of input file: 9f078b743b3fa356a41cf239f150a599
 */
...
template <class T, typename Context>
inline void load_assignment(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug);
template <class T, typename Context>
inline void load_packer_constraints(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug);
template <class T, typename Context>
...
template <class T>
inline void write_packer_constraints(T &in, std::ostream &os, const void *data, void *iter);
template <class T>
...
enum class atok_t_set_global_signal {NAME, ROUTE_MODEL, TYPE};
...
enum class atok_t_assignment {NET, PIN};
constexpr const char *atok_lookup_t_assignment[] = {"net", "pin"};

enum class gtok_t_packer_constraints {ASSIGNMENT};
constexpr const char *gtok_lookup_t_packer_constraints[] = {"assignment"};
enum class gtok_t_vpr_constraints {PARTITION_LIST, GLOBAL_ROUTE_CONSTRAINTS, PACKER_CONSTRAINTS};
constexpr const char *gtok_lookup_t_vpr_constraints[] = {"partition_list", "global_route_constraints", "packer_constraints"};
...
inline atok_t_assignment lex_attr_t_assignment(const char *in, const std::function<void(const char *)> * report_error){
	unsigned int len = strlen(in);
	switch(len){
	case 3:
		switch(in[0]){
	...
	noreturn_report(report_error, ("Found unrecognized attribute " + std::string(in) + " of <assignment>.").c_str());
}

inline gtok_t_packer_constraints lex_node_t_packer_constraints(const char *in, const std::function<void(const char *)> *report_error){
	unsigned int len = strlen(in);
	switch(len){
	case 10:
		switch(*((triehash_uu64*)&in[0])){
		case onechar('a', 0, 64) | onechar('s', 8, 64) | onechar('s', 16, 64) | onechar('i', 24, 64) | onechar('g', 32, 64) | onechar('n', 40, 64) | onechar('m', 48, 64) | onechar('e', 56, 64):
			switch(in[8]){
			case onechar('n', 0, 8):
				switch(in[9]){
				case onechar('t', 0, 8):
					return gtok_t_packer_constraints::ASSIGNMENT;
				break;
				default: break;
				}
			break;
	...
	noreturn_report(report_error, ("Found unrecognized child " + std::string(in) + " of <packer_constraints>.").c_str());
}
...
inline gtok_t_vpr_constraints lex_node_t_vpr_constraints(const char *in, const std::function<void(const char *)> *report_error){
	...
	case 18:
		switch(*((triehash_uu64*)&in[0])){
		case onechar('p', 0, 64) | onechar('a', 8, 64) | onechar('c', 16, 64) | onechar('k', 24, 64) | onechar('e', 32, 64) | onechar('r', 40, 64) | onechar('_', 48, 64) | onechar('c', 56, 64):
			switch(*((triehash_uu64*)&in[8])){
			case onechar('o', 0, 64) | onechar('n', 8, 64) | onechar('s', 16, 64) | onechar('t', 24, 64) | onechar('r', 32, 64) | onechar('a', 40, 64) | onechar('i', 48, 64) | onechar('n', 56, 64):
				switch(in[16]){
				case onechar('t', 0, 8):
					switch(in[17]){
					case onechar('s', 0, 8):
						return gtok_t_vpr_constraints::PACKER_CONSTRAINTS;
					break;
					default: break;
					}
				break;
...
inline void load_add_region_required_attributes(const pugi::xml_node &root, int * x_high, int * x_low, int * y_high, int * y_low, const std::function<void(const char *)> * report_error){
	...
	for(pugi::xml_attribute attr = root.first_attribute(); attr; attr = attr.next_attribute()){
		...
		else noreturn_report(report_error, ("Duplicate attribute " + std::string(attr.name()) + " in <add_region>.").c_str());
		switch(in){
...
inline void load_assignment(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug){
	...
	*offset_debug = root.offset_debug();
	...
	for(pugi::xml_attribute attr = root.first_attribute(); attr; attr = attr.next_attribute()){
		atok_t_assignment in = lex_attr_t_assignment(attr.name(), report_error);
		switch(in){
		case atok_t_assignment::NET:
			out.set_assignment_net(attr.value(), context);
			break;
		case atok_t_assignment::PIN:
			out.set_assignment_pin(attr.value(), context);
			break;
		default: break; /* Not possible. */
		}
	}
	...
	if(root.first_child().type() == pugi::node_element)
		noreturn_report(report_error, "Unexpected child element in <assignment>.");

}

constexpr int NUM_T_PACKER_CONSTRAINTS_STATES = 2;
constexpr const int NUM_T_PACKER_CONSTRAINTS_INPUTS = 1;
constexpr int gstate_t_packer_constraints[NUM_T_PACKER_CONSTRAINTS_STATES][NUM_T_PACKER_CONSTRAINTS_INPUTS] = {
	{0},
	{0},
};
template<class T, typename Context>
inline void load_packer_constraints(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug){
	...
	*offset_debug = root.offset_debug();
	...
	if(root.first_attribute())
		noreturn_report(report_error, "Unexpected attribute in <packer_constraints>.");
	...
	size_t assignment_count = 0;
	{
		...
		for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()) {
			*offset_debug = node.offset_debug();
			gtok_t_packer_constraints in = lex_node_t_packer_constraints(node.name(), report_error);
			next = gstate_t_packer_constraints[state][(int)in];
			if(next == -1)
				dfa_error(gtok_lookup_t_packer_constraints[(int)in], gstate_t_packer_constraints[state], gtok_lookup_t_packer_constraints, 1, report_error);
			state = next;
			switch(in) {
			case gtok_t_packer_constraints::ASSIGNMENT:
				assignment_count += 1;
				break;
			default: break; /* Not possible. */
			}
		...
		out.preallocate_packer_constraints_assignment(context, assignment_count);
	}
	int next, state=1;
	for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()){
		*offset_debug = node.offset_debug();
		...
		case gtok_t_packer_constraints::ASSIGNMENT:
			{
				auto child_context = out.add_packer_constraints_assignment(context);
				load_assignment(node, out, child_context, report_error, offset_debug);
				out.finish_packer_constraints_assignment(child_context);
			}
			break;
		default: break; /* Not possible. */
		}
	}
	if(state != 0) dfa_error("end of input", gstate_t_packer_constraints[state], gtok_lookup_t_packer_constraints, 1, report_error);

}

constexpr int NUM_T_VPR_CONSTRAINTS_STATES = 1;
constexpr const int NUM_T_VPR_CONSTRAINTS_INPUTS = 3;
...
inline void load_vpr_constraints(const pugi::xml_node &root, T &out, Context &context, const std::function<void(const char*)> *report_error, ptrdiff_t *offset_debug){
	...
	*offset_debug = root.offset_debug();
	...
	size_t packer_constraints_count = 0;
	{
		int next, state=0;
		for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()) {
			*offset_debug = node.offset_debug();
			...
			if(next == -1)
				dfa_error(gtok_lookup_t_vpr_constraints[(int)in], gstate_t_vpr_constraints[state], gtok_lookup_t_vpr_constraints, 3, report_error);
			state = next;
			switch(in) {
			...
			case gtok_t_vpr_constraints::GLOBAL_ROUTE_CONSTRAINTS:
				...
				break;
			case gtok_t_vpr_constraints::PACKER_CONSTRAINTS:
				packer_constraints_count += 1;
				break;
			default: break; /* Not possible. */
			}
		}
		...
		out.preallocate_vpr_constraints_packer_constraints(context, packer_constraints_count);
	}
	int next, state=0;
	for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()){
		*offset_debug = node.offset_debug();
		...
		case gtok_t_vpr_constraints::PACKER_CONSTRAINTS:
			{
				auto child_context = out.add_vpr_constraints_packer_constraints(context);
				load_packer_constraints(node, out, child_context, report_error, offset_debug);
				out.finish_vpr_constraints_packer_constraints(child_context);
			}
			break;
		default: break; /* Not possible. */
		}
	}
	if(state != 0) dfa_error("end of input", gstate_t_vpr_constraints[state], gtok_lookup_t_vpr_constraints, 3, report_error);
...
template<class T, typename Context>
inline void write_packer_constraints(T &in, std::ostream &os, Context &context){
	(void)in;
	(void)os;
	(void)context;
	{
		for(size_t i=0, n=in.num_packer_constraints_assignment(context); i<n; i++){
			auto child_context = in.get_packer_constraints_assignment(i, context);
			os << "<assignment";
			os << " net=\"" << in.get_assignment_net(child_context) << "\"";
			os << " pin=\"" << in.get_assignment_pin(child_context) << "\"";
			os << "/>\n";
		}
	}
}
...
inline void write_vpr_constraints(T &in, std::ostream &os, Context &context){
	...
	{
		for(size_t i=0, n=in.num_vpr_constraints_packer_constraints(context); i<n; i++){
			auto child_context = in.get_vpr_constraints_packer_constraints(i, context);
			os << "<packer_constraints>\n";
			write_packer_constraints(in, os, child_context);
			os << "</packer_constraints>\n";
		}
	}
}
