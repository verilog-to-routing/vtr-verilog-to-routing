...
bool PartialPlacement::verify_locs(const APNetlist& netlist, size_t grid_width, size_t grid_height) const {
    ...
    if (block_x_locs.size() != netlist.blocks().size()) return false;
    if (block_y_locs.size() != netlist.blocks().size()) return false;
    ...
    for (APBlockId blk_id : netlist.blocks()) {
        ...
        if (std::isnan(x_pos) || x_pos < 0.0 || x_pos >= grid_width) return false;
        if (std::isnan(y_pos) || y_pos < 0.0 || y_pos >= grid_height) return false;
        if (netlist.block_mobility(blk_id) == APBlockMobility::FIXED) {
            const APFixedBlockLoc& fixed_loc = netlist.block_loc(blk_id);
            if (fixed_loc.x != -1 && x_pos != fixed_loc.x) return false;
            if (fixed_loc.y != -1 && y_pos != fixed_loc.y) return false;
        }
    }
...
bool PartialPlacement::verify_layer_nums(const APNetlist& netlist, size_t grid_num_layers) const {
    ...
    if (block_layer_nums.size() != netlist.blocks().size()) return false;
    // Make sure all layer_nums are on the device and fixed locs are correct.
    for (APBlockId blk_id : netlist.blocks()) {
        double layer_num = block_layer_nums[blk_id];
        if (layer_num < 0.0 || layer_num >= static_cast<double>(grid_num_layers)) return false;
        if (netlist.block_mobility(blk_id) == APBlockMobility::FIXED) {
            const APFixedBlockLoc& fixed_loc = netlist.block_loc(blk_id);
            if (fixed_loc.layer_num != -1 && layer_num != fixed_loc.layer_num) return false;
        }
    }
    // If all previous checks passed, return true
    return true;
...
bool PartialPlacement::verify_sub_tiles(const APNetlist& netlist) const {
    ...
    if (block_sub_tiles.size() != netlist.blocks().size()) return false;
    ...
    for (APBlockId blk_id : netlist.blocks()) {
        ...
        if (sub_tile < 0) return false;
        if (netlist.block_mobility(blk_id) == APBlockMobility::FIXED) {
            const APFixedBlockLoc& fixed_loc = netlist.block_loc(blk_id);
            if (fixed_loc.sub_tile != -1 && sub_tile != fixed_loc.sub_tile) return false;
        }
    }
    // If all previous checks passed, return true
    return true;
...
bool PartialPlacement::verify(const APNetlist& netlist,
    ...
    if (!verify_locs(netlist, grid_width, grid_height)) return false;
    if (!verify_layer_nums(netlist, grid_num_layers)) return false;
    if (!verify_sub_tiles(netlist)) return false;
