...
/**
 * @brief Get the scalar mass of the given model.
 *
 ...
 *
 * This returns a vector representing the amount of primitives this block
 * represents.
 */
 ...
 * the tile graph. Corners do no count.
 */
 ...
 *
 * TODO: Should this be put into vtr_geometry?
 */
...
void FlowBasedLegalizer::compute_neighbors_of_bin(LegalizerBinId src_bin_id, size_t num_models) {
    ...
    // manhatten distance between the source bin and the target bin.
...
static inline float compute_cost(const LegalizerBin& src_bin,
    ...
    float weight = weight_vec.manhatten_norm();
...
std::vector<std::vector<LegalizerBinId>> FlowBasedLegalizer::get_paths(LegalizerBinId src_bin_id,
    ...
    VTR_LOGV(log_verbosity_ >= 20, "\t\tSupply of source bin: %.2f\n",
              starting_bin_supply.manhatten_norm());
...
void FlowBasedLegalizer::legalize(PartialPlacement &p_placement) {
    ...
    while (true) {
        ...
        // We take the manhatten (L1) norm here since we only care about the total
        ...
        if (log_verbosity_ >= 10) {
            ...
            print_flow_based_legalizer_status(
                    ...
                    bins_[overfilled_bins_vec.back()].supply.manhatten_norm(),
                    psi);
        }
