...
void TelegramBuffer::append(const ByteArray& bytes) { m_raw_buffer.append(bytes); }

bool TelegramBuffer::check_telegram_header_presence() {
    auto [found, signature_start_index]
        = m_raw_buffer.find_sequence(TelegramHeader::SIGNATURE, TelegramHeader::SIGNATURE_SIZE);
    if (found) {
    ...
    while (may_contain_full_telegram) {
        ...
        if (m_header_opt) {
            ...
            if (m_raw_buffer.size() >= expected_telegram_size) {
                // checksum validation
                ByteArray body(m_raw_buffer.begin() + TelegramHeader::size(),
                               m_raw_buffer.begin() + expected_telegram_size);
                ...
                if (actual_check_sum == header.body_check_sum()) {
                    ...
                    m_errors.push_back("wrong checkSums " + std::to_string(actual_check_sum) + " for " + header.info()
                                       + " , drop this chunk");
                }
