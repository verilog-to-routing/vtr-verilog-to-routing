 ...
 * 3. a virtual source or sink node (SOURCE or SINK), which are starting/ending
 *points of routing trees.
 *
 * Edge
 * ----
 ...
 * While the RRG could contain millions (even much larger) of edges, there are
 *only
 * a limited number of types of switches.
 * Hence, we use a flyweight pattern to store switch-related information that
 *differs
 * only for types of switches (switch type, drive strength, R, C, etc.).
 * Each edge stores the ids of the switch that implements it so this additional
 *information
 * can be easily looked up.
 *
 * Note: All the switch-related information, such as R, C, should be placed in
 *rr_switch_inf
 * but NOT directly in the edge-related data of RRGraph.
 * If you wish to create a new data structure to represent switches between
 *routing resources,
 ...
 *accessor.
 * to ensure that your RRGraph does not include invalid
 *nodes/edges/switches/segements
 * as well as connections.
 * The validate() function gurantees the consistency between internal data
 *structures,
 ...
 *unexpected
 * or unusual in an FPGA, and likely indicates a problem in your graph
 *generation.
 * However, if you are intentionally creating an RRGraph with this unusual,
 * buts still technically legal, behaviour, you can write your own
 ...
 *accessors/mutators
 * Please update and comment on the added features properly to keep this data
 *structure friendly to be extended.
 *
 * Try to keep your extension within only graph-related internal data to
 *RRGraph.
 * In other words, extension is necessary when the new node/edge attributes are
 *needed.
 * RRGraph should NOT include other data which are shared by other data
 *structures outside.
 * The rr-graph is the single largest data structure in VPR,
 * so avoid adding unnecessary information per node or per edge to it, as it
 *will impact memory footprint.
 * Instead, using indices to point to the outside data source instead of
 *embedding to RRGraph
 * For example:
 *   For any placement/routing cost related information, try to extend
 *t_rr_indexed_data, but not RRGraph
 *   For any placement/routing results, try to extend PlaceContext and
 *RoutingContext, but not RRGraph
 *
 ...
 *    To check if your id is valid or not, use the INVALID() function of
 *StrongId class.
 *    Example:
...
class RRGraph {
   ...
   * nodes/edges/switches/segments */
    ...
    typedef vtr::vector<RRSegmentId, RRSegmentId>::const_iterator
        segment_iterator;
   ...
   */
    ...
    size_t node_index(const RRNodeId& node) const; /* TODO: deprecate this
                                                    accessor as outside
                                                    functions should use
                                                    RRNodeId */

    /* get the type of a RRGraph node : types of each node, can be channel wires
   * (CHANX or CHANY) or
   *                                  logic block pins(OPIN or IPIN) or virtual
   * nodes (SOURCE or SINK)
   ...
   *   Therefore, (xlow, ylow) is a starting point for a CHANX/CHANY in
   *INC_DIRECTION
   ...
   * Note that it is ONLY meaningful for CHANX and CHANY, which represents the
   * number of logic blocks that a routing track spans
   * For nodes that are OPIN/IPIN/SOURCE/SINK, the length is supposed to be
   * always 0
   */
   ...
   * See details in the figures for node_xlow(), node_ylow(), node_xhigh() and
   *node_yhigh()
   ...
   * Typically, each node has a capacity of 1 but special nodes (SOURCE and
   * SINK) will have a
   ...
   * (true in VPR RRG that is currently supported, may not be true in customized
   *RRG)
   ...
   * that allows you to differentiate between wires, pins or sources/sinks with
   *overlapping x,y coordinates or extent.
   * This is useful for drawing rr-graphs nicely.
   * For example, all the CHANX rr_nodes that have the same y coordinate and
   *x-coordinates
   * that overlap will have different ptc numbers, by convention starting at 0.
   * This allows them to be drawn without overlapping, as the ptc num can be
   *used as a track identifier.
   ...
   * SOURCE and SINK: the class id of a pin (indicating logic equivalence of
   *pins) in the logic block data structure
   *
   ...
   * node_track_num() is designed for routing tracks, which are CHANX and CHANY
   *nodes
   * node_class_num() is designed for routing source and sinks, which are SOURCE
   *and SINK nodes
   *
   ...
   * Mainly applicable to IPIN and OPIN nodes, which locates on the perimeter of
   *logic block
   * The side should be consistent to <pinlocation> definition in architecture
   *XML
   *
   *             TOP
   *        +-----------+
   ...
   * segment that the node represents. See more details in the data structure
   * t_segment_inf
   */
   ...
   *
   */
    size_t edge_index(const RREdgeId& edge) const; /* TODO: deprecate this
                                                    accessor as outside
                                                    functions should use
                                                    RREdgeId */
   ...
   */
    ...
    /* Switch Info */
    size_t switch_index(const RRSwitchId& switch_id) const; /* TODO: deprecate
                                                             this accessor as
                                                             outside functions
                                                             should use
                                                             RRSwitchId */
    ...
    /* Segment Info */
    size_t segment_index(const RRSegmentId& segment_id)
        const; /* TODO: deprecate this accessor as outside functions should use
                RRSegmentId */
    ...
    std::vector<RREdgeId> find_edges(const RRNodeId& src_node,
                                     const RRNodeId& sink_node) const;
    /* Find a node with given features from internal fast look-up */
    RRNodeId find_node(const short& x,
                       const short& y,
                       const t_rr_type& type,
                       const int& ptc,
                       const e_side& side = NUM_SIDES) const;
    /* Find the number of routing tracks in a routing channel with a given
   * coordinate */
    short chan_num_tracks(const short& x,
                          const short& y,
                          const t_rr_type& type) const;
  ...
  public: /* Echos */
    void print_node(const RRNodeId& node)
        const; /* Print the detailed information of a node */
   ...
   * 3. check that all the nodes representing routing tracks have valid id
   *linked to its segment-related data structure
   * 4. check that all the edges have valid id linked to its switch-related data
   *structure
   *
   * Any valid and non-dirty RRGraph should pass this check
   * It is highly recommended to run this function after building any RRGraph
   *object
   */
    bool validate() const;
   ...
   */
    ...
    /* We use long data type for edges because the number of edges is very large
   */
    ...
    /* Reserve the lists of input/output edges for a RR node to be memory
   *efficient.
   * This function is mainly used to reserve memory space inside RRGraph,
   * when adding a large number of nodes/edge/switches/segments,
   * in order to avoid memory fragements
   ...
   */
    ...
    void reserve_node_out_edges(const RRNodeId& node,
   ...
   */
    RREdgeId create_edge(const RRNodeId& source,
                         const RRNodeId& sink,
                         const RRSwitchId& switch_id);
   ...
   * This function just turn the nodes and edges to an invalid id without
   * deletion
   * This will cause the RRGraph to be marked as dirty (is_dirty(): false =>
   * true)
   ...
   */
    ...
    /* Only applicable to IPIN and OPIN, set the ptc_num for a node, which is the
   * pin id in a logic block,
   * See definition in t_type_descriptor data structure
   */
    void set_node_pin_num(const RRNodeId& node, const short& pin_id);

    /* Only applicable to CHANX and CHANY, set the ptc_num for a node, which is
   * the track id in a routing channel,
   * Routing channel is a group of routing tracks, each of which has a unique
   * index
   * An example
   *       Routing Channel
   *    +------------------+
   ...
   */
    ...
    /* Only applicable to SOURCE and SINK, set the ptc_num for a node, which is
   * the class number in a logic block,
   * See definition in t_type_descriptor data structure
   */
    ...
    /* Set the routing segment linked to a node, only applicable to CHANX and
   * CHANY */
    void set_node_segment(const RRNodeId& node, const RRSegmentId& segment_index);

    /* Edge partitioning is performed for efficiency,
   * so we can store configurable and non-configurable edge lists for a node in
   * one vector,
   * and efficiently iterate over all edges, or only the configurable or
   * non-configurable subsets.
   * This function will re-organize the incoming and outgoing edges of each
   * node,
   ...
   * This will clear the dirty flag (query by is_dirty()) of RRGraph object, if
   * it was set
   */
    void compress();
  ...
  private: /* Internal Mutators to perform edge partitioning */
    /* classify the input edges of each node to be configurable (1st part) and
   * non-configurable (2nd part) */
    void partition_node_in_edges(const RRNodeId& node);

    /* classify the output edges of each node to be configurable (1st part) and
   * non-configurable (2nd part) */
  ...
  private: /* Internal validators and builders */
    ...
    /* Edge-level checking */
    bool validate_node_is_edge_src(const RRNodeId& node,
                                   const RREdgeId& edge) const;
    bool validate_node_is_edge_sink(const RRNodeId& node,
                                    const RREdgeId& edge) const;
  ...
  private: /* Internal Data */
    /* Node related data */
    vtr::vector<RRNodeId, RRNodeId>
        node_ids_; /* Unique identifiers for the nodes */
    ...
    vtr::vector<RRNodeId, RRSegmentId> node_segments_; /* Segment ids for each
                                                        node */
    /* Record the dividing point between configurable and non-configurable edges
   * for each node */
   ...
   */
    vtr::vector<RRSegmentId,
                RRSegmentId>
        segment_ids_;                                  /* unique identifiers for routing
                                            segments which are used in the
                                            RRGraph */
    vtr::vector<RRSegmentId, t_segment_inf> segments_; /* detailed information
                                                        about the segments,
                                                        which are used in the
                                                        RRGraph */

    /* Misc. */
    /* A flag to indicate if the graph contains invalid elements (nodes/edges
   * etc.) */
    bool dirty_ = false;

    /* Fast look-up to search a node by its type, coordinator and ptc_num
   * Indexing of fast look-up:
