...
void VprTimingGraphResolver::get_detailed_interconnect_components_helper(std::vector<tatum::DelayComponent>& components, t_rt_node* node) const {
    ...
    while (node != nullptr) {
        ...
        if (rr_type == OPIN
            ...
            if (device_ctx.rr_graph.node_type(RRNodeId(node->inode)) == CHANX || device_ctx.rr_graph.node_type(RRNodeId(node->inode)) == CHANY) { //for channels, we would like to describe the component with segment specific information
