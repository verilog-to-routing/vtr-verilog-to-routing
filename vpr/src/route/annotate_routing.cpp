...
vtr::vector<RRNodeId, ClusterNetId> annotate_rr_node_nets(const ClusteringContext& cluster_ctx,
    ...
    for (auto net_id : netlist.nets()) {
        ...
        for (auto& rt_node : tree->all_nodes()) {
            ...
            if ((SOURCE != rr_graph.node_type(rr_node)) && (SINK != rr_graph.node_type(rr_node))) {
                 ...
                 * whose capacity is 1
                 */
                if ((rr_node_nets[rr_node]) && (1 == rr_graph.node_capacity(rr_node))
                    && (net_id != rr_node_nets[rr_node])) {
                    VPR_FATAL_ERROR(
                        VPR_ERROR_ANALYSIS,
                        "Detect two nets '%s' and '%s' that are mapped to the same rr_node '%ld'!\n%s\n",
                        netlist.net_name(net_id).c_str(), netlist.net_name(rr_node_nets[rr_node]).c_str(),
                        size_t(rr_node),
                        describe_rr_node(rr_graph, device_ctx.grid, device_ctx.rr_indexed_data, rr_node, false)
                            .c_str());
                } else {
