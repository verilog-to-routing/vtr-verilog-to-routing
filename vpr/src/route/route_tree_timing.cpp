...
static t_rt_node* add_non_configurable_to_route_tree(const int rr_node, const bool reached_by_non_configurable_edge, std::unordered_set<int>& visited, PathManager& rcv_path_manager);
...
t_rt_node* update_route_tree(t_heap* hptr, int target_net_pin_index, SpatialRouteTreeLookup* spatial_rt_lookup, PathManager& rcv_path_manager) {
    ...
    start_of_new_subtree_rt_node = add_subtree_to_route_tree(hptr, target_net_pin_index, &sink_rt_node, rcv_path_manager);
...
static t_rt_node* 
add_subtree_to_route_tree(t_heap* hptr, int target_net_pin_index, t_rt_node** sink_rt_node_ptr, PathManager& rcv_path_manager) {
    ...
    for (int rr_node : main_branch_visited) {
        add_non_configurable_to_route_tree(rr_node, false, all_visited, rcv_path_manager);
    }
...
static t_rt_node* add_non_configurable_to_route_tree(const int rr_node, const bool reached_by_non_configurable_edge, std::unordered_set<int>& visited, PathManager& rcv_path_manager) {
    ...
    if (!visited.count(rr_node) || !reached_by_non_configurable_edge) {
        ...
        for (int iedge : device_ctx.rr_nodes[rr_node].non_configurable_edges()) {
            ...
            //Recurse
            t_rt_node* child_rt_node = add_non_configurable_to_route_tree(to_rr_node, true, visited, rcv_path_manager);
