...
static void profile_lookahead_overestimation(std::ofstream& os,
    ...
    // We are going to collect info on the overestimation from each RR node type.
    ...
    for (unsigned i = 0; i < max_attempts; i++) {
        ...
        for (RRNodeId rr_node_id : sink_rr_nodes) {
            ...
            if (heuristic_delay > path_delay) {
                ...
                t_overestimation_info& overestimation_info = max_overestimation_per_type[(int)sample_rr_node_type];
    ...
    for (size_t l = 0; l < max_overestimation_per_type.size(); l++) {
        os << vtr::string_fmt("\t%6s: %.3g\n",
                              ...
                              max_overestimation_per_type[l].overestimation);
    }
    os << "\n";
    ...
    for (size_t l = 0; l < max_overestimation_per_type.size(); l++) {
        const t_overestimation_info& overestimation_info = max_overestimation_per_type[l];
        ...
        os << vtr::string_fmt("\t\t%zu -> %zu (%s)\n",
                              ...
                              get_rr_node_type_str(device_ctx.rr_graph.node_type(overestimation_info.target_node)).c_str());
