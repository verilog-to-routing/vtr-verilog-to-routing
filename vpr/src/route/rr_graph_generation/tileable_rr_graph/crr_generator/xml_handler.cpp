...
inline void write_attr(std::ostream& os, const char* name, const std::string& v) {
  os << ' ' << name << "=\""; xml_escape_and_write(os, v); os << '"';
}
template <typename T>
...
void XMLHandler::write_channels(std::ostream& out, const RRGraph& graph) {
  out << "  <channels>\n";
  for (const auto& channel : graph.get_channels()) write_channel(out, channel);
  for (const auto& xy_list : graph.get_xy_lists()) write_xy_list(out, xy_list);
  out << "  </channels>\n";
}
...
void XMLHandler::write_switches(std::ostream& out, const RRGraph& graph) {
  out << "  <switches>\n";
  for (const auto& s : graph.get_switches()) write_switch(out, s);
  out << "  </switches>\n";
}
...
void XMLHandler::write_block_types(std::ostream& out, const RRGraph& graph) {
  out << "  <block_types>\n";
  for (const auto& bt : graph.get_block_types()) write_block_type(out, bt);
  out << "  </block_types>\n";
}
...
void XMLHandler::write_block_type(std::ostream& out, const BlockType& bt) {
  ...
  out << ">\n";
  for (const auto& pc : bt.get_pin_classes()) write_pin_class(out, pc);
  out << "    </block_type>\n";
}
...
void XMLHandler::write_pin_class(std::ostream& out, const PinClass& pc) {
  ...
  out << ">\n";
  for (const auto& p : pc.get_pins()) write_pin(out, p);
  out << "      </pin_class>\n";
}
...
void XMLHandler::write_grid(std::ostream& out, const RRGraph& graph) {
  out << "  <grid>\n";
  for (const auto& gl : graph.get_grid_locs()) write_grid_loc(out, gl);
  out << "  </grid>\n";
}
...
void XMLHandler::write_nodes(std::ostream& out, const RRGraph& graph) {
  out << "  <rr_nodes>\n";
  for (const auto& n : graph.get_nodes()) write_node(out, n);
  out << "  </rr_nodes>\n";
}
...
void XMLHandler::write_edges(std::ostream& out, const RRGraph& graph) {
  out << "  <rr_edges>\n";
  for (const auto& e : graph.get_edges()) write_edge(out, e);
  out << "  </rr_edges>\n";
}
