...
class RRNode {
 public:
  RRNode() = default;
  RRNode(NodeId id, NodeType type, int capacity, const std::string& direction,
         const Location& location, const NodeTiming& timing,
         const NodeSegmentId& segment_id)
      : id_(id),
        type_(type),
        capacity_(capacity),
  ...
  bool is_single_coordinate() const {
    return (location_.x_low == location_.x_high) &&
           (location_.y_low == location_.y_high);
  }
...
class RREdge {
 public:
  ...
  RREdge(NodeId src_node, NodeId sink_node, SwitchId switch_id)
      : src_node_(src_node), sink_node_(sink_node), switch_id_(switch_id) {}
  ...
  bool operator<(const RREdge& other) const {
    return std::tie(src_node_, sink_node_, switch_id_) <
           std::tie(other.src_node_, other.sink_node_, other.switch_id_);
  }

  bool operator==(const RREdge& other) const {
    return src_node_ == other.src_node_ && sink_node_ == other.sink_node_ &&
...
class XYList {
 public:
  enum class Type { X_LIST, Y_LIST, INVALID_LIST };

  XYList() = default;
  XYList(Type type, int index, int info)
      : type_(type), index_(index), info_(info) {}
...
class Pin {
 public:
  Pin() = default;
  Pin(const int ptc, const std::string& value) : ptc_(ptc), value_(value) {}
...
class BlockType {
 public:
  BlockType() = default;
  BlockType(int id, const std::string& name, int height, int width,
...
class GridLoc {
 public:
  GridLoc() = default;
  GridLoc(int type_id, int height_offset, int width_offset, int x, int y,
          int layer)
      : type_id_(type_id),
...
class Switch {
 public:
  Switch() = default;
  Switch(SwitchId id, const std::string& name, const std::string& type,
         const Timing& timing, const Sizing& sizing)
      : id_(id), name_(name), type_(type), timing_(timing), sizing_(sizing) {}
...
class Segment {
 public:
  Segment() = default;
  Segment(SegmentId id, const std::string& name, int length,
          const std::string& res_type)
      : id_(id), name_(name), length_(length), res_type_(res_type) {}
...
class RRGraph {
 public:
  ...
  void add_node(NodeId id, NodeType type, int capacity,
  ...
  void add_switch(SwitchId id, const std::string& name, const std::string& type,
                  const Timing& timing, const Sizing& sizing);
  ...
  void add_segment(SegmentId id, const std::string& name, int length,
                   const std::string& res_type);
  ...
  void add_block_type(int id, const std::string& name, int height, int width,
                      const std::vector<PinClass>& pin_classes);
  ...
  void add_grid_loc(int type_id, int height_offset, int width_offset, int x,
                    int y, int layer);
  ...
  void shrink_to_fit() { nodes_.shrink_to_fit(); edges_.shrink_to_fit(); }
  ...
  // Edge filtering
  std::vector<RREdge> get_preserved_edges(
      const bool preserve_ipin_connections,
      const bool preserve_opin_connections) const;

  // Metadata
  void set_tool_info(const std::string& name, const std::string& version,
