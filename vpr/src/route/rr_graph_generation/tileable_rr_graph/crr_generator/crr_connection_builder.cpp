...
CRRConnectionBuilder::CRRConnectionBuilder(const RRGraph& rr_graph,
    ...
    : rr_graph_(rr_graph), node_lookup_(node_lookup), sb_manager_(sb_manager) {}
...
void CRRConnectionBuilder::build_connections_for_location(Coordinate x,
  ...
  for (auto row_iter = df->begin(); row_iter != df->end(); ++row_iter) {
    ...
    if (row_idx < NUM_EMPTY_ROWS) continue;
      ...
      if (!cell.is_empty()) {
        ...
        if (source_it != source_nodes.end() && sink_it != sink_nodes.end()) {
          ...
          if (source_node_type == NodeType::IPIN) {
            ...
            if (sink_node_type == NodeType::CHANX ||
                sink_node_type == NodeType::CHANY) {
...
std::vector<Connection> CRRConnectionBuilder::get_tile_connections(Coordinate tile_x, Coordinate tile_y) {
  std::vector<Connection> tile_connections;
...
std::map<size_t, NodeId> CRRConnectionBuilder::get_vertical_nodes(
  ...
  for (size_t row = NUM_EMPTY_ROWS; row < df.rows(); ++row) {
    ...
    } else if (info.side == Side::LEFT || info.side == Side::RIGHT ||
               info.side == Side::TOP || info.side == Side::BOTTOM) {
      node_id =
...
CRRConnectionBuilder::SegmentInfo CRRConnectionBuilder::parse_segment_info(
    const DataFrame& df, size_t row_or_col, bool is_vertical) {
  SegmentInfo info;
...
NodeId CRRConnectionBuilder::process_channel_node(
    ...
    int& prev_seg_index, Side& prev_side, std::string& prev_seg_type,
    int& prev_ptc_number, bool is_vertical) {
  // Check grid boundaries
  if ((info.side == Side::RIGHT && x == fpga_grid_x_) ||
      (info.side == Side::TOP && y == fpga_grid_y_)) {
    return 0;
  }
...
void CRRConnectionBuilder::calculate_segment_coordinates(
    ...
    Coordinate& x_high, Coordinate& y_low, Coordinate& y_high,
    int& physical_length, int& truncated, bool is_vertical) {
  ...
  truncated = (std::max(x_low, static_cast<Coordinate>(1)) - x_low) -
              (x_high - std::min(x_high, fpga_grid_x_));
  truncated += (std::max(y_low, static_cast<Coordinate>(1)) - y_low) -
...
SwitchId CRRConnectionBuilder::get_edge_switch_id(
    const std::string& cell_value, const std::string& sink_node_type,
  ...
  if (is_integer(cell_value) && is_annotated_excel_) {
    ...
    if (default_switch_id_.find(switch_id_key) !=
        default_switch_id_.end()) {
      return default_switch_id_[switch_id_key];
    } else if (default_switch_id_.find(capitalized_switch_id_key) !=
        default_switch_id_.end()) {
      return default_switch_id_[capitalized_switch_id_key];
    } else {
      throw std::runtime_error("Default switch id not found for Node Type: " +
                               lower_case_sink_node_type + " and Switch ID Key: " +
                               capitalized_switch_id_key);
    }
  }
}
...
void CRRConnectionBuilder::update_progress() {
  ...
  if (current % std::max(size_t(1), total_locations_ / 20) == 0 ||
