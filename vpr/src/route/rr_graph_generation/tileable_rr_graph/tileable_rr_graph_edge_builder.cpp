...
void build_rr_graph_regular_edges(const RRGraphView& rr_graph,
    ...
    // Pre-create one CRR switch per integer delay (ps) in the range observed
    // across all switch template files. Edge building then looks up the
    // switch id by delay and asserts if a connection's delay is missing.
    std::unordered_map<int, RRSwitchId> delay_to_switch_id;
    if (build_crr_edges) {
        delay_to_switch_id = pre_create_crr_switches(sb_manager->get_switch_delay_min_ps(),
                                                     sb_manager->get_switch_delay_max_ps(),
                                                     route_verbosity);
    }
    /* Go Switch Block by Switch Block */
    for (size_t ix = 0; ix <= gsb_range.x(); ++ix) {
