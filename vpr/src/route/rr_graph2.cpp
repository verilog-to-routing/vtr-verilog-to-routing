...
bool verify_rr_node_indices(const DeviceGrid& grid, const t_rr_node_indices& rr_node_indices, const t_rr_graph_storage& rr_nodes) {
                    ...
                    for (int inode : rr_node_indices[rr_type][x][y][side]) {
                        ...
                        if (device_ctx.rr_graph.node_type(RRNodeId(inode)) != rr_type) {
                            VPR_ERROR(VPR_ERROR_ROUTE, "RR node type does not match between rr_nodes and rr_node_indices (%s/%s): %s",
                                      rr_node_typename[device_ctx.rr_graph.node_type(RRNodeId(inode))],
                                      rr_node_typename[rr_type],
                                      describe_rr_node(inode).c_str());
                        }

                        if (device_ctx.rr_graph.node_type(RRNodeId(inode)) == CHANX) {
                        ...
                        } else if (device_ctx.rr_graph.node_type(RRNodeId(inode)) == CHANY) {
                            ...
                            VTR_ASSERT(device_ctx.rr_graph.node_type(RRNodeId(inode)) == IPIN || device_ctx.rr_graph.node_type(RRNodeId(inode)) == OPIN);
    ...
    for (auto kv : rr_node_counts) {
        ...
        if (device_ctx.rr_graph.node_type(RRNodeId(inode)) == SOURCE || device_ctx.rr_graph.node_type(RRNodeId(inode)) == SINK) {
             ...
             */
        } else if ((OPIN != device_ctx.rr_graph.node_type(RRNodeId(inode))) && (IPIN != device_ctx.rr_graph.node_type(RRNodeId(inode)))) {
...
void add_to_rr_node_indices(t_rr_node_indices& rr_node_indices, const t_rr_graph_storage& rr_nodes, int inode) {
    ...
    for (int x = rr_node.xlow(); x <= rr_node.xhigh(); ++x) {
        for (int y = rr_node.ylow(); y <= rr_node.yhigh(); ++y) {
            auto rr_type = device_ctx.rr_graph.node_type(RRNodeId(inode));
            if (rr_type == IPIN || rr_type == OPIN) {
                for (const e_side& side : SIDES) {
                    if (!rr_node.is_node_on_specific_side(side)) {
                        continue;
                    }
                    insert_at_ptc_index(rr_node_indices[device_ctx.rr_graph.node_type(RRNodeId(inode))][x][y][side], rr_node.ptc_num(), inode);
                }
            } else if (rr_type == CHANX) {
                //CHANX uses odd swapped x/y indices....
                insert_at_ptc_index(rr_node_indices[device_ctx.rr_graph.node_type(RRNodeId(inode))][y][x][0], rr_node.ptc_num(), inode);
            } else {
                insert_at_ptc_index(rr_node_indices[device_ctx.rr_graph.node_type(RRNodeId(inode))][x][y][0], rr_node.ptc_num(), inode);
            }
        }
    }
}
