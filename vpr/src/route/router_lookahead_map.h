...
class MapLookahead : public RouterLookahead {
  ...
  private:
    ...
    std::unordered_map<t_physical_tile_type_ptr, util::t_ipin_primitive_sink_delays> inter_tile_pin_primitive_pin_delay;
    std::unordered_map<t_physical_tile_type_ptr, std::unordered_map<int, util::Cost_Entry>> tile_min_cost;
    vtr::NdMatrix<util::Cost_Entry, 2> distance_based_min_cost;
