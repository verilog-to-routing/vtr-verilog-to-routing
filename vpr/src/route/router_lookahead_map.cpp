 ...
 * @param device_ctx
 */
static void compute_tiles_lookahead(
 ...
 */
static void compute_tile_lookahead(
    std::unordered_map<int, util::t_ipin_primitive_sink_delays>& intra_tile_pin_primitive_pin_delay,
 ...
 * @param intra_tile_pin_primitive_pin_delay [physical_tile_type_idx][from_pin_ptc_num][sink_ptc_num] -> cost
 */
static void store_min_cost_to_sinks(
    std::unordered_map<int, std::unordered_map<int, util::Cost_Entry>>& tile_min_cost,
    t_physical_tile_type_ptr physical_tile,
...
static void set_lookahead_map_costs(int from_layer_num,
                                    int segment_index,
                                    e_rr_type chan_type,
...
static util::Cost_Entry get_nearby_cost_entry(int from_layer_num,
                                              int x,
                                              int y,
                                              int to_layer_num,
                                              int segment_index,
...
float MapLookahead::get_expected_cost(RRNodeId current_node,
...
float MapLookahead::get_expected_cost_flat_router(RRNodeId current_node,
                                                  RRNodeId target_node,
                                                  const t_conn_cost_params& params,
                                                  float R_upstream) const {
    auto& device_ctx = g_vpr_ctx.device();
    ...
    t_physical_tile_type_ptr from_physical_type = device_ctx.grid.get_physical_type(
        {rr_graph.node_xlow(current_node), rr_graph.node_ylow(current_node), rr_graph.node_layer(current_node)});
    int from_node_ptc_num = rr_graph.node_ptc_num(current_node);
    t_physical_tile_type_ptr to_physical_type = device_ctx.grid.get_physical_type(
        {rr_graph.node_xlow(target_node), rr_graph.node_ylow(target_node), rr_graph.node_layer(target_node)});
    ...
    if (from_rr_type == CHANX || from_rr_type == CHANY) {
        ...
        if (is_inter_cluster_node(rr_graph, current_node)) {
            ...
            std::tie(delay_cost, cong_cost)
                = get_expected_delay_and_cong(current_node, target_node, params, R_upstream);
            ...
            if (node_in_same_physical_tile(current_node, target_node)) {
                delay_offset_cost = 0.;
                cong_offset_cost = 0.;
                const auto& pin_delays
                    = intra_tile_pin_primitive_pin_delay.at(from_physical_type->index)[from_node_ptc_num];
                ...
                delay_cost
                    = params.criticality
                      * chann_distance_based_min_cost[rr_graph.node_layer(current_node)][to_layer_num][delta_x][delta_y]
                            .delay;
                cong_cost
                    = (1. - params.criticality)
                      * chann_distance_based_min_cost[rr_graph.node_layer(current_node)][to_layer_num][delta_x][delta_y]
                            .congestion;
...
std::pair<float, float> MapLookahead::get_expected_delay_and_cong(RRNodeId from_node,
                                                                  RRNodeId to_node,
                                                                  const t_conn_cost_params& params,
                                                                  float /*R_upstream*/) const {
    auto& device_ctx = g_vpr_ctx.device();
    ...
    if (from_type == SOURCE || from_type == OPIN) {
        ...
        t_physical_tile_type_ptr from_tile_type = device_ctx.grid.get_physical_type(
            {rr_graph.node_xlow(from_node), rr_graph.node_ylow(from_node), from_layer_num});
        ...
        std::tie(expected_delay_cost, expected_cong_cost)
            = util::get_cost_from_src_opin(src_opin_delays[from_layer_num][from_tile_index][from_ptc][to_layer_num],
                                           delta_x, delta_y, to_layer_num, get_wire_cost_entry);

        expected_delay_cost *= params.criticality;
        expected_cong_cost *= (1 - params.criticality);

        VTR_ASSERT_SAFE_MSG(
            std::isfinite(expected_delay_cost),
            vtr::string_fmt(
                "Lookahead failed to estimate cost from %s: %s", rr_node_arch_name(from_node, is_flat_).c_str(),
                describe_rr_node(rr_graph, device_ctx.grid, device_ctx.rr_indexed_data, from_node, is_flat_).c_str())
                .c_str());
        ...
        util::Cost_Entry cost_entry
            = get_wire_cost_entry(from_type, from_seg_index, from_layer_num, delta_x, delta_y, to_layer_num);
...
void MapLookahead::compute_intra_tile() {
    ...
    compute_tiles_lookahead(intra_tile_pin_primitive_pin_delay, tile_min_cost, det_routing_arch_, g_vpr_ctx.device());
}
...
void MapLookahead::write_intra_cluster(const std::string& file) const {
    write_intra_cluster_router_lookahead(file, intra_tile_pin_primitive_pin_delay);
}

float MapLookahead::get_opin_distance_min_delay(int physical_tile_idx,
                                                int from_layer,
                                                int to_layer,
                                                int dx,
                                                int dy) const {
...
static void compute_router_wire_lookahead(const std::vector<t_segment_inf>& segment_inf_vec) {
    ...
    f_wire_cost_map = t_wire_cost_map({static_cast<unsigned long>(grid.get_num_layers()),
                                       static_cast<unsigned long>(grid.get_num_layers()), 2, segment_inf_vec.size(),
    ...
    for (unsigned to_layer = 0; to_layer < routing_cost_map.dim_size(0); to_layer++) {
        for (unsigned ix = 0; ix < routing_cost_map.dim_size(1); ix++) {
            for (unsigned iy = 0; iy < routing_cost_map.dim_size(2); iy++) {
                ...
                f_wire_cost_map[from_layer_num][to_layer][chan_index][segment_index][ix][iy]
        ...
        for (int to_layer_num = 0; to_layer_num < device_ctx.grid.get_num_layers(); ++to_layer_num) {
            for (unsigned ix = 0; ix < device_ctx.grid.width(); ix++) {
                for (unsigned iy = 0; iy < device_ctx.grid.height(); iy++) {
                    util::Cost_Entry cost_entry
                        = f_wire_cost_map[from_layer_num][to_layer_num][chan_index][segment_index][ix][iy];

                    if (std::isnan(cost_entry.delay) && std::isnan(cost_entry.congestion)) {
                        util::Cost_Entry copied_entry = get_nearby_cost_entry_average_neighbour(
                            from_layer_num, static_cast<int>(ix), static_cast<int>(iy), to_layer_num, segment_index,
                            chan_index);
...
static util::Cost_Entry get_nearby_cost_entry(int from_layer_num,
    ...
    util::Cost_Entry copy_entry
        = f_wire_cost_map[from_layer_num][to_layer_num][chan_index][segment_index][copy_x][copy_y];
...
static util::Cost_Entry get_nearby_cost_entry_average_neighbour(int from_layer_num,
    ...
    VTR_ASSERT(std::isnan(
        f_wire_cost_map[from_layer_num][to_layer_num][chan_index][segment_index][missing_dx][missing_dy].delay));
    VTR_ASSERT(std::isnan(
        f_wire_cost_map[from_layer_num][to_layer_num][chan_index][segment_index][missing_dx][missing_dy].congestion));
...
static void compute_tile_lookahead(
    ...
    int x = 1;
    int y = 1;
    build_tile_rr_graph(rr_graph_builder, det_routing_arch, physical_tile, layer, x, y, delayless_switch);

    RRGraphView rr_graph{rr_graph_builder.rr_nodes(),         rr_graph_builder.node_lookup(),
                         rr_graph_builder.rr_node_metadata(), rr_graph_builder.rr_edge_metadata(),
                         g_vpr_ctx.device().rr_indexed_data,  g_vpr_ctx.device().rr_rc_data,
                         rr_graph_builder.rr_segments(),      rr_graph_builder.rr_switch()};

    util::t_ipin_primitive_sink_delays pin_delays
        = util::compute_intra_tile_dijkstra(rr_graph, physical_tile, layer, x, y);
...
static void store_min_cost_to_sinks(
    ...
    for (auto& primitive_sink_pair : physical_tile->primitive_class_inf) {
        ...
        auto min_cost
            = util::Cost_Entry(std::numeric_limits<float>::max() / 1e12, std::numeric_limits<float>::max() / 1e12);
...
static void min_chann_global_cost_map(vtr::NdMatrix<util::Cost_Entry, 4>& distance_min_cost) {
    ...
    distance_min_cost.resize({static_cast<unsigned long>(num_layers), static_cast<unsigned long>(num_layers),
                    ...
                    for (int chan_idx = 0; chan_idx < (int)f_wire_cost_map.dim_size(2); chan_idx++) {
                        for (int seg_idx = 0; seg_idx < (int)f_wire_cost_map.dim_size(3); seg_idx++) {
                            auto cost = util::Cost_Entry(
                                f_wire_cost_map[from_layer_num][to_layer_num][chan_idx][seg_idx][dx][dy].delay,
...
static void min_opin_distance_cost_map(const util::t_src_opin_delays& src_opin_delays,
    ...
    int width = (int)g_vpr_ctx.device().grid.width();
    int height = (int)g_vpr_ctx.device().grid.height();
    distance_min_cost.resize({static_cast<unsigned long>(num_tile_types), static_cast<unsigned long>(num_layers),
                              static_cast<unsigned long>(num_layers), static_cast<unsigned long>(width),
                                ...
                                if (layer_src_opin_delay_map.empty()) {
                                    ...
                                    for (const auto& kv : layer_src_opin_delay_map) {
                                        ...
                                        wire_cost_entry = get_wire_cost_entry(
                                            reachable_wire_inf.wire_rr_type, reachable_wire_inf.wire_seg_index,
                                            reachable_wire_inf.layer_number, dx, dy, to_layer_num);
...
static void toIntEntry(std::vector<int>& out, int idx, const int& cost) { out[idx] = cost; }

static void fromIntEntry(::capnp::List<int64_t, ::capnp::Kind::PRIMITIVE>::Builder& out, int idx, const int& cost) {
    out.set(idx, cost);
}
...
static void getIntraClusterArrayFlatSize(
    int& num_tile_types,
    int& num_pins,
    int& num_sinks,
    const std::unordered_map<int, util::t_ipin_primitive_sink_delays>& intra_tile_pin_primitive_pin_delay) {
...
static void read_intra_cluster_router_lookahead(
    ...
    toVector<int64_t, int>(physical_tile_num_pin_arr, map.getPhysicalTileNumPins(), toIntEntry);

    std::vector<int> pin_num_sink_arr;
    toVector<int64_t, int>(pin_num_sink_arr, map.getPinNumSinks(), toIntEntry);
    ...
    for (int physical_tile_idx = 0; physical_tile_idx < (int)physical_tile_num_pin_arr.size(); physical_tile_idx++) {
        ...
        for (int pin_num = 0; pin_num < num_pins; pin_num++) {
            ...
            toUnorderedMap<int64_t, VprMapCostEntry, int, util::Cost_Entry>(
                pin_sink_cost_map, num_seen_pair, num_seen_pair + pin_num_sink_arr[num_seen_pin], map.getPinSinks(),
                map.getPinSinkCosts(), toPairEntry);
...
static void write_intra_cluster_router_lookahead(
    ...
    getIntraClusterArrayFlatSize(num_tile_types, num_pins, num_sinks, intra_tile_pin_primitive_pin_delay);
    ...
    std::vector<int> physical_tile_num_pin_arr(num_tile_types, 0);
    {
        ...
        fromVector<int64_t, int>(physical_tile_num_pin_arr_builder, physical_tile_num_pin_arr, fromIntEntry);
    }
    ...
    std::vector<int> pin_num_sink_arr(num_pins, 0);
    {
        ...
        fromVector<int64_t, int>(pin_num_sink_arr_builder, pin_num_sink_arr, fromIntEntry);
    }

    {
        ...
        for (int physical_tile_idx = 0; physical_tile_idx < num_tile_types; ++physical_tile_idx) {
            for (int pin_num = 0; pin_num < physical_tile_num_pin_arr[physical_tile_idx]; ++pin_num) {
                const std::unordered_map<int, util::Cost_Entry>& pin_sinks
                    = intra_tile_pin_primitive_pin_delay.at(physical_tile_idx).at(pin_num);
                FromUnorderedMap<int64_t, VprMapCostEntry, int, util::Cost_Entry>(
                    pin_sink_arr_builder, pin_sink_cost_builder, num_seen_pin, pin_sinks, fromPairEntry);
