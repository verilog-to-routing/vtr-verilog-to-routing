...
static void adjust_one_rr_occ_and_acc_cost(int inode,
                                           int add_or_sub,
                                           float acc_fac,
...
void pathfinder_incremental_update_acc_cost(float acc_fac,
    ...
    /* This routine performs the same task as pathfinder_update_acc_cost, but do so  *
     * in an incremental way. Instead of going through each rr node, it goes through *
     * the traceback of all the rerouted nets. We also need to account for the       *
     * overused nodes generated by the function reserve_locally_used_opins.          *
     *
     * This incremental technique should be more efficient, since usually only a     *
     * portion of the RR graph is used by routing, particularly on large devices.    *
     *
     ...
     * overused_nodes to avoid double counting the overuse. Upon completion, we also *
     * update the overuse info for the current routing iteration.                    */
    ...
    for (auto net_id : rerouted_nets) {
        for (t_trace* tptr = route_ctx.trace[net_id].head; tptr != nullptr; tptr = tptr->next) {
            ...
            if (overuse <= 0) { // Not overused
                continue;
            }

            auto result = checked_nodes.insert(inode);
            if (!result.second) { //Already updated
                continue;
            }
    ...
    // Go through updated local opins
    for (auto inode : overused_local_nodes) {
        overuse = route_ctx.rr_node_route_inf[inode].occ() - device_ctx.rr_nodes[inode].capacity();
        if (overuse <= 0) { // Not overused
            continue;
        }
...
//
// Record any rr_node ID that has been detected as overused. Might not stay overused upon the exit of this function.
        ...
        for (auto blk_id : cluster_ctx.clb_nlist.blocks()) {
            ...
            for (iclass = 0; iclass < (int)type->class_inf.size(); iclass++) {
                ...
                for (ipin = 0; ipin < num_local_opin; ipin++) {
                    ...
                    adjust_one_rr_occ_and_acc_cost(inode, -1, acc_fac, overused_local_nodes);
                }
            }
        }
    }
...
static void adjust_one_rr_occ_and_acc_cost(int inode,
    ...
    if (new_occ > capacity) {
        overused_local_nodes.insert(inode);
        if (add_or_sub == 1) {
