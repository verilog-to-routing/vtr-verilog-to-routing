...
static std::tuple<t_rr_type, int, RRNodeId> get_pin_spec_from_class (RRGraphBuilder& rr_graph_builder,
                                                                    t_physical_tile_type_ptr physical_tile,
                                                                    int i,
                                                                    int j,
                                                                    int pin_logical_num,
...
static std::function<void(t_chan_width*)>   alloc_and_load_rr_graph(RRGraphBuilder& rr_graph_builder,
    ...
    *Fc_clipped = false;
    ...
    // #TODO: Internal edges can't be built in stage - Reason: The mode of the blocks is not known, thus extra edges would be build, which may confuse the router
//    if(is_flat) {
...
static void add_rr_class(const t_class* class_inf,
    ...
    inode = rr_graph_builder.node_lookup().find_node(i, j, node_type, class_id);

    for (int ipin = 0; ipin < class_inf->num_pins; ++ipin) {
        int pin_logical_num = class_inf->pinlist[ipin];
        t_rr_type pin_type;
        RRNodeId pin_node_id;
        std::tie(pin_type, std::ignore, pin_node_id) = get_pin_spec_from_class(rr_graph_builder,
                                                                               physical_tile,
                                                                               i,
                                                                               j,
                                                                               pin_logical_num,
                                                                               class_id);
        VTR_ASSERT(pin_type == IPIN || pin_type == OPIN);
        if(class_type == DRIVER)
            rr_edges_to_create.emplace_back(inode, pin_node_id, delayless_switch);
        else
            rr_edges_to_create.emplace_back(pin_node_id, inode, delayless_switch);
    }
        ...
        for (e_side side : sides) {
            for (int width_offset = 0; width_offset < max_width_offset; ++width_offset) {
                for (int height_offset = 0; height_offset < max_height_offset; ++height_offset) {
                    if (is_tile_pin) {
                        if (!physical_tile->pinloc[width_offset][height_offset][side][ipin])
                            continue;
                    }
...
//                const t_pb_graph_pin* from_pin = &(pins[port_idx][pin_idx]);
                // TODO: This is temporarily commented
...
//
//    std::tie(node_type, node_ptc, node_id) = get_pin_spec_from_class (rr_graph_builder,
//                                                          physical_tile,
//                                                          sub_tile,
//                                                          logical_block,
//                                                          sub_tile_relative_cap,
...
//        t_rr_type sec_node_type;
//        int sec_node_ptc;
//        RRNodeId sec_node_id;
//        std::tie(sec_node_type, sec_node_ptc, sec_node_id) = get_pin_spec_from_class(rr_graph_builder,
//                                                                          physical_tile,
//                                                                          sub_tile,
//                                                                          logical_block,
//                                                                          sub_tile_relative_cap,
...
//    int num_edges;
//    int num_pins;
//    auto pin_type = get_pin_type_from_pin_physical_num(pb_graph_pin);
