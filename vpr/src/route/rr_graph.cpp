...
static void add_chain_node_fan_in_edges(RRGraphBuilder& rr_graph_builder,
                            ...
                            const std::vector<int>& chain_pins,
                            float R_minW_nmos,
                            float R_minW_pmos,
                            int chain_idx,
                            int node_idx,
...
static void add_intra_tile_edges_rr_graph(RRGraphBuilder& rr_graph_builder,
    ...
    for (int pin_physical_num : pin_num_vec) {
        ...
        float primitive_comb_delay = 0.;
        if(is_primitive_pin(physical_tile, pin_physical_num)) {
            primitive_comb_delay = get_pin_primitive_comb_delay(physical_tile,
                                                                logical_block,
                                                                pin_physical_num);
        }
        for (auto driving_pin : driving_pins) {
            ...
            if(primitive_comb_delay != 0.) {
                float edge_delay = get_edge_delay(physical_tile,
                                                  logical_block,
                                                  driving_pin,
                                                  pin_physical_num);
                sw_idx = find_create_intra_cluster_sw_arch_idx(arch_sw_inf_map,
                                                               edge_delay + primitive_comb_delay);
            } else {
                sw_idx = get_edge_sw_arch_idx(physical_tile,
                                              logical_block,
                                              driving_pin,
                                              pin_physical_num);
            }
...
static void add_pb_edges(RRGraphBuilder& rr_graph_builder,
    ...
    for (auto pin_physical_num = pin_num_range.low; pin_physical_num <= pin_num_range.high; pin_physical_num++) {
        ...
        for (auto conn_pin_physical_num : conn_pins_physical_num) {
            // The pin belongs to a chain - incoming edges to this pin will be added later
            if (pin_chain_idx[conn_pin_physical_num] != OPEN) {
                continue;
            }
...
static int add_edges_for_collapsed_nodes(RRGraphBuilder& rr_graph_builder,
    ...
    std::vector<std::vector<int>> chain_pins(num_chain);

    // Cache the sink for each chain and store the pin number of the pins in a chain
    for (int chain_idx = 0; chain_idx < num_chain; chain_idx++) {
        ...
        for (int node_idx = 0; node_idx < num_nodes; node_idx++) {
            ...
            chain_pins[chain_idx].push_back(chain[node_idx].pin_physical_num);
        }
    ...
    for (int chain_idx = 0; chain_idx < num_chain; chain_idx++) {
        ...
        for (int node_idx = 0; node_idx < num_nodes; node_idx++) {
            add_chain_node_fan_in_edges(rr_graph_builder,
                            ...
                            nodes_to_collapse,
                            cluster_pins_set,
                            chain_pins[chain_idx],
                            R_minW_nmos,
                            R_minW_pmos,
                            chain_idx,
                            node_idx,
                            chain_sink_pin[chain_idx],
                            i,
                            j);
        }
    }
...
static void add_chain_node_fan_in_edges(RRGraphBuilder& rr_graph_builder,
    ...
    if (pin_physical_num == sink_pin_num) {
        ...
        for (auto pin_num : sink_pins) {
            ...
            rr_edges_to_create.emplace_back(sink_rr_node_id, rr_node_id, arch_sw);
        }
    } else {
