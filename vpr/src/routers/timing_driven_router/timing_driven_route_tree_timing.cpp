...
bool alloc_route_tree_timing_structs(bool exists_ok) {
  ...
  bool route_tree_structs_are_allocated =
    (rr_node_to_rt_node.size() == size_t(device_ctx.rr_graph.nodes().size())
...
t_rt_node* init_route_tree_to_source(ClusterNetId inet) {
  ...
  rt_root->inode = inode;
  /* FIXME convert to NodeID, to be removed when update all relevant data structures to use NodeId*/
  RRNodeId inode_id = RRNodeId(inode);
  rt_root->C_downstream = device_ctx.rr_graph.node_C(inode_id);
  rt_root->R_upstream = device_ctx.rr_graph.node_R(inode_id);
  rt_root->Tdel = 0.5 * device_ctx.rr_graph.node_R(inode_id) * device_ctx.rr_graph.node_C(inode_id);
...
t_rt_node* update_route_tree(t_heap * hptr, SpatialRouteTreeLookup* spatial_rt_lookup) {
  ...
  if (subtree_parent_rt_node != nullptr) { /* Parent exists. */
     ...
     */
    RRSwitchId iswitch_id = RRSwitchId(iswitch);
    
    Tdel_start += device_ctx.rr_graph.get_switch(iswitch_id).R * unbuffered_subtree_rt_root->C_downstream;
    Tdel_start += device_ctx.rr_graph.get_switch(iswitch_id).Tdel;
...
add_subtree_to_route_tree(t_heap *hptr, t_rt_node ** sink_rt_node_ptr) {
  ...
  for (t_heap_prev prev : hptr->nodes) {
    ...
    while (rr_node_to_rt_node[inode] == nullptr) { //Not connected to existing routing
       ...
       */
      RRNodeId inode_id = RRNodeId(inode);
      if (IPIN == device_ctx.rr_graph.node_type(inode_id)) {
...
static t_rt_node* add_non_configurable_to_route_tree(const int rr_node, const bool reached_by_non_configurable_edge, std::unordered_set<int>& visited) {
   ...
   * to be removed when update all relevant data structures to use NodeId
   */
  RRNodeId node_id = RRNodeId(rr_node);
  ...
  if (!visited.count(rr_node) || !reached_by_non_configurable_edge) {
    ...
    if (!reached_by_non_configurable_edge) { //An existing main branch node
      ...
      if (!rt_node) {
        ...
        if (IPIN == device_ctx.rr_graph.node_type(node_id)) {
          rt_node->re_expand = false;
        } else {
          rt_node->re_expand = true;
        }
    ...
    for (auto iedge : device_ctx.rr_graph.node_non_configurable_out_edges(node_id)) {
      //Recursive case: expand children
      VTR_ASSERT (!device_ctx.rr_graph.edge_is_configurable(iedge));

      int to_rr_node = size_t(device_ctx.rr_graph.edge_sink_node(iedge));
...
void load_new_subtree_R_upstream(t_rt_node* rt_node) {
  ...
  if (parent_rt_node) {
    ...
    R_upstream += device_ctx.rr_graph.get_switch(iswitch_id).R; //Parent switch R
  }
  R_upstream += device_ctx.rr_graph.node_R(inode_id); //Current node R
...
float load_new_subtree_C_downstream(t_rt_node* rt_node) {
  ...
  if (rt_node) {
    ...
    for (t_linked_rt_edge* edge = rt_node->u.child_list; edge != nullptr; edge = edge->next) {
       ...
       * to be removed when update all relevant data structures to use NodeId
       */
      RRSwitchId iswitch_id = RRSwitchId(edge->iswitch);

      if (!device_ctx.rr_graph.get_switch(iswitch_id).buffered()) {
...
void load_route_tree_Tdel(t_rt_node * subtree_rt_root, float Tarrival) {
  ...
  Tdel = Tarrival + 0.5 * subtree_rt_root->C_downstream * device_ctx.rr_graph.node_R(inode_id);
  ...
  while (linked_rt_edge != nullptr) {
    ...
    Tchild = Tdel + device_ctx.rr_graph.get_switch(iswitch_id).R * child_node->C_downstream;
    Tchild += device_ctx.rr_graph.get_switch(iswitch_id).Tdel; /* Intrinsic switch delay. */
...
void print_route_tree(const t_rt_node* rt_node, int depth) {
  ...
  VTR_LOG("%srt_node: %d (%s)", 
          indent.c_str(), 
          rt_node->inode, 
          device_ctx.rr_graph.node_type_string(inode_id));

  if (rt_node->parent_switch != OPEN) {
    /* FIXME convert to NodeID, 
     * to be removed when update all relevant data structures to use NodeId
     */
    RRSwitchId parent_switch_id = RRSwitchId(rt_node->parent_switch);

    bool parent_edge_configurable = device_ctx.rr_graph.get_switch(parent_switch_id).configurable();
  ...
  if (route_ctx.rr_node_route_inf[rt_node->inode].occ() > device_ctx.rr_graph.node_capacity(inode_id)) {
    VTR_LOG(" x");
  }
...
static t_trace* traceback_to_route_tree_branch(t_trace* trace, std::map<int,t_rt_node*>& rr_node_to_rt) {
  ...
  if (trace) {
    ...
    if (itr == rr_node_to_rt.end()) {
      ...
      auto node_type = device_ctx.rr_graph.node_type(inode_id);
...
t_trace* traceback_from_route_tree(ClusterNetId inet, const t_rt_node* root, int num_routed_sinks) {
  ...
  for (t_trace* trace = head; trace != nullptr; trace = trace->next) {
     ...
     * to be removed when update all relevant data structures to use NodeId
     */
    RRNodeId inode_id = RRNodeId(trace->index);
...
t_rt_node* prune_route_tree_recurr(t_rt_node* node, CBRR& connections_inf, bool force_prune) {
   ...
   * to be removed when update all relevant data structures to use NodeId
   */
  RRNodeId inode_id = RRNodeId(node->inode);

  bool congested = (route_ctx.rr_node_route_inf[node->inode].occ() > device_ctx.rr_graph.node_capacity(inode_id));
  ...
  if (SINK == device_ctx.rr_graph.node_type(inode_id)) {
    ...
    if (node->parent_node) {
       ...
       * to be removed when update all relevant data structures to use NodeId
       */
      RRSwitchId parent_switch_id = RRSwitchId(node->parent_switch);

      reached_non_configurably = !device_ctx.rr_graph.get_switch(parent_switch_id).configurable();
    }
...
t_rt_node* prune_route_tree(t_rt_node* rt_root, CBRR& connections_inf) {
   ...
   * to be removed when update all relevant data structures to use NodeId
   */
  RRNodeId inode_id = RRNodeId(rt_root->inode);

  VTR_ASSERT_MSG(device_ctx.rr_graph.node_type(inode_id) == SOURCE, "Root of route tree must be SOURCE");

  VTR_ASSERT_MSG(route_ctx.rr_node_route_inf[rt_root->inode].occ() <= device_ctx.rr_graph.node_capacity(inode_id),
...
static void print_node(const t_rt_node* rt_node) {
  ...
  VTR_LOG("%5.1e %5.1e %2d%6s|%-6d-> ", 
          rt_node->C_downstream, rt_node->R_upstream,
          rt_node->re_expand, 
          device_ctx.rr_graph.node_type_string(inode_id), inode);
}
...
static void print_node_congestion(const t_rt_node* rt_node) {
  ...
  VTR_LOG("%2d %2d|%-6d-> ", 
          node_inf.pres_cost, rt_node->Tdel,
          node_state.occ(), 
          device_ctx.rr_graph.node_capacity(inode_id), inode);
}
...
bool is_valid_route_tree(const t_rt_node* root) {
  ...
  if (root->parent_node) {
    if (device_ctx.rr_graph.get_switch(iswitch_id).buffered()) {
        float R_upstream_check = device_ctx.rr_graph.node_R(inode_id) + device_ctx.rr_graph.get_switch(iswitch_id).R;
     ...
     float R_upstream_check = device_ctx.rr_graph.node_R(inode_id) + root->parent_node->R_upstream + device_ctx.rr_graph.get_switch(iswitch_id).R;
      if (!vtr::isclose(root->R_upstream, R_upstream_check, RES_REL_TOL, RES_ABS_TOL)) {
        VTR_LOG("%d mismatch R upstream %e supposed %e\n", 
                inode, root->R_upstream, R_upstream_check);
        return false;
  ...
  } else if (root->R_upstream != device_ctx.rr_graph.node_R(inode_id)) {
    VTR_LOG("%d mismatch R upstream %e supposed %e\n", 
            inode, root->R_upstream, 
            device_ctx.rr_graph.node_R(inode_id));
    return false;
  }
  ...
  while (edge) {
    ...
    if (!device_ctx.rr_graph.get_switch(RRSwitchId(edge->iswitch)).buffered()) {
  ...
  float C_downstream_check = C_downstream_children + device_ctx.rr_graph.node_C(inode_id);
...
bool is_uncongested_route_tree(const t_rt_node* root) {
   ...
   * to be removed when update all relevant data structures to use NodeId
   */
  RRNodeId inode_id = RRNodeId(root->inode);
...
void collect_route_tree_connections(const t_rt_node* node, 
                                    std::set<std::tuple<int,int,int>>& connections) {
  if (node) {
