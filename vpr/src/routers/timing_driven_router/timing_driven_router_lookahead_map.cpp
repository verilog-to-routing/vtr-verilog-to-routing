...
public:
  ...
  PQ_Entry(RRNodeId set_rr_node_ind, RRSwitchId /*switch_ind*/, 
           float parent_delay, float parent_R_upstream, 
           float parent_congestion_upstream, bool starting_node){
    ...
    if (!starting_node){
      short cost_index = device_ctx.rr_graph.node_cost_index(set_rr_node_ind);
...
void compute_router_lookahead(size_t num_segments){
  ...
  for (size_t iseg = 0; iseg < num_segments; iseg++){
    for (t_rr_type chan_type : {CHANX, CHANY}){
...
static void run_dijkstra(RRNodeId start_node_ind, int start_x, int start_y, 
  ...
  while( !pq.empty() ){
    ...
    if (node_expanded[size_t(node_ind)]){
      continue;
    }

    /* if this node is an ipin record its congestion/delay in the routing_cost_map */
    if (IPIN == device_ctx.rr_graph.node_type(node_ind)){
      short ipin_x = device_ctx.rr_graph.node_xlow(node_ind);
      short ipin_y = device_ctx.rr_graph.node_ylow(node_ind);
    ...
    node_expanded[size_t(node_ind)] = true;
  }
}
...
static void expand_dijkstra_neighbours(PQ_Entry parent_entry, 
  ...
  for (auto edge : device_ctx.rr_graph.node_out_edges(parent_ind)) {
    RRNodeId child_node_ind = device_ctx.rr_graph.edge_sink_node(edge);
    RRSwitchId switch_ind = device_ctx.rr_graph.edge_switch(edge);

    /* skip this child if it has already been expanded from */
    if (node_expanded[size_t(child_node_ind)]){
      continue;
    }
    ...
    if (node_visited_costs[size_t(child_node_ind)] >= 0 && node_visited_costs[size_t(child_node_ind)] < child_entry.cost){
      continue;
    }

    /* finally, record the cost with which the child was visited and put the child entry on the queue */
    node_visited_costs[size_t(child_node_ind)] = child_entry.cost;
    pq.push(child_entry);
  }
}
