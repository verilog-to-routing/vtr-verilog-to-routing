	...
	for (auto net_id : cluster_ctx.clb_nlist.nets()) {
        for (auto pin_id : cluster_ctx.clb_nlist.net_sinks(net_id)) {
            ...
            if ((slack_type == NEGATIVE && path_slack < 0) ||
                ...
                max_budget_change = std::max(max_budget_change, (float)abs(net_delay[net_id][ipin] * path_slack / total_path_delay));
            }
...
void route_budgets::print_route_budget() {
    ...
    fp << std::endl << std::endl << "Delay lower_bound:" << std::endl;
    for(auto net_id : cluster_ctx.clb_nlist.nets()) {
        fp << std::endl << "Net: " << size_t(net_id) << "            ";
        for (auto pin_id : cluster_ctx.clb_nlist.net_sinks(net_id)) {
            int ipin = cluster_ctx.clb_nlist.pin_net_index(pin_id);
    ...
    fp << std::endl << std::endl << "Delay upper_bound:" << std::endl;
	for (auto net_id : cluster_ctx.clb_nlist.nets()) {
        fp << std::endl << "Net: " << size_t(net_id) << "            ";
        for (auto pin_id : cluster_ctx.clb_nlist.net_sinks(net_id)) {
            int ipin = cluster_ctx.clb_nlist.pin_net_index(pin_id);
