...
#include "draw.h"
#include "rr_graph.h"
#include "timing_driven_routing_predictor.h"
#include "VprTimingGraphResolver.h"

// all functions in router::profiling:: namespace, which are only activated if PROFILE is defined
 ...
 * To call a function in a router, function need a prefix of <your_router_namespace>::
...
bool try_timing_driven_route(
  ...
  router::profiling::profiling_initialization(get_max_pins_per_net());
  ...
  for (auto net_id : cluster_ctx.clb_nlist.nets()) {
    ...
    if (router_opts.congestion_analysis) router::profiling::congestion_analysis();
    if (router_opts.fanout_analysis) router::profiling::time_on_fanout_analysis();
    // router::profiling::time_on_criticality_analysis();
  }
...
bool try_timing_driven_route_net(ClusterNetId net_id, int itry, float pres_fac,
  ...
  if (route_ctx.net_status[net_id].is_fixed) { /* Skip pre-routed nets. */
    ...
    router::profiling::net_fanout_start();
    ...
    router::profiling::net_fanout_end(cluster_ctx.clb_nlist.net_sinks(net_id).size());
...
static bool timing_driven_route_sink(ClusterNetId net_id, unsigned itarget, int target_pin,
  ...
  router::profiling::sink_criticality_start();
  ...
  router::profiling::sink_criticality_end(cost_params.criticality);
  ...
  VTR_ASSERT_DEBUG(router::validate_traceback(route_ctx.trace[net_id].head));
  ...
  router::pathfinder_update_path_cost(new_route_start_tptr, 1, pres_fac);
  empty_heap();
...
static std::vector<t_heap> timing_driven_find_all_shortest_paths_from_heap(
  ...
  auto& device_ctx = g_vpr_ctx.device();
  std::vector<t_heap> cheapest_paths(device_ctx.rr_graph.nodes().size());
...
static t_rt_node* setup_routing_resources(int itry, ClusterNetId net_id, unsigned num_sinks, float pres_fac,
  ...
  if ((int) num_sinks < min_incremental_reroute_fanout || itry == 1) {
    ...
    router::profiling::net_rerouted();

    // rip up the whole net
    router::pathfinder_update_path_cost(route_ctx.trace[net_id].head, -1, pres_fac);
    router::free_traceback(net_id);
    ...
    router::profiling::net_rebuild_start();
    ...
    if (rt_root) { //Partially pruned
      router::profiling::route_tree_preserved();
      ...
      router::profiling::route_tree_pruned();
    ...
    router::profiling::net_rebuild_end(num_sinks, remaining_targets.size());
...
static t_bb add_high_fanout_route_tree_to_heap(t_rt_node* rt_root, int target_node,
  ...
  /* FIXME: remove NodeId when upstream functions have been adapted to NodeId */
  RRNodeId target_node_id = RRNodeId(target_node);

  int target_bin_x = grid_to_bin_x(device_ctx.rr_graph.node_xlow(target_node_id), spatial_rt_lookup);
  int target_bin_y = grid_to_bin_y(device_ctx.rr_graph.node_ylow(target_node_id), spatial_rt_lookup);

  int nodes_added = 0;

  t_bb highfanout_bb;
  highfanout_bb.xmin = device_ctx.rr_graph.node_xlow(target_node_id);
  highfanout_bb.xmax = device_ctx.rr_graph.node_xhigh(target_node_id);
  highfanout_bb.ymin = device_ctx.rr_graph.node_ylow(target_node_id);
  highfanout_bb.ymax = device_ctx.rr_graph.node_yhigh(target_node_id);
  ...
  for (int dx : {0, -1, +1}) {
    ...
    for (int dy : {0, -1, +1}) {
      ...
      for (t_rt_node* rt_node : spatial_rt_lookup[bin_x][bin_y]) {
        ...
        //Update Bounding Box
        /* FIXME: remove NodeId when upstream functions have been adapted to NodeId */
        RRNodeId inode_id = RRNodeId(rt_node->inode);
        highfanout_bb.xmin = std::min<int>(highfanout_bb.xmin, device_ctx.rr_graph.node_xlow(inode_id));
        highfanout_bb.ymin = std::min<int>(highfanout_bb.ymin, device_ctx.rr_graph.node_ylow(inode_id));
        highfanout_bb.xmax = std::max<int>(highfanout_bb.xmax, device_ctx.rr_graph.node_xhigh(inode_id));
        highfanout_bb.ymax = std::max<int>(highfanout_bb.ymax, device_ctx.rr_graph.node_yhigh(inode_id));
...
static void add_route_tree_node_to_heap(t_rt_node* rt_node,
    ...
    if (delay_budget) {
      ...
      tot_cost += pow(std::max(zero, tot_cost - delay_budget->max_delay), 2) / 100e-12;
      tot_cost += pow(std::max(zero, delay_budget->min_delay - tot_cost), 2) / 100e-12;
    }
...
static void timing_driven_expand_neighbours(t_heap *current,
  ...
  t_bb target_bb;
  if (target_node != OPEN) {
    target_bb.xmin = device_ctx.rr_graph.node_xlow(target_node_id);
    target_bb.ymin = device_ctx.rr_graph.node_ylow(target_node_id);
    target_bb.xmax = device_ctx.rr_graph.node_xhigh(target_node_id);
    target_bb.ymax = device_ctx.rr_graph.node_yhigh(target_node_id);
  }
...
static void timing_driven_expand_neighbour(t_heap* current,
  ...
  int to_xlow =  device_ctx.rr_graph.node_xlow(to_node_id);
  int to_ylow =  device_ctx.rr_graph.node_ylow(to_node_id);
  int to_xhigh = device_ctx.rr_graph.node_xhigh(to_node_id);
  int to_yhigh = device_ctx.rr_graph.node_yhigh(to_node_id);
  ...
  if (target_node != OPEN) {
    t_rr_type to_type = device_ctx.rr_graph.node_type(to_node_id);
    if (to_type == IPIN) {
...
static void timing_driven_add_to_heap(
  ...
  if (device_ctx.rr_graph.node_num_non_configurable_out_edges(to_node_id) == 0) {
...
static void timing_driven_expand_node_non_configurable_recurr(
  ...
  for (auto edge : device_ctx.rr_graph.node_non_configurable_out_edges(RRNodeId(to_node))) {
    VTR_ASSERT_SAFE(!device_ctx.rr_graph.edge_is_configurable(edge)); //Forced expansion

    int to_to_node = size_t(device_ctx.rr_graph.edge_sink_node(edge));
...
static t_timing_driven_node_costs evaluate_timing_driven_node_costs(const t_timing_driven_node_costs old_costs,
  ...
  //Switch info
  RRSwitchId iswitch_id = device_ctx.rr_graph.edge_switch(RREdgeId(iconn));
  bool switch_buffered = device_ctx.rr_graph.get_switch(iswitch_id).buffered();
  float switch_R = device_ctx.rr_graph.get_switch(iswitch_id).R;
  float switch_Tdel = device_ctx.rr_graph.get_switch(iswitch_id).Tdel;

  //Node info
  // FIXME 
  RRNodeId to_node_id  = RRNodeId(to_node);
  float node_C = device_ctx.rr_graph.node_C(to_node_id);
  float node_R = device_ctx.rr_graph.node_R(to_node_id);
  ...
  if (delay_budget) {
    ...
    new_costs.total_cost += (delay_budget->short_path_criticality + cost_params.criticality) * std::max(0.f, delay_budget->target_delay - new_costs.total_cost);
    new_costs.total_cost += pow(std::max(0.f, new_costs.total_cost - delay_budget->max_delay), 2) / 100e-12;
    new_costs.total_cost += pow(std::max(0.f, delay_budget->min_delay - new_costs.total_cost), 2) / 100e-12;
  }
...
bool Connection_based_routing_resources::forcibly_reroute_connections(float max_criticality,
  ...
  for (auto net_id : cluster_ctx.clb_nlist.nets()) {
    for (auto pin_id : cluster_ctx.clb_nlist.net_sinks(net_id)) {
      ...
      router::profiling::mark_for_forced_reroute();
    }
  }
...
void Connection_based_routing_resources::clear_force_reroute_for_connection(int rr_sink_node) {
  ...
  router::profiling::perform_forced_reroute();
}
...
static WirelengthInfo calculate_wirelength_info() {
  ...
  for (auto node : device_ctx.rr_graph.nodes()) {
    if (device_ctx.rr_graph.node_type(node) == CHANX || device_ctx.rr_graph.node_type(node) == CHANY) {
      available_wirelength += device_ctx.rr_graph.node_capacity(node) +
          device_ctx.rr_graph.node_xhigh(node) - device_ctx.rr_graph.node_xlow(node) +
          device_ctx.rr_graph.node_yhigh(node) - device_ctx.rr_graph.node_ylow(node);
    }
  }
...
static t_bb calc_current_bb(const t_trace* head) {
  ...
  for (const t_trace* elem = head; elem != nullptr; elem = elem->next) {
    RRNodeId node_id = RRNodeId(elem->index);
    ...
    bb.xmin = std::min<int>(bb.xmin, device_ctx.rr_graph.node_xhigh(node_id));
    bb.ymin = std::min<int>(bb.ymin, device_ctx.rr_graph.node_yhigh(node_id));
    bb.xmax = std::max<int>(bb.xmax, device_ctx.rr_graph.node_xlow(node_id));
    bb.ymax = std::max<int>(bb.ymax, device_ctx.rr_graph.node_ylow(node_id));
  }
