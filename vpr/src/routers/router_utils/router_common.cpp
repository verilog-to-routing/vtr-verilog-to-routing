...
static vtr::vector<ClusterBlockId, std::vector<int>> load_rr_clb_sources();
...
void get_serial_num() {
  ...
  for (auto net_id : cluster_ctx.clb_nlist.nets()) {
    ...
    while (tptr != nullptr) {
      inode = tptr->index;
      /* FIXME: remove the id conversion 
       * when the upstream data strucutre is converted to RRNodeId
       */
      RRNodeId inode_id = RRNodeId(inode);
      serial_num += (size_t(net_id) + 1)
                  * (device_ctx.rr_graph.node_xlow(inode_id) 
                  * (device_ctx.grid.width()) - device_ctx.rr_graph.node_yhigh(inode_id));

      serial_num -= device_ctx.rr_graph.node_ptc_num(inode_id) * (size_t(net_id) + 1) * 10;

      serial_num -= device_ctx.rr_graph.node_type(inode_id) * (size_t(net_id) + 1) * 100;
...
bool feasible_routing() {
  ...
  for (auto inode_id : device_ctx.rr_graph.nodes()) {
    if (route_ctx.rr_node_route_inf[size_t(inode_id)].occ() 
     > device_ctx.rr_graph.node_capacity(inode_id)) {
      return (false);
    }
  }
...
std::vector<int> collect_congested_rr_nodes() {
  ...
  for (auto inode_id : device_ctx.rr_graph.nodes()) {
    short occ = route_ctx.rr_node_route_inf[size_t(inode_id)].occ();
    short capacity = device_ctx.rr_graph.node_capacity(inode_id);

    if (occ > capacity) {
      congested_rr_nodes.push_back(size_t(inode_id));
    }
  }
...
void pathfinder_update_cost(float pres_fac, float acc_fac) {
   ...
   */
  for (auto inode_id : device_ctx.rr_graph.nodes()) {
    size_t inode = size_t(inode_id); 
    occ = route_ctx.rr_node_route_inf[inode].occ();
    capacity = device_ctx.rr_graph.node_capacity(inode_id);
    if (occ > capacity) {
...
void init_route_structs(int bb_factor) {
  ...
  //Various look-ups
  route_ctx.net_rr_terminals = load_net_rr_terminals();
  route_ctx.route_bb = load_route_bb(bb_factor);
  route_ctx.rr_blk_source = load_rr_clb_sources();
...
static std::pair<t_trace*,t_trace*> add_trace_non_configurable_recurr(int node, 
   ...
   * when upstream functions are adapted to RRNodeId 
   */
  RRNodeId node_id = RRNodeId(node);
...
static vtr::vector<ClusterNetId, std::vector<int>> load_net_rr_terminals() {
  ...
  for (auto net_id : cluster_ctx.clb_nlist.nets()) {
    ...
    for (auto pin_id : cluster_ctx.clb_nlist.net_pins(net_id)) {
      ...
      RRNodeId inode_id = device_ctx.rr_graph.find_node(i, j, 
                                                        (pin_count == 0 ? SOURCE : SINK), /* First pin is driver */
                                                        iclass);
      net_rr_terminals[net_id][pin_count] = size_t(inode_id);
      pin_count++;
    }
  }
  ...
  for (auto blk_id : cluster_ctx.clb_nlist.blocks()) {
    ...
    for (iclass = 0; iclass < type->num_class; iclass++) {
      if (iclass >= class_low && iclass <= class_high) {
        ...
        inode = size_t(device_ctx.rr_graph.find_node(i, j, rr_type, iclass));
...
t_bb load_net_route_bb(ClusterNetId net_id, int bb_factor) {
  ...
  auto driver_rr = route_ctx.net_rr_terminals[net_id][0];

  /* FIXME: the node id conversion will be removed 
   * when upstream functions are adapted to RRNodeId 
   */
  RRNodeId driver_rr_id = RRNodeId(driver_rr);
  VTR_ASSERT(SOURCE == device_ctx.rr_graph.node_type(driver_rr_id));

  VTR_ASSERT(device_ctx.rr_graph.node_xlow(driver_rr_id) <= device_ctx.rr_graph.node_xhigh(driver_rr_id));
  VTR_ASSERT(device_ctx.rr_graph.node_ylow(driver_rr_id) <= device_ctx.rr_graph.node_yhigh(driver_rr_id));

  short xmin = device_ctx.rr_graph.node_xlow(driver_rr_id);
  short ymin = device_ctx.rr_graph.node_ylow(driver_rr_id);
  short xmax = device_ctx.rr_graph.node_xhigh(driver_rr_id);
  short ymax = device_ctx.rr_graph.node_yhigh(driver_rr_id);
  ...
  for (size_t ipin = 1; ipin < net_sinks.size() + 1; ++ipin) { //Start at 1 since looping through sinks
      auto sink_rr = route_ctx.net_rr_terminals[net_id][ipin];
      RRNodeId sink_rr_id = RRNodeId(sink_rr);
      VTR_ASSERT(SINK == device_ctx.rr_graph.node_type(sink_rr_id));

      VTR_ASSERT(device_ctx.rr_graph.node_xlow(sink_rr_id) <= device_ctx.rr_graph.node_xhigh(sink_rr_id));
      VTR_ASSERT(device_ctx.rr_graph.node_ylow(sink_rr_id) <= device_ctx.rr_graph.node_yhigh(sink_rr_id));

      xmin = std::min<short>(xmin, device_ctx.rr_graph.node_xlow(sink_rr_id));
      xmax = std::max<short>(xmax, device_ctx.rr_graph.node_xhigh(sink_rr_id));
      ymin = std::min<short>(ymin, device_ctx.rr_graph.node_ylow(sink_rr_id));
      ymax = std::max<short>(ymax, device_ctx.rr_graph.node_yhigh(sink_rr_id));
  }
...
void add_to_heap(t_heap *hptr) {
  router::heap_::expand_heap_if_full();
  // start with undefined hole
  ++heap_tail;
  router::heap_::sift_up(heap_tail - 1, hptr);
}
    ...
    if (!cluster_ctx.clb_nlist.net_is_ignored(net_id)) {
      ...
      if (cluster_ctx.clb_nlist.net_sinks(net_id).size() == false) {
        ...
        while (tptr != nullptr) {
           ...
           */
          RRNodeId inode_id = RRNodeId(inode);
          t_rr_type rr_type = device_ctx.rr_graph.node_type(inode_id);
          int ilow = device_ctx.rr_graph.node_xlow(inode_id);
          int jlow = device_ctx.rr_graph.node_ylow(inode_id);

          fprintf(fp, "Node:\t%d\t%6s (%d,%d) ", inode,
                  device_ctx.rr_graph.node_type_string(inode_id), ilow, jlow);

          if (   (ilow != device_ctx.rr_graph.node_xhigh(inode_id))
            || (jlow != device_ctx.rr_graph.node_yhigh(inode_id)))
            fprintf(fp, "to (%d,%d) ", device_ctx.rr_graph.node_xhigh(inode_id),
                    device_ctx.rr_graph.node_yhigh(inode_id));
          ...
          default:
              vpr_throw(VPR_ERROR_ROUTE, __FILE__, __LINE__,
                        ...
                        rr_type, device_ctx.rr_graph.node_type_string(inode_id));
              break;
          }

          fprintf(fp, "%d  ", device_ctx.rr_graph.node_ptc_num(inode_id));

          if (!is_io_type(device_ctx.grid[ilow][jlow].type) && (rr_type == IPIN || rr_type == OPIN)) {
            int pin_num = device_ctx.rr_graph.node_ptc_num(inode_id);
...
void reserve_locally_used_opins(float pres_fac, float acc_fac, bool rip_up_local_opins) {
  ...
  int num_local_opin, inode, from_node;
  int iclass, ipin;
  float cost;
  ...
  for (auto blk_id : cluster_ctx.clb_nlist.blocks()) {
    ...
    for (iclass = 0; iclass < type->num_class; iclass++) {
      ...
      for (auto edge : device_ctx.rr_graph.node_out_edges(from_node_id)) {
        ...
        VTR_ASSERT(OPIN == device_ctx.rr_graph.node_type(to_node_id));
...
void print_traceback(ClusterNetId net_id) {
  ...
  while (head) {
    ...
    if (SINK == device_ctx.rr_graph.node_type(inode_id)) {
...
void print_traceback(const t_trace* trace) {
  ...
  while (trace) {
     ...
     */
    RRNodeId inode_id = RRNodeId(inode_id);
    VTR_LOG("%d (%s)", inode, device_ctx.rr_graph.node_type_string(inode_id));

    if (trace->iswitch == OPEN) {
      VTR_LOG(" !"); //End of branch
    }

    if (prev && prev->iswitch != OPEN && !device_ctx.rr_graph.get_switch(RRSwitchId(prev->iswitch)).configurable()) {
      VTR_LOG("*"); //Reached non-configurably
    }

    if (route_ctx.rr_node_route_inf[inode].occ() > device_ctx.rr_graph.node_capacity(inode_id)) {
      VTR_LOG(" x"); //Overused
    }
    VTR_LOG("\n");
    prev = trace;
...
bool validate_traceback_recurr(t_trace* trace, std::set<int>& seen_rr_nodes) {
  ...
  if (next) {
    if (trace->iswitch == OPEN) { //End of a branch
      ...
      bool found = false;
      for (auto iedge : device_ctx.rr_graph.node_out_edges(RRNodeId(trace->index))) {
        RRNodeId to_node = device_ctx.rr_graph.edge_sink_node(iedge);

        if (to_node == RRNodeId(next->index)) {
          found = true;

          //Verify that the switch matches
          short rr_iswitch = size_t(device_ctx.rr_graph.edge_switch(iedge));
...
void print_invalid_routing_info() {
  ...
  for (auto inode_id : device_ctx.rr_graph.nodes()) {
    int inode = size_t(inode_id);
    int occ = route_ctx.rr_node_route_inf[inode].occ();
    int cap = device_ctx.rr_graph.node_capacity(inode_id);
    if (occ > cap) {
...
void print_rr_node_route_inf_dot() {
  ...
  for (size_t inode = 0; inode < route_ctx.rr_node_route_inf.size(); ++inode) {
    if (!std::isinf(route_ctx.rr_node_route_inf[inode].path_cost)) {
      ...
      VTR_LOG("\tnode%zu[label=\"{%zu (%s)", inode, inode, device_ctx.rr_graph.node_type_string(inode_id));
      if (route_ctx.rr_node_route_inf[inode].occ() > device_ctx.rr_graph.node_capacity(inode_id)) {
        VTR_LOG(" x");
      }
      VTR_LOG("}\"]\n");
...
namespace heap_ {
  ...
  void expand_heap_if_full() {
    if (heap_tail > heap_size) { /* Heap is full */
      heap_size *= 2;
      heap = (router::t_heap **) vtr::realloc((void *) (heap + 1),
  ...
  void push_back(router::t_heap* const hptr) {
    expand_heap_if_full();
    heap[heap_tail] = hptr;
    ++heap_tail;
  }
  ...
  void push_back_node(int inode, float total_cost, int prev_node, int prev_edge,
    ...
    router::t_heap* hptr = alloc_heap_data();
    hptr->index = inode;
    hptr->cost = total_cost;
        hptr->nodes.emplace_back(inode, prev_node, prev_edge);
    hptr->backward_path_cost = backward_path_cost;
