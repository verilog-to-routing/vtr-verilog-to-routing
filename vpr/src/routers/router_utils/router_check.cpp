...
void check_route(enum e_route_type route_type) {
  ...
  const int num_switches = device_ctx.rr_graph.switches().size();
  ...
  connected_to_route = (bool *) vtr::calloc(device_ctx.rr_graph.nodes().size(), sizeof(bool));
  ...
  for (auto net_id : cluster_ctx.clb_nlist.nets()) {
    if ( cluster_ctx.clb_nlist.net_is_ignored(net_id) 
      || cluster_ctx.clb_nlist.net_sinks(net_id).size() == 0) { /* Skip ignored nets. */
      continue;
    }
    ...
    while (tptr != nullptr) {
      ...
      if (prev_switch == OPEN) { //Start of a new branch
        ...
        if (SINK == device_ctx.rr_graph.node_type(RRNodeId(inode))) {
...
static void check_sink(int inode, ClusterNetId net_id, bool * pin_done) {
  ...
  i = device_ctx.rr_graph.node_xlow(RRNodeId(inode));
  j = device_ctx.rr_graph.node_ylow(RRNodeId(inode));
...
static void check_switch(t_trace *tptr, int num_switch) {
  ...
  if (device_ctx.rr_graph.node_type(RRNodeId(inode)) != SINK) {
    if (switch_type >= num_switch) {
      /* find the next node to provide more debugging information */
      int to_node = tptr->next->index;
      vpr_throw(VPR_ERROR_ROUTE, __FILE__, __LINE__,
                "in check_switch: rr_node %d (type:%s) left to rr_node %d (type:%s) via switch type %d.\n"
                "\tSwitch type in rr_graph is %d\n"
                "\tSwitch type is out of range (%d).\n", 
                inode, device_ctx.rr_graph.node_type_string(RRNodeId(inode)),
                to_node, device_ctx.rr_graph.node_type_string(RRNodeId(to_node)),
                switch_type, size_t(device_ctx.rr_graph.get_node_to_node_switch(RRNodeId(inode), RRNodeId(to_node))),
                num_switch);
    }
  } else { /* Is a SINK */
...
static bool check_adjacent(int from_node, int to_node) {
  ...
  for (auto edge : device_ctx.rr_graph.node_out_edges(RRNodeId(from_node))) {
    if (size_t(device_ctx.rr_graph.edge_sink_node(edge)) == size_t(to_node)) {
      reached = true;
      break;
    }
  }
  ...
  from_xlow =  device_ctx.rr_graph.node_xlow(RRNodeId(from_node));
  from_ylow =  device_ctx.rr_graph.node_ylow(RRNodeId(from_node));
  from_xhigh = device_ctx.rr_graph.node_xhigh(RRNodeId(from_node));
  from_yhigh = device_ctx.rr_graph.node_yhigh(RRNodeId(from_node));
  from_ptc =   device_ctx.rr_graph.node_ptc_num(RRNodeId(from_node));
  to_type =  device_ctx.rr_graph.node_type(RRNodeId(to_node));
  to_xlow =  device_ctx.rr_graph.node_xlow(RRNodeId(to_node));
  to_ylow =  device_ctx.rr_graph.node_ylow(RRNodeId(to_node));
  to_xhigh = device_ctx.rr_graph.node_xhigh(RRNodeId(to_node));
  to_yhigh = device_ctx.rr_graph.node_yhigh(RRNodeId(to_node));
...
static int chanx_chany_adjacent(int chanx_node, int chany_node) {
  ...
  chanx_y =     device_ctx.rr_graph.node_ylow(RRNodeId(chanx_node));
  chanx_xlow =  device_ctx.rr_graph.node_xlow(RRNodeId(chanx_node));
  chanx_xhigh = device_ctx.rr_graph.node_xhigh(RRNodeId(chanx_node));

  chany_x =     device_ctx.rr_graph.node_xlow(RRNodeId(chany_node));
  chany_ylow =  device_ctx.rr_graph.node_ylow(RRNodeId(chany_node));
  chany_yhigh = device_ctx.rr_graph.node_yhigh(RRNodeId(chany_node));
...
void recompute_occupancy_from_scratch() {
  ...
  for (size_t inode_idx = 0; inode_idx < device_ctx.rr_graph.nodes().size(); ++inode_idx)
...
static void check_node_and_range(int inode, enum e_route_type route_type) {
  ...
  if (inode < 0 || inode >= (int) device_ctx.rr_graph.nodes().size()) {
      vpr_throw(VPR_ERROR_ROUTE, __FILE__, __LINE__,
        "in check_node_and_range: rr_node #%d is out of legal, range (0 to %d).\n", inode, device_ctx.rr_graph.nodes().size() - 1);
  }
...
static void expand_non_configurable(int inode, std::set<t_node_edge>& edge_set) {
    ...
    for (auto iedge : device_ctx.rr_graph.node_out_edges(RRNodeId(inode))) {
        bool edge_non_configurable = !device_ctx.rr_graph.edge_is_configurable(iedge);

        if (edge_non_configurable) {
            int to_node = size_t(device_ctx.rr_graph.edge_sink_node(iedge));
