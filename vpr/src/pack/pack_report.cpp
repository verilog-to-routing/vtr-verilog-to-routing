...
void report_packing_pin_usage(std::ostream& os, const VprContext& ctx) {
    ...
    std::map<t_type_ptr,std::vector<size_t>> inputs_used;
    std::map<t_type_ptr,std::vector<size_t>> outputs_used;
    ...
    for (int itype = 0; itype < device_ctx.num_block_types; ++itype) {
        ...
        size_t max_inputs = *std::max_element(inputs_used[type].begin(), inputs_used[type].end());
        size_t min_inputs = *std::min_element(inputs_used[type].begin(), inputs_used[type].end());
        float avg_inputs = std::accumulate(inputs_used[type].begin(), inputs_used[type].end(), 0) / float(inputs_used[type].size());

        size_t max_outputs = *std::max_element(outputs_used[type].begin(), outputs_used[type].end());
        size_t min_outputs = *std::min_element(outputs_used[type].begin(), outputs_used[type].end());
