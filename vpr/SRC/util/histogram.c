#include <algorithm>
#include <string>
#include <sstream>

#include "vtr_log.h"

#include "histogram.h"


void print_histogram(std::vector<HistogramBucket> histogram) {
    size_t char_width = 80;
    
    //Determine the maximum count 
    float max_count = 0.;
    for(HistogramBucket bucket : histogram) {
        max_count = std::max(max_count, bucket.count);
    }

    //Determine the maximum prefix length
    size_t bar_len = char_width
                     - (18 +3) //bucket prefix
                     - 7 //count
                     - 2; //-2 for " |" appended after count

    for(size_t ibucket = 0; ibucket < histogram.size(); ++ibucket) {
        vtr::printf("[% 9.2g:% 9.2g) %7.3g |", histogram[ibucket].min_value, histogram[ibucket].max_value, histogram[ibucket].count);

        size_t num_chars = (histogram[ibucket].count / max_count) * bar_len;
        for(size_t i = 0; i < num_chars; ++i) {
            vtr::printf("*");
        }
        vtr::printf("\n");
    }

}
