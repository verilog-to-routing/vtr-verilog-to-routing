...
The new switch block description format allow a much finer level of control over the switch blocks generated by VPR.
Whereas a user previously only had the choice of 'wilton', 'universal', 'subset', or 'full' switch blocks, the new specification
format allows the user to specify a small set of mathematical formulas that describes the switch block connections. This format
...
This functionality is split across different files which are described below:
  ...
  * rr_graph2.c:
    * get_track_to_chan_seg is called within the get_track_to_tracks which looks at t_sb_connection_map to determine the edges that
...
For a length-4 wire segment type:
...
An example from the XML VPR architecture file is given below. The 'wireconn' entries define ordered source/destination sets of wire segments that should be connected with the specified permutation functions. A permutation function of 't/2' specifies that the t'th wire segment in the source set should connect to the [(t/2)%W]'th wire segment in the destination set where W is the size, or effective channel width, of the destination set (note that permutation functions are implicitly modulo W so that all functions evaluate to a number that indexes into the destination set). 
  ...
  ...
  <switchblocklist>  
    <switchblock name="subset" type="unidir">		<-- switch block name/type
    <switch_type name="0"/>				<-- switch used to implement connections (will move into permutation funcs)
    <switchblock_location type="EVERYWHERE"/>		<-- location to implement switch block (EVERYWHERE/CORE/PERIMETER/CORNER/FRINGE)
      <switchfuncs>					<-- permutation functions
        <func type="lr" formula="t"/>			<-- left-to-right switch block connection (TODO: change to west-to-east style of notation?)
        <func type="lt" formula="t"/>                   <-- formula supports +/-/div/mult/brackets
...
/* TODO TODO TODO!! in t_seg_details VPR referes to 'group' as a collection of L tracks that are consecutive and all 
...
/**** Defines ****/
/* REF_X & REF_Y define an arbitrary reference point within the FPGA grid relative to which a line of switch blocks
   is pre-computed and then stamped-out throughout the FPGA (which speeds-up the generation of the switch block
   t_sb_connection_map sparse array) */
#define REF_X 1
#define REF_Y 1
	...
	for (int iconn = 0; iconn < (int)sb->wireconns.size(); iconn++){
		...
		for (int iperm = 0; iperm < (int)permutations_ref.size(); iperm++){
			...
			if(dest_track_in_sp < 0){
				vpr_throw(VPR_ERROR_ARCH, __FILE__, __LINE__, "compute_track_connections: got a negative track, %d  based on source track %d. From side %d to side %d. W=%d. Permutation function: %s\n", dest_track_in_sp, src_track_in_sp, from_side, to_side, nodes_per_chan, permutations_ref.at(iperm).c_str());
			}
...
static int get_track_group(INP int nx, INP int ny, INP e_rr_type chan_type,
	...
	*/
	...
	if (seg_start != min_seg){
		...
		/* track must span the entire FPGA */	//XXX meaning what? there is only one possible group?
		group = 0;
	}

	/* the start of this track cannot be *after* the current seg coordinate */
	if (group < 0){		//XXX remove check
		vpr_throw(VPR_ERROR_ARCH, __FILE__, __LINE__, "get_track_group: invalid seg_start, seg_coord combination\n");
	}
