...
/* Allocates the g_ctx.rr_indexed_data array and loads it with appropriate values. *
...
void alloc_and_load_rr_indexed_data(const t_segment_inf * segment_inf,
	...
	g_ctx.num_rr_indexed_data = CHANX_COST_INDEX_START + (2 * num_segment);
	g_ctx.rr_indexed_data = (t_rr_indexed_data *) vtr::malloc(
			g_ctx.num_rr_indexed_data * sizeof(t_rr_indexed_data));
	...
	for (i = SOURCE_COST_INDEX; i <= IPIN_COST_INDEX; i++) {
		g_ctx.rr_indexed_data[i].ortho_cost_index = OPEN;
		g_ctx.rr_indexed_data[i].seg_index = OPEN;
		g_ctx.rr_indexed_data[i].inv_length = OPEN;
		g_ctx.rr_indexed_data[i].T_linear = OPEN;
		g_ctx.rr_indexed_data[i].T_quadratic = OPEN;
		g_ctx.rr_indexed_data[i].C_load = OPEN;
	}

	g_ctx.rr_indexed_data[IPIN_COST_INDEX].T_linear =
			g_ctx.rr_switch_inf[wire_to_ipin_switch].Tdel;
	...
	for (iseg = 0; iseg < num_segment; iseg++) {
		...
		g_ctx.rr_indexed_data[index].ortho_cost_index = index + num_segment;

		if (segment_inf[iseg].longline)
			length = g_ctx.nx;
		else
			length = min(segment_inf[iseg].length, g_ctx.nx);

		g_ctx.rr_indexed_data[index].inv_length = 1. / length;
		g_ctx.rr_indexed_data[index].seg_index = iseg;
	}
	...
	for (iseg = 0; iseg < num_segment; iseg++) {
		...
		if (segment_inf[iseg].longline)
			...
			length = min(segment_inf[iseg].length, g_ctx.ny);
...
static void load_rr_indexed_data_base_costs(int nodes_per_chan,
	...
	/* Loads the base_cost member of g_ctx.rr_indexed_data according to the specified *
	 * base_cost_type.                                                          */
	...
	if (base_cost_type == DEMAND_ONLY || base_cost_type == DELAY_NORMALIZED) {
		g_ctx.rr_indexed_data[SOURCE_COST_INDEX].base_cost = delay_normalization_fac;
		g_ctx.rr_indexed_data[SINK_COST_INDEX].base_cost = 0.;
		g_ctx.rr_indexed_data[OPIN_COST_INDEX].base_cost = delay_normalization_fac;

		g_ctx.rr_indexed_data[IPIN_COST_INDEX].base_cost = 0.95
	...
	for (index = CHANX_COST_INDEX_START; index < g_ctx.num_rr_indexed_data; index++) {
			/*       g_ctx.rr_indexed_data[index].base_cost = delay_normalization_fac /
			 g_ctx.rr_indexed_data[index].inv_length;  */

		g_ctx.rr_indexed_data[index].base_cost = delay_normalization_fac;
		/*       g_ctx.rr_indexed_data[index].base_cost = delay_normalization_fac *
		 sqrt (1. / g_ctx.rr_indexed_data[index].inv_length);  */
		/*       g_ctx.rr_indexed_data[index].base_cost = delay_normalization_fac *
		 (1. + 1. / g_ctx.rr_indexed_data[index].inv_length);  */
	}
	...
	for (index = 0; index < g_ctx.num_rr_indexed_data; index++) {
...
static float get_delay_normalization_fac(int nodes_per_chan,
	...
	for (itrack = 0; itrack < nodes_per_chan; itrack++) {
		inode = find_average_rr_node_index(g_ctx.nx, g_ctx.ny, CHANX, itrack, 
				L_rr_node_indices);
		if (inode == -1)
			continue;
		cost_index = g_ctx.rr_nodes[inode].cost_index();
		frac_num_seg = clb_dist * g_ctx.rr_indexed_data[cost_index].inv_length;
		Tdel = frac_num_seg * g_ctx.rr_indexed_data[cost_index].T_linear
				+ frac_num_seg * frac_num_seg
						* g_ctx.rr_indexed_data[cost_index].T_quadratic;
		Tdel_sum += Tdel / (float) clb_dist;
	}

	for (itrack = 0; itrack < nodes_per_chan; itrack++) {
		inode = find_average_rr_node_index(g_ctx.nx, g_ctx.ny, CHANY, itrack, 
				L_rr_node_indices);
		if (inode == -1)
			continue;
		cost_index = g_ctx.rr_nodes[inode].cost_index();
...
static void load_rr_indexed_data_T_values(int index_start,
	...
	float *C_total, *R_total; /* [0..g_ctx.num_rr_indexed_data - 1] */
	double *switch_R_total, *switch_T_total; /* [0..g_ctx.num_rr_indexed_data - 1] */
	short *switches_buffered;
	int *num_nodes_of_index; /* [0..g_ctx.num_rr_indexed_data - 1] */
	float Rnode, Cnode, Rsw, Tsw;

	num_nodes_of_index = (int *) vtr::calloc(g_ctx.num_rr_indexed_data, sizeof(int));
	C_total = (float *) vtr::calloc(g_ctx.num_rr_indexed_data, sizeof(float));
	R_total = (float *) vtr::calloc(g_ctx.num_rr_indexed_data, sizeof(float));
	...
	switch_R_total = (double *) vtr::calloc(g_ctx.num_rr_indexed_data, sizeof(double));
	switch_T_total = (double *) vtr::calloc(g_ctx.num_rr_indexed_data, sizeof(double));
	switches_buffered = (short *) vtr::calloc(g_ctx.num_rr_indexed_data, sizeof(short));
	...
	for (itrack = 0; itrack < nodes_per_chan; itrack++) {
		inode = find_average_rr_node_index(g_ctx.nx, g_ctx.ny, rr_type, itrack,
				L_rr_node_indices);
		if (inode == -1)
			continue;
		cost_index = g_ctx.rr_nodes[inode].cost_index();
		num_nodes_of_index[cost_index]++;
		C_total[cost_index] += g_ctx.rr_nodes[inode].C();
		R_total[cost_index] += g_ctx.rr_nodes[inode].R();
		...
		for (int iedge = 0; iedge < num_edges; iedge++){
			int to_node_index = g_ctx.rr_nodes[inode].edge_sink_node(iedge);
			/* want to get C/R/Tdel of switches that connect this track segment to other track segments */
			if (g_ctx.rr_nodes[to_node_index].type() == CHANX || g_ctx.rr_nodes[to_node_index].type() == CHANY){
				int switch_index = g_ctx.rr_nodes[inode].edge_switch(iedge);
				avg_switch_R += g_ctx.rr_switch_inf[switch_index].R;
				avg_switch_T += g_ctx.rr_switch_inf[switch_index].Tdel;
				...
				if (buffered == UNDEFINED) {
					...
					if (buffered != (short)(g_ctx.rr_switch_inf[switch_index].buffered)) {
						vpr_throw(VPR_ERROR_ARCH, __FILE__, __LINE__,
	...
	for (cost_index = index_start;
		...
		if (num_nodes_of_index[cost_index] == 0) { /* Segments don't exist. */
			g_ctx.rr_indexed_data[cost_index].T_linear = OPEN;
			g_ctx.rr_indexed_data[cost_index].T_quadratic = OPEN;
			g_ctx.rr_indexed_data[cost_index].C_load = OPEN;
		} else {
			...
			if (switches_buffered[cost_index]){
				g_ctx.rr_indexed_data[cost_index].T_linear = Tsw + Rsw * Cnode
						+ 0.5 * Rnode * Cnode;
				g_ctx.rr_indexed_data[cost_index].T_quadratic = 0.;
				g_ctx.rr_indexed_data[cost_index].C_load = 0.;
			} else { /* Pass transistor */
				g_ctx.rr_indexed_data[cost_index].C_load = Cnode;

				/* See Dec. 23, 1997 notes for deriviation of formulae. */

				g_ctx.rr_indexed_data[cost_index].T_linear = Tsw + 0.5 * Rsw * Cnode;
				g_ctx.rr_indexed_data[cost_index].T_quadratic = (Rsw + Rnode) * 0.5
						* Cnode;
			}
		}
	}
