...
free_matrix3(track_to_pin_map[i], 0, nodes_per_chan - 1, 0, types[i].height - 1,
...
(clb_net[inet].num_sinks + 1) * sizeof(int), &rr_mem_chunk_list_head,
...
inode = get_rr_node_index(i, j, rr_type, iclass, L_rr_node_indices);
...
itrack = tracks_connected_to_pin[ipin][ioff][iside][iconn];
...
L_rr_node[inode].ylow, L_rr_node[inode].xhigh, L_rr_node[inode].yhigh);
}
