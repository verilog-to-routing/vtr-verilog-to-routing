...
void power_lowlevel_init() {
	...
	g_ctx.power_commonly_used->NMOS_1X_C_d = C_d;
	g_ctx.power_commonly_used->NMOS_1X_C_g = C_g;
	g_ctx.power_commonly_used->NMOS_1X_C_s = C_s;

	power_calc_transistor_capacitance(&C_d, &C_s, &C_g, PMOS,
			g_ctx.power_tech->PN_ratio);
	g_ctx.power_commonly_used->PMOS_1X_C_d = C_d;
	g_ctx.power_commonly_used->PMOS_1X_C_g = C_g;
	g_ctx.power_commonly_used->PMOS_1X_C_s = C_s;

	g_ctx.power_commonly_used->NMOS_1X_st_leakage = power_calc_leakage_st(NMOS,
			1.0);
	g_ctx.power_commonly_used->PMOS_1X_st_leakage = power_calc_leakage_st(PMOS,
			1.0 * g_ctx.power_tech->PN_ratio);

	g_ctx.power_commonly_used->INV_1X_C_in = g_ctx.power_commonly_used->NMOS_1X_C_g
			+ g_ctx.power_commonly_used->PMOS_1X_C_g;
	g_ctx.power_commonly_used->INV_1X_C = g_ctx.power_commonly_used->NMOS_1X_C_g
			+ g_ctx.power_commonly_used->PMOS_1X_C_g
			+ g_ctx.power_commonly_used->NMOS_1X_C_d
			+ g_ctx.power_commonly_used->PMOS_1X_C_d;

	power_calc_transistor_capacitance(&C_d, &C_s, &C_g, NMOS, 2.0);
	g_ctx.power_commonly_used->INV_2X_C = C_g + C_d;
	power_calc_transistor_capacitance(&C_d, &C_s, &C_g, PMOS,
			2.0 * g_ctx.power_tech->PN_ratio);
	g_ctx.power_commonly_used->INV_2X_C += C_g + C_d;
...
float power_calc_node_switching(float capacitance, float density,
		float period) {
	return 0.5 * g_ctx.power_tech->Vdd * g_ctx.power_tech->Vdd * capacitance * density
			/ period;
}
...
static float power_calc_node_switching_v(float capacitance, float density,
		float period, float voltage) {
	return 0.5 * voltage * g_ctx.power_tech->Vdd * capacitance * density / period;
}
...
void power_usage_inverter(t_power_usage * power_usage, float in_dens,
	...
	float PMOS_size = g_ctx.power_tech->PN_ratio * size;
...
static float power_calc_leakage_st(e_tx_type transistor_type, float size) {
	...
	return current * g_ctx.power_tech->Vdd;
}
...
static float power_calc_leakage_st_pass_transistor(float size, float v_ds) {
	...
	if (size
			>= g_ctx.power_tech->nmos_leakage_info[g_ctx.power_tech->num_nmos_leakage_info
					- 1].nmos_size) {
		nmos_low =
				&g_ctx.power_tech->nmos_leakage_info[g_ctx.power_tech->num_nmos_leakage_info
						- 1];
		...
		for (int i = 1; i < g_ctx.power_tech->num_nmos_leakage_info; i++) {
			if (size < g_ctx.power_tech->nmos_leakage_info[i].nmos_size) {
				nmos_low = &g_ctx.power_tech->nmos_leakage_info[i - 1];
				nmos_high = &g_ctx.power_tech->nmos_leakage_info[i];
				break;
			}
		}
	}
	...
	power_low = i_ds * g_ctx.power_tech->Vdd;
    ...
    if (over_range) {
		...
		power_high = i_ds * g_ctx.power_tech->Vdd;
...
void power_usage_MUX2_transmission(t_power_usage * power_usage, float size,
	...
	leakage_p = power_calc_leakage_st(PMOS, size * g_ctx.power_tech->PN_ratio);
	...
	power_calc_transistor_capacitance(&C_d_p, &C_s_p, &C_g_p, PMOS,
			size * g_ctx.power_tech->PN_ratio);
...
// Check if nmos size is beyond range
	if (transistor_size
			>= g_ctx.power_tech->nmos_mux_info[g_ctx.power_tech->num_nmos_mux_info - 1].nmos_size) {
		mux_nmos_inf_lower =
				&g_ctx.power_tech->nmos_mux_info[g_ctx.power_tech->num_nmos_mux_info - 1];
		over_range = true;
	} else {
		for (int i = 1; i < g_ctx.power_tech->num_nmos_mux_info; i++) {
			if (transistor_size < g_ctx.power_tech->nmos_mux_info[i].nmos_size) {
				mux_nmos_inf_lower = &g_ctx.power_tech->nmos_mux_info[i - 1];
				mux_nmos_inf_upper = &g_ctx.power_tech->nmos_mux_info[i];
				break;
			}
		}
	}
...
void power_usage_level_restorer(t_power_usage * power_usage,
	...
	/* Pull-up PMOS */
	if (g_ctx.power_tech->PMOS_inf.long_trans_inf == NULL) {
		power_log_msg(POWER_LOG_ERROR,
				"No long transistor information exists.  Cannot determine transistor properties.");
		return;
	}
	C = g_ctx.power_tech->PMOS_inf.long_trans_inf->C_d
			+ g_ctx.power_tech->PMOS_inf.long_trans_inf->C_g;
	C_in = g_ctx.power_tech->PMOS_inf.long_trans_inf->C_d;
	...
	power_usage->leakage += (1 - in_prob)
			* g_ctx.power_tech->PMOS_inf.long_trans_inf->leakage_subthreshold;
...
float power_calc_buffer_sc(int stages, float gain, bool level_restorer,
	...
	size_inf = &g_ctx.power_tech->buffer_size_inf[stages];
...
float power_calc_buffer_size_from_Cout(float C_out) {
	...
	t_transistor_inf * nmos_info = &g_ctx.power_tech->NMOS_inf;
	t_transistor_inf * pmos_info = &g_ctx.power_tech->PMOS_inf;
