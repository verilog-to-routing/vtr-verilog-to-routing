...
static void power_usage_primitive(t_power_usage * power_usage, t_pb * pb,
	...
	if (strcmp(pb_graph_node->pb_type->blif_model, ".names") == 0) {
		...
		if (pb) {
            AtomBlockId blk_id = g_ctx.atom_lookup.pb_atom(pb);
			SRAM_values = alloc_SRAM_values_from_truth_table(LUT_size,
					g_ctx.atom_nl.block_truth_table(blk_id));
		} else {
			SRAM_values = alloc_SRAM_values_from_truth_table(LUT_size, AtomNetlist::TruthTable());
		}
		power_usage_lut(&sub_power_usage, LUT_size,
				g_ctx.power_arch->LUT_transistor_size, SRAM_values,
				input_probabilities, input_densities, g_ctx.solution_inf.T_crit);
		...
		clk_prob = g_ctx.clock_arch->clock_inf[0].prob;
		clk_dens = g_ctx.clock_arch->clock_inf[0].dens;

		power_usage_ff(&sub_power_usage, g_ctx.power_arch->FF_size, D_prob, D_dens,
				Q_prob, Q_dens, clk_prob, clk_dens, g_ctx.solution_inf.T_crit);
		power_add_usage(power_usage, &sub_power_usage);
...
void power_usage_local_pin_buffer_and_wire(t_power_usage * power_usage,
	...
	if (buffer_size) {
		power_usage_buffer(&sub_power_usage, buffer_size, pin_prob(pb, pin, iblk),
			pin_dens(pb, pin, iblk), false, g_ctx.solution_inf.T_crit);
		power_add_usage(power_usage, &sub_power_usage);
	}
}
...
static void power_usage_pb(t_power_usage * power_usage, t_pb * pb,
	...
	case POWER_METHOD_C_INTERNAL:
		...
		power_usage_sub.dynamic += power_calc_node_switching(
				pb_power->C_internal, dens_avg, g_ctx.solution_inf.T_crit);
...
static void power_reset_tile_usage(void) {
	...
	for (type_idx = 0; type_idx < g_ctx.num_block_types; type_idx++) {
		if (g_ctx.block_types[type_idx].pb_type) {
			power_reset_pb_type(g_ctx.block_types[type_idx].pb_type);
		}
	}
}
...
static void power_usage_blocks(t_power_usage * power_usage) {
	...
	for (x = 0; x < g_ctx.nx + 2; x++) {
		for (y = 0; y < g_ctx.ny + 2; y++) {

			if ((g_ctx.grid[x][y].width_offset != 0)
					|| (g_ctx.grid[x][y].height_offset != 0)
					|| (g_ctx.grid[x][y].type == g_ctx.EMPTY_TYPE)) {
				continue;
			}

			for (z = 0; z < g_ctx.grid[x][y].type->capacity; z++) {
				t_pb * pb = NULL;
				t_power_usage pb_power;

				if (g_ctx.grid[x][y].blocks[z] != EMPTY_BLOCK
						&& g_ctx.grid[x][y].blocks[z] != INVALID_BLOCK) {
					pb = g_ctx.blocks[g_ctx.grid[x][y].blocks[z]].pb;
				}

				/* Calculate power of this CLB */
				power_usage_pb(&pb_power, pb, g_ctx.grid[x][y].type->pb_graph_head, g_ctx.grid[x][y].blocks[z]);
	...
	for (clock_idx = 0; clock_idx < clock_arch->num_global_clocks;
		...
		if (clock_arch->num_global_clocks == 1) {
			if (clock_arch->clock_inf[clock_idx].dens == 0) {
				...
				clock_arch->clock_inf[clock_idx].period = g_ctx.solution_inf.T_crit;
			}
		}
...
static void power_usage_clock_single(t_power_usage * power_usage,
	...
	C_segment = g_ctx.power_commonly_used->tile_length * single_clock->C_wire;
	if (single_clock->autosize_buffer) {
		buffer_size = 1 + C_segment / g_ctx.power_commonly_used->INV_1X_C_in;
	} else {
	...
	length += g_ctx.ny / 2 + 2 * g_ctx.nx;

	/* 3. Ribs - to */
	length += g_ctx.nx / 2 * g_ctx.ny;
...
static void power_usage_routing(t_power_usage * power_usage,
	...
	g_ctx.power_commonly_used->num_sb_buffers = 0;
	g_ctx.power_commonly_used->total_sb_buffer_size = 0.;
	g_ctx.power_commonly_used->num_cb_buffers = 0;
	g_ctx.power_commonly_used->total_cb_buffer_size = 0.;

	/* Reset rr graph net indices */
	for (rr_node_idx = 0; rr_node_idx < g_ctx.num_rr_nodes; rr_node_idx++) {
	...
	for (size_t net_idx = 0; net_idx < g_ctx.clbs_nlist.net.size(); net_idx++) {
		struct s_trace * trace;

		for (trace = g_ctx.trace_head[net_idx]; trace != NULL; trace = trace->next) {
	...
	for (size_t net_idx = 0; net_idx < g_ctx.clbs_nlist.net.size(); net_idx++) {
		...
		for (trace = g_ctx.trace_head[net_idx]; trace != NULL; trace = trace->next) {
			t_rr_node * node = &g_ctx.rr_nodes[trace->index];
			...
			for (edge_idx = 0; edge_idx < node->num_edges(); edge_idx++) {
				if (node->edge_sink_node(edge_idx) != OPEN) {
					t_rr_node * next_node = &g_ctx.rr_nodes[node->edge_sink_node(edge_idx)];
	...
	for (rr_node_idx = 0; rr_node_idx < g_ctx.num_rr_nodes; rr_node_idx++) {
		t_power_usage sub_power_usage;
		t_rr_node * node = &g_ctx.rr_nodes[rr_node_idx];
		...
		case CHANY:
			...
			C_wire =
					wire_length
							* segment_inf[g_ctx.rr_indexed_data[node->cost_index()].seg_index].Cmetal;
			//(double)g_ctx.power_commonly_used->tile_length);
			...
			power_usage_mux_multilevel(&sub_power_usage,
					power_get_mux_arch(node->fan_in(),
							g_ctx.power_arch->mux_transistor_size),
					node_power->in_prob, node_power->in_dens,
					node_power->selected_input, true, g_ctx.solution_inf.T_crit);
			power_add_usage(power_usage, &sub_power_usage);
			power_component_add_usage(&sub_power_usage,
					POWER_COMPONENT_ROUTE_SB);

			/* Buffer Size */
			switch (g_ctx.rr_switch_inf[node_power->driver_switch_type].power_buffer_type) {
			case POWER_BUFFER_TYPE_AUTO:
				/*
				 C_per_seg_split = ((float) node->num_edges
				 * g_ctx.power_commonly_used->INV_1X_C_in + C_wire);
				 // / (float) g_ctx.power_arch->seg_buffer_split;
				...
				buffer_size = power_calc_buffer_size_from_Cout(
						g_ctx.rr_switch_inf[node_power->driver_switch_type].Cout);
				break;
			case POWER_BUFFER_TYPE_ABSOLUTE_SIZE:
				buffer_size =
						g_ctx.rr_switch_inf[node_power->driver_switch_type].power_buffer_size;
				buffer_size = max(buffer_size, 1.0F);
				break;
			...
			g_ctx.power_commonly_used->num_sb_buffers++;
			g_ctx.power_commonly_used->total_sb_buffer_size += buffer_size;

			/*
			 g_ctx.power_commonly_used->num_sb_buffers +=
			 g_ctx.power_arch->seg_buffer_split;
			 g_ctx.power_commonly_used->total_sb_buffer_size += buffer_size
			 * g_ctx.power_arch->seg_buffer_split;
			 */
			...
			power_usage_wire(&sub_power_usage, C_wire,
					clb_net_density(node_power->net_num), g_ctx.solution_inf.T_crit);
			power_add_usage(power_usage, &sub_power_usage);
			power_component_add_usage(&sub_power_usage,
			...
			if (switchbox_fanout) {
				buffer_size = power_buffer_size_from_logical_effort(
						switchbox_fanout * g_ctx.power_commonly_used->NMOS_1X_C_d);
				power_usage_buffer(&sub_power_usage, buffer_size,
			...
			if (connectionbox_fanout) {
				...
				power_usage_buffer(&sub_power_usage, buffer_size,
						...
						false, g_ctx.solution_inf.T_crit);
				power_add_usage(power_usage, &sub_power_usage);
				power_component_add_usage(&sub_power_usage,
						POWER_COMPONENT_ROUTE_CB);

				g_ctx.power_commonly_used->num_cb_buffers++;
				g_ctx.power_commonly_used->total_cb_buffer_size += buffer_size;
			}
			break;
...
void power_pb_pins_init() {
	...
	for (type_idx = 0; type_idx < g_ctx.num_block_types; type_idx++) {
		if (g_ctx.block_types[type_idx].pb_graph_head) {
			power_init_pb_pins_rec(g_ctx.block_types[type_idx].pb_graph_head);
		}
	}
}
...
void power_routing_init(const t_det_routing_arch * routing_arch) {
	...
	if (!g_ctx.clb_net_power) {
		g_ctx.clb_net_power = (t_net_power*) vtr::calloc(g_ctx.clbs_nlist.net.size(), sizeof(t_net_power));
	}
	for (size_t net_idx = 0; net_idx < g_ctx.clbs_nlist.net.size(); net_idx++) {
		g_ctx.clb_net_power[net_idx].probability = g_ctx.atom_net_power[g_ctx.atom_lookup.atom_net(net_idx)].probability;
		g_ctx.clb_net_power[net_idx].density = g_ctx.atom_net_power[g_ctx.atom_lookup.atom_net(net_idx)].density;
	}

	/* Initialize RR Graph Structures */
	rr_node_power = (t_rr_node_power*) vtr::calloc(g_ctx.num_rr_nodes,
	...
	g_ctx.power_commonly_used->max_routing_mux_size = max_fanin;
	g_ctx.power_commonly_used->max_IPIN_fanin = max_IPIN_fanin;
	g_ctx.power_commonly_used->max_seg_to_seg_fanout = max_seg_to_seg_fanout;
	g_ctx.power_commonly_used->max_seg_to_IPIN_fanout = max_seg_to_IPIN_fanout;

#ifdef PRINT_SPICE_COMPARISON
	g_ctx.power_commonly_used->max_routing_mux_size =
	max(g_ctx.power_commonly_used->max_routing_mux_size, 26);
#endif
	...
	g_ctx.power_commonly_used->max_seg_fanout = max_seg_fanout;
}
...
bool power_init(const char * power_out_filepath,
	...
	g_ctx.power_arch = arch->power;
	g_ctx.power_commonly_used = new t_power_commonly_used;
	g_ctx.power_tech = (t_power_tech*) vtr::malloc(sizeof(t_power_tech));
	g_ctx.power_output = (t_power_output*) vtr::malloc(sizeof(t_power_output));

	/* Set up Logs */
	g_ctx.power_output->num_logs = POWER_LOG_NUM_TYPES;
	g_ctx.power_output->logs = (t_log*) vtr::calloc(g_ctx.power_output->num_logs,
			sizeof(t_log));
	g_ctx.power_output->logs[POWER_LOG_ERROR].name = vtr::strdup("Errors");
	g_ctx.power_output->logs[POWER_LOG_WARNING].name = vtr::strdup("Warnings");

	/* Initialize output file */
	if (!error) {
		g_ctx.power_output->out = NULL;
		g_ctx.power_output->out = vtr::fopen(power_out_filepath, "w");
		if (!g_ctx.power_output->out) {
			error = true;
		}
	}
...
bool power_uninit(void) {
	...
	for (std::map<float, t_power_mux_info*>::iterator it =
			g_ctx.power_commonly_used->mux_info.begin();
			it != g_ctx.power_commonly_used->mux_info.end(); it++) {
	...
	delete g_ctx.power_commonly_used;

	if (g_ctx.power_output->out) {
		fclose(g_ctx.power_output->out);
	}

	/* Free logs */
	for (log_idx = 0; log_idx < g_ctx.power_output->num_logs; log_idx++) {
		for (msg_idx = 0; msg_idx < g_ctx.power_output->logs[log_idx].num_messages;
				msg_idx++) {
			free(g_ctx.power_output->logs[log_idx].messages[msg_idx]);
		}
		free(g_ctx.power_output->logs[log_idx].messages);
		free(g_ctx.power_output->logs[log_idx].name);
	}
	free(g_ctx.power_output->logs);
	free(g_ctx.power_output);
...
static void power_print_clb_detailed(FILE * fp) {
	...
	for (type_idx = 0; type_idx < g_ctx.num_block_types; type_idx++) {
		...
		power_print_pb_usage_recursive(fp, g_ctx.block_types[type_idx].pb_type,
...
/*
 ...
 g_ctx.power_commonly_used->max_seg_fanout);
 fprintf(fp, "Max Segment->Segment Fanout: %d\n",
 g_ctx.power_commonly_used->max_seg_to_seg_fanout);
 fprintf(fp, "Max Segment->IPIN Fanout: %d\n",
 g_ctx.power_commonly_used->max_seg_to_IPIN_fanout);
 fprintf(fp, "Max IPIN fanin: %d\n", g_ctx.power_commonly_used->max_IPIN_fanin);
 fprintf(fp, "Average SB Buffer Size: %.1f\n",
 g_ctx.power_commonly_used->total_sb_buffer_size
 / (float) g_ctx.power_commonly_used->num_sb_buffers);
 fprintf(fp, "SB Buffer Transistors: %g\n",
 power_count_transistors_buffer(
 g_ctx.power_commonly_used->total_sb_buffer_size
 / (float) g_ctx.power_commonly_used->num_sb_buffers));
 fprintf(fp, "Average CB Buffer Size: %.1f\n",
 g_ctx.power_commonly_used->total_cb_buffer_size
 / (float) g_ctx.power_commonly_used->num_cb_buffers);
 fprintf(fp, "Tile length (um): %.2f\n",
 g_ctx.power_commonly_used->tile_length * CONVERT_UM_PER_M);
...
static void power_print_summary(FILE * fp, const t_vpr_setup& vpr_setup) {
	...
	fprintf(g_ctx.power_output->out, "Circuit: %s\n",
			vpr_setup.FileNameOpts.CircuitName);
	fprintf(g_ctx.power_output->out, "Architecture: %s\n", arch);
	fprintf(fp, "Technology (nm): %.0f\n",
			g_ctx.power_tech->tech_size * CONVERT_NM_PER_M);
	fprintf(fp, "Voltage: %.2f\n", g_ctx.power_tech->Vdd);
	fprintf(fp, "Temperature: %g\n", g_ctx.power_tech->temperature);
...
e_power_ret_code power_total(float * run_time_s, const t_vpr_setup& vpr_setup,
	...
	power_print_title(g_ctx.power_output->out, "Summary");
	power_print_summary(g_ctx.power_output->out, vpr_setup);

	/* Print Error & Warning Logs */
	output_logs(g_ctx.power_output->out, g_ctx.power_output->logs,
			g_ctx.power_output->num_logs);

	//power_print_title(g_ctx.power_output->out, "Statistics");
	//power_print_stats(g_ctx.power_output->out);

	power_print_title(g_ctx.power_output->out, "Power Breakdown");
	power_print_breakdown_summary(g_ctx.power_output->out);

	power_print_title(g_ctx.power_output->out, "Power Breakdown by PB");
	power_print_breakdown_pb(g_ctx.power_output->out);

	//power_print_title(g_ctx.power_output->out, "Spice Comparison");
	//power_print_spice_comparison();
	...
	*run_time_s = (float) (t_end - t_start) / CLOCKS_PER_SEC;
	...
	/* Return code */
	if (g_ctx.power_output->logs[POWER_LOG_ERROR].num_messages) {
		return POWER_RET_CODE_ERRORS;
	} else if (g_ctx.power_output->logs[POWER_LOG_WARNING].num_messages) {
...
static void power_print_breakdown_pb(FILE * fp) {
	...
	for (int type_idx = 0; type_idx < g_ctx.num_block_types; type_idx++) {
		if (g_ctx.block_types[type_idx].pb_type) {
			power_print_breakdown_pb_rec(g_ctx.power_output->out,
					g_ctx.block_types[type_idx].pb_type, 0);
		}
	}
	fprintf(fp, "\n");
}
...
static void power_print_breakdown_component(FILE * fp, const char * name,
	...
	power_print_breakdown_entry(fp, indent_level,
			...
			power_sum_usage(&g_ctx.power_by_component.components[type]),
			power_sum_usage(
					&g_ctx.power_by_component.components[POWER_COMPONENT_TOTAL]),
			power_perc_dynamic(&g_ctx.power_by_component.components[type]), NULL);
