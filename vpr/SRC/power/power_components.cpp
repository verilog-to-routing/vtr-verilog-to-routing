		...
		for (out_port_idx = 0;
			...
			for (pin_idx = 0;
				...
				if (pb) {
					...
					AtomNetId output_pin_net = cluster_ctx.clb_nlist.block_pb((BlockId)iblk)->pb_route[cluster_pin_idx].atom_net_id;
					if (!output_pin_net) {
						selected_input = 0;
					} else {
						for (in_port_idx = 0;
							...
							AtomNetId input_pin_net = cluster_ctx.clb_nlist.block_pb((BlockId)iblk)->pb_route[input_pin->pin_count_in_cluster].atom_net_id;
