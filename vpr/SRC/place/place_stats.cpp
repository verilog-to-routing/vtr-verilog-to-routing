...
print_relative_pos_distr(void)
{
	...
	total_conn = (int *)vtr::malloc((g_ctx.nx + g_ctx.ny + 1) * sizeof(int));
	relapos = (int **)vtr::malloc((g_ctx.nx + g_ctx.ny + 1) * sizeof(int *));
	relapos_distr = (double **)vtr::malloc((g_ctx.nx + g_ctx.ny + 1) * sizeof(double *));
	for (len = 0; len <= g_ctx.nx + g_ctx.ny; len++)
	{
	...
	for (inet = 0; inet < g_ctx.clbs_nlist.net.size(); inet++)
	{
		if (g_ctx.clbs_nlist.net[inet].is_global == false)
		{
			...
			src_x = g_ctx.blocks[g_ctx.clbs_nlist.net[inet].pins[0].g_ctx.blocks].x;
			src_y = g_ctx.blocks[g_ctx.clbs_nlist.net[inet].pins[0].g_ctx.blocks].y;

			for (sink_pin = 1; sink_pin < g_ctx.clbs_nlist.net[inet].pins.size();
					sink_pin++)
			{
				dst_x = g_ctx.blocks[g_ctx.clbs_nlist.net[inet].pins[sink_pin].g_ctx.blocks].x;
				dst_y = g_ctx.blocks[g_ctx.clbs_nlist.net[inet].pins[sink_pin].g_ctx.blocks].y;
	...
	for (len = 1; len <= g_ctx.nx + g_ctx.ny; len++)
	{
		if (total_conn[len] != 0)
		{
