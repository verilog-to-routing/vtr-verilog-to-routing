 ...
 * [0...g_ctx.num_block_types-1][0...num_pins-1]                                                */
...
static void find_all_the_macro (int * num_of_macro, int * pl_macro_member_blk_num_of_this_blk, 
	...
	for (iblk = 0; iblk < g_ctx.num_blocks; iblk++) {
		...
		num_blk_pins = g_ctx.blocks[iblk].type->num_pins;
		for (to_iblk_pin = 0; to_iblk_pin < num_blk_pins; to_iblk_pin++) {
			
			to_inet = g_ctx.blocks[iblk].nets[to_iblk_pin];
			to_idirect = f_idirect_from_blk_pin[g_ctx.blocks[iblk].type->index][to_iblk_pin];
			to_src_or_sink = f_direct_type_from_blk_pin[g_ctx.blocks[iblk].type->index][to_iblk_pin];
			...
			if ( to_src_or_sink == SINK && to_idirect != OPEN && to_inet == OPEN ) {
				...
				for (from_iblk_pin = 0; from_iblk_pin < num_blk_pins; from_iblk_pin++) {
					from_inet = g_ctx.blocks[iblk].nets[from_iblk_pin];
					from_idirect = f_idirect_from_blk_pin[g_ctx.blocks[iblk].type->index][from_iblk_pin];
					from_src_or_sink = f_direct_type_from_blk_pin[g_ctx.blocks[iblk].type->index][from_iblk_pin];
					...
					if ( from_src_or_sink == SOURCE && to_idirect == from_idirect && from_inet != OPEN) {
						...
						while (next_inet != OPEN) {
							...
							VTR_ASSERT(g_ctx.clbs_nlist.net[curr_inet].num_sinks() == 1);
							next_iblk = g_ctx.clbs_nlist.net[curr_inet].pins[1].block;
							
							// Assume that the from_iblk_pin index is the same for the next block
							VTR_ASSERT(f_idirect_from_blk_pin[g_ctx.blocks[next_iblk].type->index][from_iblk_pin] == from_idirect
									&& f_direct_type_from_blk_pin[g_ctx.blocks[next_iblk].type->index][from_iblk_pin] == SOURCE);
							next_inet = g_ctx.blocks[next_iblk].nets[from_iblk_pin];
...
int alloc_and_load_placement_macros(t_direct_inf* directs, int num_directs, t_pl_macro ** macros){
	 ...
	 * g_ctx.num_blocks^2 (There are big benckmarks VPR that have g_ctx.num_blocks *
	 * in the 100k's range).                                           *
	 *                                                                 *
	...
	pl_macro_num_members = (int *) vtr::calloc (g_ctx.num_blocks , sizeof(int));
	pl_macro_idirect = (int *) vtr::calloc (g_ctx.num_blocks , sizeof(int));
	pl_macro_member_blk_num = (int **) vtr::calloc (g_ctx.num_blocks , sizeof(int*));
	pl_macro_member_blk_num_of_this_blk = (int *) vtr::calloc (g_ctx.num_blocks , sizeof(int));
...
static void alloc_and_load_imacro_from_iblk(t_pl_macro * macros, int num_macros) {
	...
	temp_imacro_from_iblk = (int *)vtr::malloc(g_ctx.num_blocks * sizeof(int));
	for(iblk = 0; iblk < g_ctx.num_blocks; iblk ++) {
