...
static bool check_global_net_with_array(INP t_net* net_arr,
	...
	for(i = 0; i < num_net_arr; i++){
		if(strcmp(net_arr[i].name,g_nlist->nets[i].name))
			return false;
		if(net_arr[i].is_routed != g_nlist->nets[i].is_routed)
			return false;
		if(net_arr[i].is_fixed != g_nlist->nets[i].is_fixed)
			return false;
		if(net_arr[i].is_global != g_nlist->nets[i].is_global)
			return false;
		if(net_arr[i].is_const_gen != g_nlist->nets[i].is_const_gen)
			return false;

		for(j = 0; j <= net_arr[i].num_sinks; j++){
			if(net_arr[i].node_block[j] != g_nlist->nets[i].nodes[j].block)
				return false;

			if(net_arr[i].node_block_port)
				if(net_arr[i].node_block_port[j] != g_nlist->nets[i].nodes[j].block_port)
					return false;

			if(net_arr[i].node_block_pin[j] != g_nlist->nets[i].nodes[j].block_pin)
				return false;
		}
	
	}

	assert(num_net_arr == (int)g_nlist->nets.size());
