...
bool place_and_route(t_placer_opts placer_opts,
    ...
    if (!placer_opts.doPlacement || placer_opts.place_freq == PLACE_NEVER) {
        ...
        read_place(filename_opts.NetFile.c_str(), filename_opts.PlaceFile.c_str(), filename_opts.verify_file_digests, device_ctx.grid, cluster_ctx.clb_nlist.blocks().size());
        sync_grid_to_blocks();
    } else {
