...
void routing_stats(bool full_stats, enum e_route_type route_type,
	...
	area = 0;
	for (i = 1; i <= g_ctx.nx; i++) {
		for (j = 1; j <= g_ctx.ny; j++) {
			if (g_ctx.grid[i][j].width_offset == 0 && g_ctx.grid[i][j].height_offset == 0) {
				if (g_ctx.grid[i][j].type->area == UNDEFINED) {
					area += grid_logic_tile_area * g_ctx.grid[i][j].type->width * g_ctx.grid[i][j].type->height;
				} else {
					area += g_ctx.grid[i][j].type->area;
				}
			}
		}
	}
	...
	for (i = 0; i < g_ctx.num_blocks; i++) {
		if (g_ctx.blocks[i].type != g_ctx.IO_TYPE) {
			if (g_ctx.blocks[i].type->area == UNDEFINED) {
				used_area += grid_logic_tile_area * g_ctx.blocks[i].type->width * g_ctx.blocks[i].type->height;
			} else {
				used_area += g_ctx.blocks[i].type->area;
			}
		}
	}
    ...
    if (timing_analysis_enabled) {
        ...
        auto routing_delay_calc = std::make_shared<RoutingDelayCalculator>(g_ctx.atom_nl, g_ctx.atom_lookup, net_delay);
...
void length_and_bends_stats(void) {
	...
	for (inet = 0, l = g_ctx.clbs_nlist.net.size(); inet < l; inet++) {
		if (g_ctx.clbs_nlist.net[inet].is_global == false && g_ctx.clbs_nlist.net[inet].num_sinks() != 0) { /* Globals don't count. */
		...
		} else if (g_ctx.clbs_nlist.net[inet].is_global) {
			num_global_nets++;
		} else {
	...
	av_bends = (float) total_bends / (float) ((int) g_ctx.clbs_nlist.net.size() - num_global_nets);
	vtr::printf_info("\n");
	vtr::printf_info("Average number of bends per net: %#g  Maximum # of bends: %d\n", av_bends, max_bends);
	vtr::printf_info("\n");

	av_length = (float) total_length / (float) ((int) g_ctx.clbs_nlist.net.size() - num_global_nets);
	...
	av_segments = (float) total_segments / (float) ((int) g_ctx.clbs_nlist.net.size() - num_global_nets);
...
static void get_channel_occupancy_stats(void) {
	...
	int **chanx_occ; /* [1..g_ctx.nx][0..g_ctx.ny] */
	int **chany_occ; /* [0..g_ctx.nx][1..g_ctx.ny] */

	chanx_occ = vtr::alloc_matrix<int>(1, g_ctx.nx, 0, g_ctx.ny);
	chany_occ = vtr::alloc_matrix<int>(0, g_ctx.nx, 1, g_ctx.ny);
	...
	for (int j = 0; j <= g_ctx.ny; ++j) {
		total_x += g_ctx.chan_width.x_list[j];
		float ave_occ = 0.0;
		int max_occ = -1;
		...
		ave_occ /= g_ctx.nx;
		vtr::printf_info("                      %4d %7d %7.3f %8d\n", j, max_occ, ave_occ, g_ctx.chan_width.x_list[j]);
	}
	...
	for (int i = 0; i <= g_ctx.nx; ++i) {
		total_y += g_ctx.chan_width.y_list[i];
		float ave_occ = 0.0;
		int max_occ = -1;
		...
		ave_occ /= g_ctx.ny;
		vtr::printf_info("                      %4d %7d %7.3f %8d\n", i, max_occ, ave_occ, g_ctx.chan_width.y_list[i]);
	}
    ...
    vtr::free_matrix(chanx_occ, 1, g_ctx.nx, 0);
	vtr::free_matrix(chany_occ, 0, g_ctx.nx, 1);
}
...
static void load_channel_occupancies(int **chanx_occ, int **chany_occ) {
	...
	for (inet = 0, l = g_ctx.clbs_nlist.net.size(); inet < l; inet++) {
		...
		if (g_ctx.clbs_nlist.net[inet].is_global && g_ctx.clbs_nlist.net[inet].num_sinks() != 0) /* Skip global and empty nets. */
			continue;
		...
		while (tptr != NULL) {
			...
			if (rr_type == SINK) {
				...
				for (i = g_ctx.rr_nodes[inode].xlow(); i <= g_ctx.rr_nodes[inode].xhigh(); i++)
					chanx_occ[i][j]++;
			}

			else if (rr_type == CHANY) {
				i = g_ctx.rr_nodes[inode].xlow();
				for (j = g_ctx.rr_nodes[inode].ylow(); j <= g_ctx.rr_nodes[inode].yhigh(); j++)
					chany_occ[i][j]++;
			}
...
void get_num_bends_and_length(int inet, int *bends_ptr, int *len_ptr,
	...
	prevptr = g_ctx.trace_head[inet]; /* Should always be SOURCE. */
	if (prevptr == NULL) {
		vpr_throw(VPR_ERROR_OTHER, __FILE__, __LINE__,
				"in get_num_bends_and_length: net #%d has no traceback.\n", inet);
	}
	inode = prevptr->index;
	prev_type = g_ctx.rr_nodes[inode].type();

	tptr = prevptr->next;

	while (tptr != NULL) {
		inode = tptr->index;
		curr_type = g_ctx.rr_nodes[inode].type();
		...
		if (curr_type == SINK) { /* Starting a new segment */
			...
			curr_type = g_ctx.rr_nodes[tptr->index].type();
		}

		else if (curr_type == CHANX || curr_type == CHANY) {
			segments++;
			length += 1 + g_ctx.rr_nodes[inode].xhigh() - g_ctx.rr_nodes[inode].xlow()
					+ g_ctx.rr_nodes[inode].yhigh() - g_ctx.rr_nodes[inode].ylow();
...
void print_wirelen_prob_dist(void) {
	...
	prob_dist_size = g_ctx.nx + g_ctx.ny + 10;
	prob_dist = (float *) vtr::calloc(prob_dist_size, sizeof(float));
	norm_fac = 0.;

	for (inet = 0; inet < g_ctx.clbs_nlist.net.size(); inet++) {
		if (g_ctx.clbs_nlist.net[inet].is_global == false && g_ctx.clbs_nlist.net[inet].num_sinks() != 0) {
			get_num_bends_and_length(inet, &bends, &length, &segments);
...
void print_lambda(void) {
	...
	for (bnum = 0; bnum < g_ctx.num_blocks; bnum++) {
		...
		if (type != g_ctx.IO_TYPE) {
			for (ipin = 0; ipin < type->num_pins; ipin++) {
				...
				if (type->class_inf[iclass].type == RECEIVER) {
					inet = g_ctx.blocks[bnum].nets[ipin];
					if (inet != OPEN) /* Pin is connected? */
						if (g_ctx.clbs_nlist.net[inet].is_global == false) /* Not a global clock */
							num_inputs_used++;
				}
			}
		}
	...
	lambda = (float) num_inputs_used / (float) g_ctx.num_blocks;
...
int count_netlist_clocks(void) {
    ...
    for(auto blk_id : g_ctx.atom_nl.blocks()) {
        for(auto pin_id : g_ctx.atom_nl.block_clock_pins(blk_id)) {
            ...
            clock_names.insert(g_ctx.atom_nl.net_name(net_id));
        }
    }
