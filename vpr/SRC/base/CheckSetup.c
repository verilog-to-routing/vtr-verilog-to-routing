...
void CheckSetup(INP enum e_operation Operation,
	...
	if ((GLOBAL == RouterOpts.route_type)
		...
		vpr_printf(TIO_MESSAGE_ERROR, "The global router does not support timing-drvien "
		"routing.\n");
		exit(1);
	}
	...
	if (!PlacerOpts.doPlacement && (USER == PlacerOpts.pad_loc_type)) {
		vpr_printf(
				TIO_MESSAGE_ERROR, "A pad location file requires that placement is enabled.\n");
		exit(1);
	}
	...
	if (DETAILED == RouterOpts.route_type) {
		if ((Chans.chan_x_dist.type != UNIFORM)
			...
			vpr_printf(TIO_MESSAGE_ERROR, "Detailed routing currently only supported "
	...
	for (i = 0; i < RoutingArch.num_segment; ++i) {
		...
		if (FALSE == switch_inf[Tmp].buffered) {
			vpr_printf(TIO_MESSAGE_ERROR, "opin_switch (#%d) of segment type #%d "
			"is not buffered.\n", Tmp, i);
			exit(1);
		}
	}
