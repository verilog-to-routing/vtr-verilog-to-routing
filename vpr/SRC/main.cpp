...
int main(int argc, const char **argv) {
	...
	try {
        ...
        vtr::printf_info("Timing analysis took %g seconds (%g STA, %g slack) (%d full updates).\n", 
                            g_ctx.timing_analysis_profile_stats.timing_analysis_wallclock_time(), 
                            g_ctx.timing_analysis_profile_stats.sta_wallclock_time, 
                            g_ctx.timing_analysis_profile_stats.slack_wallclock_time, 
                            g_ctx.timing_analysis_profile_stats.num_full_updates);
#ifdef ENABLE_CLASSIC_VPR_STA
        vtr::printf_info("Old VPR Timing analysis took %g seconds (%g STA, %g delay annotitaion) (%d full updates).\n", 
                            g_ctx.timing_analysis_profile_stats.old_timing_analysis_wallclock_time(),
                            g_ctx.timing_analysis_profile_stats.old_sta_wallclock_time,
                            g_ctx.timing_analysis_profile_stats.old_delay_annotation_wallclock_time,
                            g_ctx.timing_analysis_profile_stats.num_old_sta_full_updates);
        vtr::printf_info("\tSTA       Speed-up: %.2fx\n", 
                            g_ctx.timing_analysis_profile_stats.old_sta_wallclock_time / g_ctx.timing_analysis_profile_stats.sta_wallclock_time);
        vtr::printf_info("\tSTA+Slack Speed-up: %.2fx\n", 
                            g_ctx.timing_analysis_profile_stats.old_timing_analysis_wallclock_time() / g_ctx.timing_analysis_profile_stats.timing_analysis_wallclock_time());
#endif
