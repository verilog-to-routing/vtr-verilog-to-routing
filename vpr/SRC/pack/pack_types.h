...
/* Describes different types of intra-logic g_ctx.blocks routing resource nodes */
enum e_lb_rr_type {
...
/* Stores statistical information for a physical g_ctx.blocks such as costs and usages */
typedef struct s_pb_stats {
	/* Packing statistics */
	std::map<AtomBlockId, float> gain; /* Attraction (inverse of cost) function */

	std::map<AtomBlockId, float> timinggain; /* The timing criticality score of this atom g_ctx.blocks. 
	                                            Determined by the most critical atom net 
                                                between this atom g_ctx.blocks and any atom g_ctx.blocks in 
                                                the current pb */
	std::map<AtomBlockId, float> connectiongain; /* Weighted sum of connections to attraction function */
	std::map<AtomBlockId, float> sharinggain; /* How many nets on an atom g_ctx.blocks are already in the pb under consideration */

	/* This is the gain used for hill-climbing. It stores*
	 * the reduction in the number of pins that adding this atom g_ctx.blocks to the the*
	 * current pb will have. This reflects the fact that sometimes the *
	 * addition of an atom g_ctx.blocks to a pb may reduce the number of inputs     *
	...
	/* Array of feasible blocks to select from [0..max_array_size-1] 
	 Sorted in ascending gain order so that the last g_ctx.blocks is the most desirable (this makes it easy to pop blocks off the list
	 */
...
/* Describes a routing resource node within a logic g_ctx.blocks type */
struct t_lb_type_rr_node {
	...
	enum e_lb_rr_type type;	/* Type of logic g_ctx.blocks resource node */	
...
/* A routing traceback data structure, provides a logic g_ctx.blocks instance specific trace lookup directly from the t_lb_type_rr_node array index 
...
/* Describes the status of a logic g_ctx.blocks routing resource node for a given logic g_ctx.blocks instance */
...
/* 
  Data structure forming the route tree of a net within one logic g_ctx.blocks.  
...
/* Represents a net used inside a logic g_ctx.blocks and the physical nodes used by the net */
...
/* Stores tuning parameters used by intra-logic g_ctx.blocks router */
...
struct t_expansion_node {
	int node_index;		/* Index of logic g_ctx.blocks rr node this expansion node represents */
	int prev_index;		/* Index of logic g_ctx.blocks rr node that drives this expansion node */
	float cost;
...
/* Stores all data needed by intra-logic g_ctx.blocks router */
struct t_lb_router_data {
	/* Physical Architecture Info */
    std::vector<t_lb_type_rr_node> *lb_type_graph;	/* Pointer to physical intra-logic g_ctx.blocks type rr graph */
	
	/* Logical Netlist Info */
    std::vector<t_intra_lb_net> *intra_lb_nets;		/* Pointer to vector of intra logic g_ctx.blocks nets and their connections */
	
	/* Saved nets */
    std::vector<t_intra_lb_net> *saved_lb_nets;		/* Save vector of intra logic g_ctx.blocks nets and their connections */
	...
	t_lb_rr_node_stats *lb_rr_node_stats;		/* [0..lb_type_graph->size()-1] Stats for each logic g_ctx.blocks rr node instance */
