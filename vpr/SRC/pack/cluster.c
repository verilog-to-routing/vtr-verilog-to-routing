...
for (ipin = 1; ipin <= vpack_net[inet].num_sinks; ipin++) {
	...
	if (logical_block[iblk].clb_index == NO_CLUSTER) {
		...
		cur_pb->pb_stats.connectiongain[iblk] += 1
				/ (float) (vpack_net[inet].num_sinks - num_internal_connections
						+ 1 + 1 * num_stuck_connections);
	}
}
}
...
if (logical_block[iblk].clb_index == NO_CLUSTER) {
	...
	cur_pb->pb_stats.connectiongain[iblk] += 1
			/ (float) (vpack_net[inet].num_sinks - num_internal_connections + 1
					+ 1 * num_stuck_connections);
}
}
}
...
/*****************************************/
static void mark_and_update_partial_gain(int inet, enum e_gain_update gain_flag,
int clustered_block, int port_on_clustered_block, int pin_on_clustered_block,
...
for (i = 0; i < cur_pb->pb_stats.num_marked_blocks; i++) {
	...
	if (timing_driven) {
		cur_pb->pb_stats.gain[iblk] = alpha * cur_pb->pb_stats.lengthgain[iblk]
...
mode = pb->mode;
for (i = 0;
i < pb_type->modes[mode].num_pb_type_children && pb->child_pbs != NULL; i++) {
for (j = 0;
j < pb_type->modes[mode].pb_type_children[i].num_pb && pb->child_pbs[i] != NULL;
j++) {
if (pb->child_pbs[i][j].name != NULL) {
...
compute_and_mark_lookahead_pins_used_for_pin(&pb_graph_node->clock_pins[0][0],
cur_pb, logical_block[ilogical_block].clock_net);
}
clock_port++;
...
for (i = 0;
skip == FALSE
&& i < output_pb_graph_pin->num_connectable_primtive_input_pins[depth]; i++) {
if (pb_graph_pin
...
* AAPACK_MAX_OVERUSE_LOOKAHEAD_PINS_FAC; i++) {
...
if (get_net_corresponding_to_pb_graph_pin(cur_pb,
	pb_graph_pin->list_of_connectable_input_pin_ptrs[depth][i]) == inet) {
count++;
}
}
...
for (i = 0; success && i < pb_type->modes[cur_pb->mode].num_pb_type_children;
i++) {
if (cur_pb->child_pbs[i] != NULL) {
for (j = 0;
success && j < pb_type->modes[cur_pb->mode].pb_type_children[i].num_pb; j++) {
