...
static void load_pin_class_by_depth(INOUTP t_pb_graph_node *pb_graph_node, INP int depth, OUTP int *count);
...
static void expand_pb_graph_node_and_load_pin_class_by_depth(INOUTP t_pb_graph_pin *current_pb_graph_pin, INP t_pb_graph_pin *reference_pb_graph_pin, INP int depth, OUTP int *count);
...
void load_pin_classes_in_pb_graph_head(INOUTP t_pb_graph_node *pb_graph_node) {
	...
	for(i = 0; i < depth; i++) {
		...
		load_pin_class_by_depth(pb_graph_node, i, &count);
	}
	...
	if(pb_graph_node->pb_type->num_modes == 0) {
		...
		for(i = 0; i < pb_graph_node->num_input_ports; i++) {
			for(j = 0; j < pb_graph_node->num_input_pins[i]; j++) {
				pb_graph_node->input_pins[i][j].parent_pin_class = (int *) my_calloc(pb_graph_node->pb_type->depth, sizeof(int*));
		...
		for(i = 0; i < pb_graph_node->num_output_ports; i++) {
			for(j = 0; j < pb_graph_node->num_output_pins[i]; j++) {
				pb_graph_node->output_pins[i][j].parent_pin_class = (int *) my_calloc(pb_graph_node->pb_type->depth, sizeof(int*));
				pb_graph_node->output_pins[i][j].list_of_connectable_input_pin_ptrs = (t_pb_graph_pin ***)my_calloc(pb_graph_node->pb_type->depth, sizeof(t_pb_graph_pin**));
				pb_graph_node->output_pins[i][j].num_connectable_primtive_input_pins = (int*)my_calloc(pb_graph_node->pb_type->depth, sizeof(int));
				for(k = 0; k < pb_graph_node->pb_type->depth; k++) {
		...
		for(i = 0; i < pb_graph_node->num_clock_ports; i++) {
			for(j = 0; j < pb_graph_node->num_clock_pins[i]; j++) {
				pb_graph_node->clock_pins[i][j].parent_pin_class = (int *) my_calloc(pb_graph_node->pb_type->depth, sizeof(int*));
				for(k = 0; k < pb_graph_node->pb_type->depth; k++) {
...
static void load_pin_class_by_depth(INOUTP t_pb_graph_node *pb_graph_node, INP int depth, OUTP int *count) {
			...
			for(i = 0; i < pb_graph_node->num_input_ports; i++) {
				for(j = 0; j < pb_graph_node->num_input_pins[i]; j++) {
					if(pb_graph_node->input_pins[i][j].parent_pin_class[depth] == OPEN) {
						expand_pb_graph_node_and_load_pin_class_by_depth(&pb_graph_node->input_pins[i][j], &pb_graph_node->input_pins[i][j], depth, count);	
						(*count)++;
					}
				}
			}
			for(i = 0; i < pb_graph_node->num_output_ports; i++) {
				for(j = 0; j < pb_graph_node->num_output_pins[i]; j++) {
					if(pb_graph_node->output_pins[i][j].parent_pin_class[depth] == OPEN) {
						expand_pb_graph_node_and_load_pin_class_by_depth(&pb_graph_node->output_pins[i][j], &pb_graph_node->output_pins[i][j], depth, count);	
						(*count)++;
					}
				}
			}
			for(i = 0; i < pb_graph_node->num_clock_ports; i++) {
				for(j = 0; j < pb_graph_node->num_clock_pins[i]; j++) {
					if(pb_graph_node->clock_pins[i][j].parent_pin_class[depth] == OPEN) {
						expand_pb_graph_node_and_load_pin_class_by_depth(&pb_graph_node->clock_pins[i][j], &pb_graph_node->clock_pins[i][j], depth, count);	
						(*count)++;
					}
				}
			}
	...
	for(i = 0; i < pb_graph_node->pb_type->num_modes; i++) {
		for(j = 0; j < pb_graph_node->pb_type->modes[i].num_pb_type_children; j++) {
			for(k = 0; k < pb_graph_node->pb_type->modes[i].pb_type_children[j].num_pb; k++) {
				load_pin_class_by_depth(&pb_graph_node->child_pb_graph_nodes[i][j][k], depth, count);
			}
		}
	}
	...
	if(pb_graph_node->pb_type->depth == depth && pb_graph_node->pb_type->num_modes != 0) {
		...
		pb_graph_node->num_pin_classes = *count;
		pb_graph_node->pin_class_size = (int*) my_calloc(*count, sizeof(int));
...
static void load_list_of_connectable_input_pin_ptrs(INOUTP t_pb_graph_node *pb_graph_node) {
	...
	if(pb_graph_node->pb_type->num_modes == 0) {
		...
		for(i = 0; i < pb_graph_node->num_output_ports; i++) {
			for(j = 0; j < pb_graph_node->num_output_pins[i]; j++) {
				for(k = 0; k < pb_graph_node->pb_type->depth; k++) {
					expand_pb_graph_node_and_load_output_to_input_connections(&pb_graph_node->output_pins[i][j], &pb_graph_node->output_pins[i][j], k);	
...
static void expand_pb_graph_node_and_load_output_to_input_connections(INOUTP t_pb_graph_pin *current_pb_graph_pin, INOUTP t_pb_graph_pin *reference_pin, INP int depth) {
	...
	if(current_pb_graph_pin->scratch_pad == OPEN && current_pb_graph_pin->parent_node->pb_type->depth > depth) {
		...
		if(current_pb_graph_pin->parent_node->pb_type->num_modes == 0 && current_pb_graph_pin->port->type == IN_PORT) {
			...
			reference_pin->list_of_connectable_input_pin_ptrs[depth] = my_realloc(reference_pin->list_of_connectable_input_pin_ptrs[depth], reference_pin->num_connectable_primtive_input_pins[depth] * sizeof(t_pb_graph_pin*));
			reference_pin->list_of_connectable_input_pin_ptrs[depth][reference_pin->num_connectable_primtive_input_pins[depth] - 1] = current_pb_graph_pin;
		}
	}
}
...
static void expand_pb_graph_node_and_load_pin_class_by_depth(INOUTP t_pb_graph_pin *current_pb_graph_pin, INP t_pb_graph_pin *reference_pb_graph_pin, INP int depth, OUTP int *count) {
	...
	if(current_pb_graph_pin->scratch_pad != *count) {
		if(current_pb_graph_pin->parent_node->pb_type->num_modes == 0) {
			current_pb_graph_pin->scratch_pad = *count;
			...
			if( current_pb_graph_pin->parent_pin_class[depth] == OPEN &&
				...
				current_pb_graph_pin->parent_pin_class[depth] = *count;				
			}
			for(i = 0; i < current_pb_graph_pin->num_input_edges; i++) {
				assert(current_pb_graph_pin->input_edges[i]->num_input_pins == 1);
				expand_pb_graph_node_and_load_pin_class_by_depth(current_pb_graph_pin->input_edges[i]->input_pins[0], reference_pb_graph_pin, depth, count);
			}
			for(i = 0; i < current_pb_graph_pin->num_output_edges; i++) {
				assert(current_pb_graph_pin->output_edges[i]->num_output_pins == 1);
				expand_pb_graph_node_and_load_pin_class_by_depth(current_pb_graph_pin->output_edges[i]->output_pins[0], reference_pb_graph_pin, depth, count);
			}
			...
			if(current_pb_graph_pin->port->type == OUT_PORT) {
				if(reference_pb_graph_pin->port->type == OUT_PORT) {
					...
					current_pb_graph_pin->pin_class = *count;
				}
				for(i = 0; i < current_pb_graph_pin->num_input_edges; i++) {
					assert(current_pb_graph_pin->input_edges[i]->num_input_pins == 1);
					expand_pb_graph_node_and_load_pin_class_by_depth(current_pb_graph_pin->input_edges[i]->input_pins[0], reference_pb_graph_pin, depth, count);
...
static void sum_pin_class(INOUTP t_pb_graph_node *pb_graph_node) {
	...
	for(i = 0; i < pb_graph_node->num_input_ports; i++) {
		for(j = 0; j < pb_graph_node->num_input_pins[i]; j++) {
			assert(pb_graph_node->input_pins[i][j].pin_class < pb_graph_node->num_pin_classes);
			...
			pb_graph_node->pin_class_size[pb_graph_node->input_pins[i][j].pin_class]++;
		}
	}
	for(i = 0; i < pb_graph_node->num_output_ports; i++) {
		for(j = 0; j < pb_graph_node->num_output_pins[i]; j++) {
			assert(pb_graph_node->output_pins[i][j].pin_class < pb_graph_node->num_pin_classes);
			...
			pb_graph_node->pin_class_size[pb_graph_node->output_pins[i][j].pin_class]++;
		}
	}
	for(i = 0; i < pb_graph_node->num_clock_ports; i++) {
		for(j = 0; j < pb_graph_node->num_clock_pins[i]; j++) {
			assert(pb_graph_node->clock_pins[i][j].pin_class < pb_graph_node->num_pin_classes);
			...
			pb_graph_node->pin_class_size[pb_graph_node->clock_pins[i][j].pin_class]++;
		}
	}	
}
