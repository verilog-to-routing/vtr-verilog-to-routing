...
void print_routing_in_clusters(FILE *fpout, ClusterBlockId iclb);
...
void print_atom_block(FILE *fpout, AtomBlockId atom_blk) {
	...
	pb_route = cluster_ctx.clb_nlist.block_pb((ClusterBlockId)clb_index)->pb_route;
    ...
    if (atom_ctx.nlist.block_type(atom_blk) == AtomBlockType::INPAD) {
		...
		fprintf(fpout, ".names %s clb_%d_rr_node_%d\n", atom_ctx.nlist.block_name(atom_blk).c_str(), clb_index, node_index);
		fprintf(fpout, "1 1\n\n");
		...
		for (int i = 0; i < pb_type->num_ports; i++) {
			if (pb_type->ports[i].type == IN_PORT) {
				...
				*/
                ...
                AtomPortId port_id = atom_ctx.nlist.BaseNetlist::find_port(atom_blk, pb_type->ports[i].name);
                if(port_id) {
		...
		for (int i = 0; i < pb_type->num_ports; i++) {
			if (pb_type->ports[i].type == OUT_PORT) {
                ...
                auto port_id = atom_ctx.nlist.BaseNetlist::find_port(atom_blk, pb_type->ports[i].name);
		...
		for (int i = 0; i < pb_type->num_ports; i++) {
			if (pb_type->ports[i].type == OUT_PORT) {
				...
				fprintf(fpout, "\n.names %s clb_%d_rr_node_%d\n", atom_ctx.nlist.net_name(net_id).c_str(), clb_index, node_index);
				fprintf(fpout, "1 1\n");
			}
		}
	} else {
		...
		while (port != NULL) {
            AtomPortId port_id = atom_ctx.nlist.BaseNetlist::find_port(atom_blk, port->name);
        ...
        while (port != NULL) {
            ...
            for (int ipin = 0; ipin < port->size; ipin++) {
                ...
                if (net_id) {
                    ...
                    fprintf(fpout, ".names %s clb_%d_rr_node_%d\n", atom_ctx.nlist.net_name(net_id).c_str(), clb_index, pb_graph_pin->pin_count_in_cluster);
                    fprintf(fpout, "1 1\n\n");
                }
            }
            port = port->next;
...
void print_routing_in_clusters(FILE *fpout, ClusterBlockId iclb) {
	...
	pb_graph_pin_lookup = alloc_and_load_pb_graph_pin_lookup_from_index(cluster_ctx.clb_nlist.block_type(iclb));
	pb_graph_node = cluster_ctx.clb_nlist.block_pb(iclb)->pb_graph_node;
	max_pb_graph_pin = pb_graph_node->total_pb_pins;
	pb_route = cluster_ctx.clb_nlist.block_pb(iclb)->pb_route;
