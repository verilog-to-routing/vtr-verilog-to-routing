...
static void print_net_name(AtomNetId net_id, int *column, FILE * fpout) {
	...
	/* This routine prints out the g_ctx.atom_nl net name (or open) and limits the    *
	 * length of a line to LINELENGTH characters by using \ to continue *
	 * lines.  net_id is the id of the g_ctx.atom_nl net to be printed, while     *
	...
	if (!net_id) {
		...
		str_ptr = new char[strlen(g_ctx.atom_nl.net_name(net_id).c_str()) + 7];
		sprintf(str_ptr, "__|e%s", g_ctx.atom_nl.net_name(net_id).c_str());
	}
...
void print_atom_block(FILE *fpout, AtomBlockId atom_blk, t_block *clb) {
	...
	clb_index = g_ctx.atom_lookup.atom_clb(atom_blk);
	VTR_ASSERT(clb_index != OPEN);
	pb_route = clb[clb_index].pb_route;
	VTR_ASSERT(pb_route != NULL);
	pb_graph_node = g_ctx.atom_lookup.atom_pb_graph_node(atom_blk);
	pb_type = pb_graph_node->pb_type;


    if (g_ctx.atom_nl.block_type(atom_blk) == AtomBlockType::INPAD) {
		int node_index = pb_graph_node->output_pins[0][0].pin_count_in_cluster;
		fprintf(fpout, ".names %s clb_%d_rr_node_%d\n", g_ctx.atom_nl.block_name(atom_blk).c_str(), clb_index, node_index);
		fprintf(fpout, "1 1\n\n");
    } else if (g_ctx.atom_nl.block_type(atom_blk) == AtomBlockType::OUTPAD) {
        ...
        auto input_pins = g_ctx.atom_nl.block_input_pins(atom_blk);
        VTR_ASSERT(input_pins.size() == 1);
        auto input_pin = *input_pins.begin();
        auto input_net = g_ctx.atom_nl.pin_net(input_pin);

        const char* outpad_input_net = g_ctx.atom_nl.net_name(input_net).c_str();
        const char* trimmed_outpad_name = g_ctx.atom_nl.block_name(atom_blk).c_str() + 4;
		if (strcmp(outpad_input_net, trimmed_outpad_name) != 0) {
			fprintf(fpout, ".names clb_%d_rr_node_%d %s\n", clb_index, node_index, trimmed_outpad_name);
			fprintf(fpout, "1 1\n\n");
		}
	}
	else if(strcmp(g_ctx.atom_nl.block_model(atom_blk)->name, "names") == 0) {
		...
		for (int i = 0; i < pb_type->num_ports; i++) {
			if (pb_type->ports[i].type == IN_PORT) {
				...
				*/
                ...
                AtomPortId port_id = g_ctx.atom_nl.find_port(atom_blk, pb_type->ports[i].name);
                if(port_id) {
                    for (int j = 0; j < pb_type->ports[i].num_pins; j++) {
                        AtomPinId pin_id = g_ctx.atom_nl.port_pin(port_id, j);
                        if(pin_id) {
                            ...
                            if(k == pb_type->ports[i].num_pins) {
                                ...
                                vpr_throw(VPR_ERROR_PACK, __FILE__, __LINE__,
                                    ...
                                    g_ctx.atom_nl.block_name(atom_blk).c_str(), g_ctx.atom_nl.net_name(net_id).c_str());
                            }
                        }
                    }
                }
		...
		for (int i = 0; i < pb_type->num_ports; i++) {
			if (pb_type->ports[i].type == OUT_PORT) {
                ...
                auto port_id = g_ctx.atom_nl.find_port(atom_blk, pb_type->ports[i].name);
                auto pin_id = g_ctx.atom_nl.port_pin(port_id, 0);
                VTR_ASSERT(pin_id);
                VTR_ASSERT(g_ctx.atom_nl.pin_net(pin_id) == pb_route[node_index].atom_net_id);

				fprintf(fpout, "%s\n", g_ctx.atom_nl.block_name(atom_blk).c_str());
			}
		}

        //Print the truth table
        const auto& truth_table = g_ctx.atom_nl.block_truth_table(atom_blk);
	...
	} else if (strcmp(g_ctx.atom_nl.block_model(atom_blk)->name, "latch") == 0) {
		...
		for (int i = 0; i < pb_type->num_ports; i++) {
			if (pb_type->ports[i].type == IN_PORT
                ...
                auto input_pins = g_ctx.atom_nl.block_input_pins(atom_blk);
                VTR_ASSERT(input_pins.size() == 1);
                VTR_ASSERT_MSG(g_ctx.atom_nl.pin_net(*input_pins.begin()), "Valid input net");
                ...
                auto output_pins = g_ctx.atom_nl.block_output_pins(atom_blk);
                VTR_ASSERT(output_pins.size() == 1);
                auto output_net_id = g_ctx.atom_nl.pin_net(*output_pins.begin());
                VTR_ASSERT_MSG(output_net_id, "Valid output net");


				fprintf(fpout, "%s re ", g_ctx.atom_nl.net_name(output_net_id).c_str());
			} else if (pb_type->ports[i].type == IN_PORT
					&& pb_type->ports[i].is_clock == true) {
				VTR_ASSERT(pb_type->ports[i].num_pins == 1);

                auto clock_pins = g_ctx.atom_nl.block_clock_pins(atom_blk);
                VTR_ASSERT(clock_pins.size() == 1);
                VTR_ASSERT_MSG(g_ctx.atom_nl.pin_net(*clock_pins.begin()), "Valid clock net");
		...
		for (int i = 0; i < pb_type->num_ports; i++) {
			if (pb_type->ports[i].type == OUT_PORT) {
				...
				VTR_ASSERT(pb_type->ports[i].num_pins == 1);
                AtomPortId port_id = g_ctx.atom_nl.find_port(atom_blk, pb_type->ports[i].name);
                auto net_id = g_ctx.atom_nl.port_net(port_id, 0);
				VTR_ASSERT(net_id == pb_route[node_index].atom_net_id);
				fprintf(fpout, "\n.names %s clb_%d_rr_node_%d\n", g_ctx.atom_nl.net_name(net_id).c_str(), clb_index, node_index);
				fprintf(fpout, "1 1\n");
			}
		}
	} else {
		const t_model *cur = g_ctx.atom_nl.block_model(atom_blk);
		...
		while (port != NULL) {
            AtomPortId port_id = g_ctx.atom_nl.find_port(atom_blk, port->name);
			for (int i = 0; i < port->size; i++) {
				...
				if (port->is_clock == true) {
                    ...
                    VTR_ASSERT(g_ctx.atom_nl.port_type(port_id) == AtomPortType::CLOCK);
                }

                AtomNetId net_id = g_ctx.atom_nl.port_net(port_id, i);

                //Output the net
                if(net_id) {
                    fprintf(fpout, "%s", g_ctx.atom_nl.net_name(net_id).c_str());
                } else {
		...
		while (port != NULL) {
			...
			for (int i = 0; i < port->size; i++) {
                ...
                if(net_id) {
                    fprintf(fpout, "%s[%d]=%s ", port->name, i, g_ctx.atom_nl.net_name(net_id).c_str());
                } else {
        ...
        while (port != NULL) {
            ...
            for (int ipin = 0; ipin < port->size; ipin++) {
                ...
                if (net_id) {
                    ...
                    fprintf(fpout, ".names %s clb_%d_rr_node_%d\n", g_ctx.atom_nl.net_name(net_id).c_str(), clb_index, pb_graph_pin->pin_count_in_cluster);
                    fprintf(fpout, "1 1\n\n");
                }
            }
            port = port->next;
...
void output_blif (const t_arch *arch, t_block *clb, int num_clusters, const char *out_fname) {
	...
	column = 0;
	fprintf(fpout, ".model %s\n", g_ctx.atom_nl.netlist_name().c_str());
	...
	for (auto blk_id : g_ctx.atom_nl.blocks()) {
		if (g_ctx.atom_nl.block_type(blk_id) == AtomBlockType::INPAD) {
			print_string(g_ctx.atom_nl.block_name(blk_id).c_str(), &column, fpout);
		}
	}
	...
	for (auto blk_id : g_ctx.atom_nl.blocks()) {
		if (g_ctx.atom_nl.block_type(blk_id) == AtomBlockType::OUTPAD) {
			...
			print_string(g_ctx.atom_nl.block_name(blk_id).c_str() + 4, &column, fpout);
		}
	}
