...
std::unique_ptr<SetupTimingInfo> make_setup_timing_info(std::shared_ptr<DelayCalc> delay_calculator) {
    ...
    std::shared_ptr<tatum::SetupTimingAnalyzer> analyzer = tatum::AnalyzerFactory<tatum::SetupAnalysis>::make(*g_ctx.timing_graph, *g_ctx.timing_constraints, *delay_calculator);

     return std::unique_ptr<ConcreteSetupTimingInfo<DelayCalc>>(
             new ConcreteSetupTimingInfo<DelayCalc>(g_ctx.timing_graph, g_ctx.timing_constraints, delay_calculator, analyzer)
            );
}

template<class DelayCalc>
std::unique_ptr<HoldTimingInfo> make_hold_timing_info(std::shared_ptr<DelayCalc> delay_calculator) {
    std::shared_ptr<tatum::HoldTimingAnalyzer> analyzer = tatum::AnalyzerFactory<tatum::HoldAnalysis>::make(*g_ctx.timing_graph, *g_ctx.timing_constraints, *delay_calculator);

     return std::unique_ptr<ConcreteHoldTimingInfo<DelayCalc>>(
             new ConcreteHoldTimingInfo<DelayCalc>(g_ctx.timing_graph, g_ctx.timing_constraints, delay_calculator, analyzer)
            );
}

template<class DelayCalc>
std::unique_ptr<SetupHoldTimingInfo> make_setup_hold_timing_info(std::shared_ptr<DelayCalc> delay_calculator) {
    std::shared_ptr<tatum::SetupHoldTimingAnalyzer> analyzer = tatum::AnalyzerFactory<tatum::SetupHoldAnalysis>::make(*g_ctx.timing_graph, *g_ctx.timing_constraints, *delay_calculator);

     return std::unique_ptr<ConcreteSetupHoldTimingInfo<DelayCalc>>(
             new ConcreteSetupHoldTimingInfo<DelayCalc>(g_ctx.timing_graph, g_ctx.timing_constraints, delay_calculator, analyzer)
            );
}
