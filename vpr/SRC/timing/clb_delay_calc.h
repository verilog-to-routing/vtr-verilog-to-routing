...
class ClbDelayCalc {
    public:
        ...
        float clb_input_to_internal_sink_delay(const ClusterBlockId block_id, const int pin_index, int internal_sink_pin) const;

        //Delay from an internal driver to the CLB's output pin
        float internal_src_to_clb_output_delay(const ClusterBlockId block_id, const int pin_index, int internal_src_pin) const;

        //Delay from an internal driver to an internal sink within the same CLB
        float internal_src_to_internal_sink_delay(const ClusterBlockId clb, int internal_src_pin, int internal_sink_pin) const;

    private:
        float trace_max_delay(ClusterBlockId clb, int src_pb_route_pin, int sink_pb_route_pin) const;

        float pb_route_max_delay(ClusterBlockId clb, int pb_route_idx) const;
