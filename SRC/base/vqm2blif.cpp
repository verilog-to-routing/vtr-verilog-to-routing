...
void 	init_blif_subckts (	t_node **vqm_nodes, 
	...
	t_node* temp_node;		//struct from vqm_dll.h

	//Housekeeping initializations before the subcircuits can be assigned.
...
void subckt_housekeeping(t_model* cur_model){
/*  Accomplishes three housekeeping tasks before the subcircuits of a blif_model can be initialized.
 *
 *	ARGUMENTS
 *  cur_model:
 *	Beginning of a Linked List containing all model information from the Architecture file.
 */
...
//TASK 2: Allocate and initialize the model_count array (global).
	model_count = (int*)malloc((num_models)*sizeof(int));
 ...
 */
	...
	string search;		//temporary variable to construct a mode-hashed block name within.
	...
	if (!cur_model){	
		...
		model_count[cur_model->index]++;	//indicate this model should appear in the BLIF
 ...
 */	
	while(temp_model){
		if (model_count[temp_model->index] > 0){
			...
			//that are flagged as "used" in the model_count array
			if (!debug){
