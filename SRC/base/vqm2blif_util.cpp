...
/* Identifies if a given node in the VQM File can be elaborated into a BLIF-Format LUT.
 * 
 *	ARGUMENTS
 *  vqm_node:
 *	Node read from the VQM File to be analyzed. 
 *
 *	NOTE: This function currently only elaborates a specifically configured stratixiv_lcell_comb.
 */
	bool isLut = true;
	...
	if (strcmp(vqm_node->type, "stratixiv_lcell_comb") != 0){
		isLut = false;
	} else {
		for (int i = 0; i < vqm_node->number_of_params; i++){
