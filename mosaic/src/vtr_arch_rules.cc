...
// vtr_arch_rules Yosys pass: generate arch-specific mapping rules for the MOSAIC
// flow at synthesis time directly from the VTR arch xml.

// thin cli over the rule-generator registry in arch_rule_gen.{h,cc}. each enabled
// generator substitutes arch-derived values into template files (-tpldir) and
// writes rule files into -outdir, including bram_memory_map.txt, tech_bram.v,
// vtr_hardblock_lib.v, mult_map.v, mul2dsp_map.v, add_sub_map.v, and one
// <model>_map.v per -exotic combinational block.

// files are required because Yosys consumers (memory_libmap -lib, techmap -map,
// read_verilog -lib) only accept file paths. the abc -luts cost spec is not
// emitted here. it lives as the lutCost knob in synthesis.tcl.

// arch facts (mode addr/data widths, multiply modes, hardblock presence) come
// from the xml. flow policy (libmap costs) arrives as pass options from the
// synthesis.tcl knob block.
USING_YOSYS_NAMESPACE
PRIVATE_NAMESPACE_BEGIN

// HELPER: derive a short arch name from the xml file path for output naming.
...
// HELPER: join integers for log messages.
...
// HELPER: format port width maps for -list-modes debug output.
...
// HELPER: print scanned bram and hardblock mode geometry for -list-modes.
void listScannedModes(const mosaic::VtrArchInfo &info) {
    ...
    for (const auto &m : info.bramModes) {
        log("  %s %s addrA=%d dataA=%d addrB=%d dataB=%d\n", m.isSp ? "sp" : "dp", m.name.c_str(), m.addrBitsA,
            m.dataBitsA, m.addrBitsB, m.dataBitsB);
    }
    ...
    for (const auto &kv : info.hardblockModes) {
        ...
        for (const auto &mode : kv.second) {
            log("    opmode{%s} pb=%s in={%s} out={%s}\n", mode.modeQualifier.c_str(), mode.pbTypeName.c_str(),
                joinPortWidths(mode.inputWidths).c_str(), joinPortWidths(mode.outputWidths).c_str());
        }
    }
}

// HELPER: apply one -alias <role>=<model> mapping to ClassicModelNames.
void applyClassicAlias(mosaic::ClassicModelNames &classic, const std::string &arg) {
...
struct VtrArchRulesPass : public Pass {
    VtrArchRulesPass() : Pass("vtr_arch_rules", "generate arch mapping rules from a vtr arch xml") {}
    ...
    void execute(std::vector<std::string> args, RTLIL::Design *) override {
        ...
        // default enables all built-ins, and exotic generators always run because
        // they were explicitly requested on the command line.
        const std::set<std::string> builtinBlocks = {"bram", "adder", "multiply", "hardblock-lib"};
        ...
        log("vtr_arch_rules: %s: %zu bram modes, %zu hardblockModes (%zu models), "
            ...
            archName.c_str(), info.bramModes.size(), hardblockModeCount, info.hardblockModes.size(),
            info.multiplyModes.empty() ? "-" : joinInts(info.multiplyModes, ",").c_str(),
        ...
        log("vtr_arch_rules: arch facts: vtrRamAbits=%d dspMaxWidth=%d "
            ...
            maxRamAbits, multiplyPresent ? info.multiplyModes.back() : 0, multiplyPresent ? 1 : 0,
            info.adder.present ? 1 : 0);
        ...
        for (size_t i = 0; i < exoticRoleModels.size(); ++i) {
            ...
            gens.push_back(mosaic::makeRoleRuleGen(request, &roleMapEmittedPaths[i]));
        }

        // emit enabled generators in registry order and collect combinational-kind
        // generators as stub providers for hardblock-lib. when stub-all-exotics is
        // set, per-model -exotic generators still emit techmaps but skip stub
        // contribution so the lib does not get duplicate modules.
        ...
        if (stubAllHardblocks) {
            ...
            for (const auto &kv : info.hardblockModels) {
                if (kv.first == policy.classic.multiply || kv.first == policy.classic.adder ||
                    kv.first == policy.classic.singlePortRam || kv.first == policy.classic.dualPortRam)
                    continue;
                exoticCount++;
            }
        ...
        if (enabled.count("hardblock-lib"))
            mosaic::makeHardblockLibGen(stubProviders)->emit(info, policy, outDir);
    }
} VtrArchRulesPass;
