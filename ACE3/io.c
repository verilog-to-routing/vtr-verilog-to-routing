	...
	Abc_NtkForEachObj(ntk, obj, i)
	{
		...
		if (name && strcmp(name, "unconn"))
		{
			if (fp != NULL)
			{
				...
				fprintf (fp, "%s %f %f %f\n", name, info->static_prob, info->switch_prob, info->switch_act);

			}
			else
			{
				printf ("%s %f %f %f\n", Abc_ObjName(obj), info->static_prob, info->switch_prob, info->switch_act);
			}
		}
	}
}
...
int ace_io_read_activity (Abc_Ntk_t * ntk, FILE * in_file_desc, ace_pi_format_t pi_format, double p, double d, const char * clk_name)
{
	int error = 0;
	int i;
	Abc_Obj_t * obj_ptr;
	...
	else
	{
		...
		if (pi_format == ACE_ACT)
		{
			...
			{
				sscanf(line, "%s %lf %lf\n", pi_name, &static_prob, &switch_prob);

				pi_obj_id = Nm_ManFindIdByName(name_manager, pi_name, ABC_OBJ_PI);
				if (pi_obj_id == -1)
				{
		...
		else if (pi_format == ACE_VEC)
		{
			...
			while(!feof(in_file_desc))
			{
				...
				{
					printf ("Error: vector length (%d) doesn't match number of inputs (%d)\n", strlen(vector), num_Pi);
					error =  ACE_ERROR;
					break;
				}
