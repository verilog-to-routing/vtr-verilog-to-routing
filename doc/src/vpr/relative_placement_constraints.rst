...
* **Interaction with carry chains:** a carry chain may overlap a relative macro, but its atoms may only be listed in one group. If the chain spans several clusters, those clusters are pulled into the macro at the offsets fixed by the chain, and the whole shape moves as one rigid macro.
