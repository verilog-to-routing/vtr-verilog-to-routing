...
Lines written by VPR (:option:`vpr --write_flat_place`) carry three additional columns:
the cluster block number the atom was packed into, the primitive type it was placed on,
and the path of that primitive:
