...
.. option:: -temp_dir <path>
    ...
    Temporary directory for files generated by a set of tasks.
    The script will automatically create this directory if necessary.

    **Default:** ``config/..`` for each task being run

    Specifies the parent directory for this set of tasks, which will contain ``<task_name>/run<#>`` directories.
