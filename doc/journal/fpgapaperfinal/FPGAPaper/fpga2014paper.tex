...
\title{Making Multiple Dice Work as One: CAD Flows and Routing Architectures for Silicon Interposer FPGAs}
%
%
...
Enabling the enhancements described in Section~\ref{cad_enh_placer_subsection} improves the minimum routable channel width and circuit speed. As shown in Figure~\ref{fig:comparison_minW}, for architectures with low \textit{\% wires cut} (high interposer capacity), the placer optimizations are only marginally beneficial compared to the original VPR placer since the interposer wires are not scarce; but as more wires are cut and we reach the realistic interposer capacity of commercial FPGAs, the placer optimizations provide higher and higher reduction in minimum channel width. When 80\% of wires are cut, the placer optimizations result in 11.3\% reduction in minimum channel width.
...
The optimizations also help the placer to place related entities on the same die to reduce the number of inter-die signal crossings and hence increase circuit speed. At 80\% wires cut, the placer optimizations result in 8.4\% higher circuit speed.
\begin{figure}[!htbp]
\centering
...
\begin{figure}[!h]
\centering
\includegraphics[width=\linewidth]{arch_experiments3.eps}
...
To examine the impact of the number of dice used to construct an interposer-based FPGA, we measured the minimum channel width for an FPGA device with four dice, varying the fraction of wires cut. We used the best CAD flow from Section \ref{sec:partitioner_stage_results} and the best architecture parameters from this section (TODO JAVEED -- spell them out?). The results, shown in Figure \ref{fig:wires_cut_4part}, show the same trend as Figure \ref{fig:wires_cut}.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth]{wires_cut_4part.eps}
\caption{Impact of partitioning-based CAD flow on routability for a range of architectures with 4 dice. TODO JAVEED -- LU32 90\% result is missing}
\label{fig:wires_cut_4part}
\end{figure}

TODO JAVEED -- show critical path delay? (it's uninteresting)
...
TODO: Javeed
We investigated the implications of the device aspect ratio on routability for the four-partition case. In particular, we compared a device with a square interposer (four rectangular dice) to a device with a rectangular interposer (four square dice.) We used the best CAD flow from Section \ref{sec:partitioner_stage_results} and the best architecture parameters from this section. The results are shown in Table \ref{table:squares}.
...
geomean & 140.7481367 & 251.805728 \\ \hline
\end{tabular}
\caption{Minimum channel width for square interposer and square dice configurations.}
\label{table:squares}
\end{table}

The square interposer configuration performs significantly better than the square dice configuration.

TODO JAVEED -- Vaughn commented that we should talk about which is easier to manufacture. I think the square dice device would actually be easier to manufacture, because the more complex dice will fit well on a round wafer. Isn't it fine for the interposer to be non-square?
