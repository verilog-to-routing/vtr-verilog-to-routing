...
TODO: Javeed
We investigated the implications of the device aspect ratio on routability for the four-partition case. In particular, we compared a device with a square interposer (four rectangular dice) to a device with a rectangular interposer (four square dice), as shown in \ref{fig:square_tall}. We used the best CAD flow from Section \ref{sec:partitioner_stage_results} and the best architecture parameters from this section. The results are shown in Table \ref{table:squares}.

\begin{figure}[!htbp]
\centering
\subfigure[]{\includegraphics[height = 5cm]{square_fpga.pdf}}
\subfigure[]{\includegraphics[height = 5cm]{tall_fpga.pdf}}
