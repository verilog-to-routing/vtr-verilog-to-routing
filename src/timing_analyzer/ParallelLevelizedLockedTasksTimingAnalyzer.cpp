...
void ParallelLevelizedLockedTasksTimingAnalyzer::calculate_timing(TimingGraph& timing_graph) {
...
void ParallelLevelizedLockedTasksTimingAnalyzer::create_locks(TimingGraph& tg) {
    node_locks_ = std::vector<omp_lock_t>(tg.num_nodes());
}

void ParallelLevelizedLockedTasksTimingAnalyzer::init_locks() {
#pragma omp for
...
void ParallelLevelizedLockedTasksTimingAnalyzer::cleanup_locks() {
#pragma omp for
    for(int i = 0; i < (int) node_locks_.size(); i++) {
...
void ParallelLevelizedLockedTasksTimingAnalyzer::pre_traversal(TimingGraph& timing_graph) {
    /*
...
#pragma omp single
    for(int level_idx = 0; level_idx < timing_graph.num_levels(); level_idx++) {
        ...
        for(int i = 0; i < (int) level_ids.size(); i++) {
            int level_id = level_ids[i];
#pragma omp task shared(timing_graph)
            pre_traverse_node(timing_graph, level_id, level_idx);
        }
    }
}

void ParallelLevelizedLockedTasksTimingAnalyzer::forward_traversal(TimingGraph& timing_graph) {
...
void ParallelLevelizedLockedTasksTimingAnalyzer::backward_traversal(TimingGraph& timing_graph) {
...
void ParallelLevelizedLockedTasksTimingAnalyzer::pre_traverse_node(TimingGraph& tg, TimingGraph::NodeId node_id, int level_idx) {
...
void ParallelLevelizedLockedTasksTimingAnalyzer::forward_traverse_node(TimingGraph& tg, TimingGraph::NodeId node_id) {
...
void ParallelLevelizedLockedTasksTimingAnalyzer::backward_traverse_node(TimingGraph& tg, TimingGraph::NodeId node_id) {
