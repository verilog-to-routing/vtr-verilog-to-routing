...
//int verify_analyzer(const TimingGraph& tg, const std::shared_ptr<TimingAnalyzer<SetupTraversal<Traversal>>> analyzer, const VprArrReqTimes& expected_arr_req_times, const std::set<NodeId>& const_gen_fanout_nodes, const std::set<NodeId>& clock_gen_fanout_nodes);
