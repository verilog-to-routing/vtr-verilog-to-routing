...
flags:
    ...
    --flows <names...>     frankenstein and/or vanilla_vtr (default: both)
...
def stageLutCounts(tempDir: Path, design: str) -> Dict[str, str]:
    ...
    for key, names in (
        ("synth_luts", (f"{design}.frankenstein.blif", f"{design}.parmys.blif")),
        ...
        for name in names:
            ...
            if value:
                counts[key] = value
                break
    # frankenstein abc is in-yosys so pre-vpr is the post-abc netlist; when
    # only frankenstein.blif exists, treat that as synth and leave abc blank
    if counts["synth_luts"] and not counts["abc_luts"]:
        preVpr = tempDir / f"{design}.pre-vpr.blif"
        frank = tempDir / f"{design}.frankenstein.blif"
        if preVpr.is_file() and frank.is_file():
            counts["abc_luts"] = countBlifNames(preVpr)
    for logName in ("frankenstein.out", "parmys.out"):
    ...
    if not counts["synth_ff"]:
        synthBlif = tempDir / f"{design}.frankenstein.blif"
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if not args.watch:
        print(
            f"watch:   python3 frankenstein/scripts/watch_compare.py --dir {outDir}"
        )
    print(
