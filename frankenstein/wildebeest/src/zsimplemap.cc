...
void zsimplemap_bitop(RTLIL::Module *module, RTLIL::Cell *cell) {
  ...
  if (cell->type != ID($bweqx)) {
    sig_a.extend_u0(GetSize(sig_y),
                    cell->parameters.at(ID::A_SIGNED).as_bool());
    sig_b.extend_u0(GetSize(sig_y),
                    cell->parameters.at(ID::B_SIGNED).as_bool());
  }
...
void zsimplemap_reduce(RTLIL::Module *module, RTLIL::Cell *cell) {
  ...
  if (sig_y.size() > 1) {
    module->connect(RTLIL::SigSig(sig_y.extract(1, sig_y.size() - 1),
                                  RTLIL::SigSpec(0, sig_y.size() - 1)));
...
void zsimplemap_eqne(RTLIL::Module *module, RTLIL::Cell *cell) {
  ...
  RTLIL::SigSpec xor_out =
      module->addWire(NEW_ID, max(GetSize(sig_a), GetSize(sig_b)));
  RTLIL::Cell *xor_cell =
      module->addXor(NEW_ID, sig_a, sig_b, xor_out, is_signed);
...
struct ZSimplemapPass : public Pass {
  ...
  void help() override {
    ...
    log("\n");
    log("This pass maps a small selection of simple coarse-grain cells to "
        "yosys gate\n");
    log("primitives. The following internal cell types are mapped by this "
        "pass:\n");
    log("\n");
    log("  $not, $pos, $and, $or, $xor, $xnor\n");
    log("  $reduce_and, $reduce_or, $reduce_xor, $reduce_xnor, $reduce_bool\n");
    log("  $logic_not, $logic_and, $logic_or, $mux, $tribuf\n");
    log("  $sr, $ff, $dff, $dffe, $dffsr, $dffsre, $adff, $adffe, $aldff, "
        "$aldffe, $sdff,\n");
