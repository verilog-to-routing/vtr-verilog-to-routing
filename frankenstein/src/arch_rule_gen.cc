...
public:
  ...
  void emit(const VtrArchInfo &info, const ArchRulePolicy &policy,
    ...
    // ram abits knob consumed as VTR_RAM_ABITS by vtr_ram_whitebox.v /
    // vtr_ram_bit_lib.v (-D at read time). keeps the rtl whitebox pad and
    // the -lib stubs at the arch ram addr width instead of a hardcoded k6.
    std::ostringstream knobs;
    knobs << "# generated by vtr_arch_rules from " << policy.archName << "\n"
          << "set vtrRamAbits " << maxAbits << "\n";
    writeFile(outDir + "/vtr_ram_knobs.tcl", knobs.str());
  }
};
...
// built-in always-fail map for archs without an adder model: $add/$sub
// stay soft (not part of the k6 byte-parity target, so it needs no
// template).
