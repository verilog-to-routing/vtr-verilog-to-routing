...
def xCLB(k_LUT, N_BLE, I_CLB, I_BLE, fracture_level, num_FF, crossbar_str):
    ...
    for frac_stage in reversed(list(range(frac_stages))):
        ...
        if special_stage:
            ...
            xprop(
                "input",
                "soft_logic.in[" + str(I_soft - 1) + ":" + str(I_soft - LUT_size_special) + "]",
            )
...
def gen_arch(
    dir, k_LUT, N_BLE, I_CLB, I_BLE, fracture_level, num_FF, seg_length, tech_nm, crossbar_str
):
    global f
    global tabs
