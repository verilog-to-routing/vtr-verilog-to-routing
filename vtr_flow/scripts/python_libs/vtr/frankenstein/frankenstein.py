"""
Module to run Frankenstein (the vendored wildebeest yosys plugin with the
...
# supported input file types by the frankenstein yosys template
FILE_TYPES = {
    ".v": "Verilog",
    ".vh": "Verilog",
    ".sv": "SystemVerilog",
...
def run(
    ...
    log_filename="frankenstein.out",
    yosys_exec=None,
    frankenstein_script=None,
):
    """
    Runs Frankenstein (Yosys + wildebeest plugin) on the specified
    architecture file and circuit

    .. note :: Usage: vtr.frankenstein.run(<architecture_file>,<circuit_file>,<output_netlist>,[OPTIONS])
    ...
    ----------------
        ...
        frankenstein_script :
            Custom frankenstein yosys template script (defaults to
            vtr_flow/misc/frankenstein/template/synthesis.tcl)

    """
    temp_dir = Path(temp_dir) if not isinstance(temp_dir, Path) else temp_dir
    temp_dir.mkdir(parents=True, exist_ok=True)

    if frankenstein_args is None:
    ...
    if circuit_file.suffix not in FILE_TYPES:
        raise vtr.VtrError(
            "Frankenstein expects an HDL input file {} (got '{}')".format(
    ...
    if not plugin_path.is_file():
        raise vtr.VtrError(
            ...
            + "build it with: bash {}".format(vtr.paths.frankenstein_build_script_path)
        )

    fix_blif_script = Path(vtr.paths.frankenstein_fix_blif_script_path)
    if not fix_blif_script.is_file():
        raise vtr.VtrError("fix_blif_for_vpr.py not found at {}".format(fix_blif_script))

    if frankenstein_script is None:
        base_script = str(vtr.paths.frankenstein_script_path)
    else:
        base_script = str(Path(frankenstein_script).resolve())

    # Copy the template script into the run directory
    yosys_script = "synthesis_frankenstein.tcl"
    ...
    top_module = frankenstein_args.pop("top_module", "") or ""
    ...
    for arg, value in frankenstein_args.items():
    ...
    if not raw_netlist.is_file():
        raise vtr.VtrError(
            "Frankenstein did not produce {} (see {})".format(raw_netlist_name, log_filename)
        )
    ...
    command_runner.run_system_command(
        ...
        temp_dir=temp_dir,
        log_filename="frankenstein_fix_blif.out",
        indent_depth=1,
    )
