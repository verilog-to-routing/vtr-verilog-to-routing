...
"""Generate NxN clock sector architecture variants from the base
'k6_frac_N10_frac_chain_mem32K_clk_2_by_2.xml' architecture.

This is used to sweep clock sector size on VTR's flagship architecture,
similar in spirit to Intel Stratix-10's sectored clock network: the device
is divided into an NxN grid of independently-driven clock sectors, so
larger N means more (smaller) sectors and smaller N means fewer (larger)
ones. A single sparse clock_switch_grid at the top of the clock hierarchy
drives one spine and one rib per sector, each sector's rib feeding the
CLOCK pins within its own region. See the 2x2 architecture for the
hand-written version this pattern is based on.
...
HEADER_COMMENT_FMT = """
  This architecture is VTR's flagship architecture (k6_frac_N10_frac_chain_mem32K_40nm.xml)
  with a {n}x{n} grid of clock sectors added, similar in spirit to Intel Stratix-10's
  sectored clock network. The device is divided into {n}x{n} = {n_sq} independently-driven
  sectors; each sector has its own spine/rib pair fed from a shared, sparse clock_switch_grid.

  Generated by clock_sectors/generate_clock_sector_archs.py; do not edit by hand.
"""
...
def qname(col: int, row: int) -> str:
    """Format a sector identifier for use in switch_point/clock_network names.
...
def build_switch_grid(n: int, denom: int) -> Any:
    ...
    Args:
        n: Grid size (the network divides the device into an NxN sector grid).
    ...
    grid.append(comment("Tap into each sector's spine."))
...
def build_spine(col: int, row: int, n: int, denom: int) -> Any:
    """Build one sector's spine clock_network element.
...
def build_rib(col: int, row: int, n: int, denom: int) -> Any:
    """Build one sector's rib clock_network element.
...
def build_clock_routing(n: int, denom: int) -> Any:
    ...
    routing.append(comment("Connect the clock switch grid to the sector spines."))
    for row in range(1, n + 1):
        for col in range(1, n + 1):
            etree.SubElement(
                routing,
    ...
    routing.append(comment("Connect each spine to its own sector's rib."))
    for row in range(1, n + 1):
        for col in range(1, n + 1):
            etree.SubElement(
                routing,
...
def build_clocknetworks(n: int) -> Any:
    """Build the full <clocknetworks> element for an NxN clock sector grid.
    ...
    clocknetworks.append(comment("Create the ribs for each sector, driven from their centers."))
    for row in range(1, n + 1):
        for col in range(1, n + 1):
...
def generate_arch(n: int, template_path: str, output_dir: str, parser: Any) -> None:
    """Generate one NxN clock sector architecture variant from the base template.
    ...
    # The template's leading "why this architecture exists" comment is
    # specific to its own sector count, so replace it with one describing
    # this variant's NxN grid instead of carrying the template's text over.
    header = xml_root.getprevious()
    if header is not None:
        header.text = HEADER_COMMENT_FMT.format(n=n, n_sq=n * n)
...
if __name__ == "__main__":
    ...
    generate_clock_sector_archs([4, 8, 16, 32], os.path.join(script_dir, TEMPLATE_NAME), script_dir)
