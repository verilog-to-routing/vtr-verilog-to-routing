...
for top_dir in *;do 
	if [ -d "$top_dir" ]; then 
		...
		for inner_dir in $top_dir/*;do
			...
			(time $RUN_SCRIPT $inner_dir) >>~/Desktop/run_results_power.txt  2>>~/Desktop/timing_results_power.txt
			 	
		done
		echo "$outer_dir"
		if [[ "$check" == "1" ]]; then
			echo "$top_dir" >>~/Desktop/timing_results_power.txt
			(time $RUN_SCRIPT $top_dir) >>~/Desktop/run_results_power.txt  2>>~/Desktop/timing_results_power.txt
