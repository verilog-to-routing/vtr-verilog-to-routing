...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Test building VTR for multiple different compilers and build configurations"
    )
...
def build_config(args, targets, config):
    ...
    with open(log_file, "w") as f:
        ...
        if "CMAKE_TOOLCHAIN_FILE" in config["CMAKE_PARAMS"]:
            ...
            print(
                " Toolchain={}".format(config["CMAKE_PARAMS"]["CMAKE_TOOLCHAIN_FILE"]),
                end=" ",
                file=f,
            )
        print("")
