...
def parse_args():
    parser = argparse.ArgumentParser(
        description="VTR uses external tools/libraries which are developed outside of the VTR source tree. Some of these are included in the VTR source tree using git's subtree feature. This script is used to update git-subtrees in a consistent manner. It must be run from the root of the VTR source tree."
    )
    ...
    parser.add_argument("-m", "--message", help="Commit comment when updating a component")

    exclusive_group = parser.add_mutually_exclusive_group()
    exclusive_group.add_argument(
        "--list", action="store_true", default=False, help="List known components"
    )

    exclusive_group.add_argument(
        "--update", action="store_true", default=False, help="Update components subtrees"
    )
...
def update_component(args, subtree_info):
    ...
    print(
        "Component: {:<15} Path: {:<30} URL: {:<45} URL_Ref: {}".format(
            subtree_info.name, subtree_info.internal_path, subtree_info.external_url, external_ref
        )
    )
    ...
    assert (
        args.message
    ), "Must provide a commit message (-m/--message) describing why component is being updated"
