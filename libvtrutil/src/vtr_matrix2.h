...
//
//Since this expansion happens at compiler time all the proxy classes get optimized away, high performance
//despite the generality.
//
...
class Matrix {
    public:
        ...
        Matrix(std::array<size_t,N> dim_sizes, T value=T()) {
        ...
        //      ...
        //with optional fill value
        Matrix(std::array<DimRange,N> dim_ranges, T value=T())
            : dim_ranges_(dim_ranges) {
            alloc();
            fill(value);
        }
    public: //Accessors
        ...
        const MatrixProxy<T,N-1> operator[](size_t index) const {
            VTR_ASSERT_SAFE_MSG(index >= dim_ranges_[0].begin_index(), "Index out of range (below dimension minimum)");
            VTR_ASSERT_SAFE_MSG(index < dim_ranges_[0].end_index(), "Index out of range (above dimension maximum)");

            //The elements are stored in zero-indexed form, so adjust for any 
            //non-zero minimum index in this dimension
            size_t effective_index = index - dim_ranges_[0].begin_index();

            //Calculate the stride for the current dimension
            size_t dim_stride = size() / dim_size(0);

            //Calculate the stride for the next dimension
            size_t next_dim_stride = dim_stride / dim_size(1);

            //Peel off the first dimension
            return MatrixProxy<T,N-1>(dim_ranges_.data(),                        //Pass the dimension information
                                      1,                                         //Pass the next dimension
                                      next_dim_stride,                           //Pass the stride for the next dimension
                                      data_.get() + dim_stride*effective_index); //Advance to index in this dimension
        }
    ...
    public: //Lifetime management
        ...
        friend void swap(Matrix<T,N>& m1, Matrix<T,N>& m2) {
            using std::swap;
